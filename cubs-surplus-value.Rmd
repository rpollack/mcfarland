...
team_surplus_value <- warcels %>%
  ...
  summarize(`Avg. Years of Control of Roster` = round(mean(years_of_control), 2),
            `Avg. Surplus Value per Year (Millions USD)` = sum(controlled_surplus_value) / sum(years_of_control))

ggplot(team_surplus_value, aes(`Avg. Years of Control of Roster`, `Avg. Surplus Value per Year (Millions USD)`, Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + ggtitle("MLB Controlled Surplus Value, 2017 - 2021") + fgt
...
cubs_surplus_value <- warcels %>%
  ...
  select(Name = FullName, `Years of Control` = years_of_control, `WAR Under Control` = war_under_control,
         `Salary Under Control*` = salary_under_control, `Surplus Value Under Control*` =
           surplus_value_under_control, `Surplus Value MLB Rank (%)` = surplus_value_under_control_rank) %>%
...
rank_90th_percentile <- warcels %>%
  filter(surplus_value_under_control_rank >= 90.0) %>%
  group_by(Team) %>%
  summarize(`No. Players Above 90th Percentile for Controlled Surplus Value` = n()) %>%
  arrange(desc(`No. Players Above 90th Percentile for Controlled Surplus Value`)) %>%
...
```{r}
rank_75th_percentile <- warcels %>%
  filter(surplus_value_under_control_rank >= 75.0) %>%
  group_by(Team) %>%
  ...
  write_csv("rank-75th.csv")


rank_75th_percentile
```
