...
cubs_tbl <- warcels %>%
  group_by(Team) %>%
  summarize(`Controlled Surplus Value Through 2021*` = sum(surplus_value_under_control)) %>%
...
ggplot(team_surplus_value, aes(avg_years_of_control, avg_surplus_value_per_year, Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="Avg. no. of years team controls its current players", y="Avg surplus value of roster per year (Millions USD)") + ggtitle("MLB Team Surplue Value Under Control, 2017 - 2021") + xlim(2.5,5) + fgt

for_fg <- team_surplus_value %>%
  select(avg_years_of_control, avg_surplus_value_per_year) %>%
