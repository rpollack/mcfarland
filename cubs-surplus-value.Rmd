...
To project WAR, I used a method Tom Tango proposed that weights the past three years' WAR and applies an aging curve. The details are here [http://tangotiger.com/index.php/site/comments/war-marcels-...-warcels]. The method isn't perfect, particularly when it comes to pitchers, but I trust Tango's methodology enough to go forward. For missing years I gave the player 0 WAR.
...
Cubs win! They control 133 WAR, nearly 20 more than the Dodgers who control 114. That's just an average four wins per year, but because the teams play in different divisions, Theo Epstein is probably fine with this situation.

The closest NL Central rival to the Cubs is the Cardinals, who control 84 WAR through this same time period. John Mozeliak has his work cut out for him, as do all the other NL Central GMs. Here's a chart showing just the NL Central teams:

```{r}
nl_central_war <- war_tbl %>%
  filter(Team %in% c("CHC", "STL", "PIT", "CIN", "MIL")) %>%
  write_csv("nl-central-controlled-war-thru-2021.csv")
...
ggplot(data=war_timeline, aes(avg_years_of_control, avg_war_under_control_pear_year, color=Team)) + geom_point() + geom_text(aes(label=Team), hjust=-0.01, vjust=0) + labs(x="Avg. Length of Control of Roster (Years)", y="Avg. Roster WAR per Year") + ggtitle("MLB Controlled WAR Over Time, 2017 – 2021") + fgt +
...
The results again paint Theo Epstein's group in a favorable light. For the next three years or so, the Cubs project to average over 40 WAR per year as a team. That's a projected average of 93 wins each year. And remember, this analysis doesn't have the team picking up Anthony Rizzo's options (which they surely will). Nor does it have them extending Kris Bryant or any other young stars, which they'll surely attempt to do.
...
- I assumed $535k for all pre-arbitration salaries unless otherwise listed. I did not step up the salary each year as the latest CBA dictates. Some later entries, like the Cubs rewarding its young stars, aren't reflected in the dataset.
...
salary_tbl <- warcels %>%
  ...
  arrange(desc(`Controlled Salary Through 2021`)) %>%
...
Now we get to the true art of the Cubs' rebuild: surplus value. I've shown how the Cubs project to have the most team WAR in baseball over the next five years. While the salary analysis appears to show them spending a lot, the reality is they've spent wisely.

For each player I calculated their surplus value under control by setting the value of 1 WAR equal to $8.5 million and inflating it by 5% each year. The following chart shows the results:
...
surplus_tbl <- warcels %>%
  ...
  select(`Controlled Surplus Value Through 2021`, Team) %>%
...
This graph shows how successful Epstein's rebuild has been. The Cubs are running the highest-value roster in the sport, with just over $800M in surplus value over the next five years. 

At nearly $270M in surplus value by himself, more than many entire teams, Kris Bryant accounts for the vast majority of this value. His contract, such as it is, ranks #1 in baseball by surplus value. Teammates Addison Russell and Anthony Rizzo join him near the top of the MLB rankings.  

```{r}
cubs_warcels <- warcels %>%
  filter(Team == "CHC") %>%
  select(Name, `Years of Control` = years_of_control, `WAR Under Control` = war_under_control, 
         `Salary Under Control` = salary_under_control, `Surplus Value Under Control` = controlled_surplus_value,
         `Surplus Value Rank` = surplus_value_rank) %>%
  mutate(`Years of Control` = as.integer(`Years of Control`, 0)) %>%
  arrange(desc(`Surplus Value Rank`)) %>%
  write_csv("cubs-surplus-value-players.csv")

cubs_warcels
```

Six Cubs rank at the 90th percentile or above, more than any other team.     

```{r}
rank_90th_percentile <- warcels %>%
  filter(surplus_value_rank >= 90.0) %>%
  group_by(Team) %>%
  summarize(`No. Players >= 90th Percentile for Surplus Value` = n()) %>%
  arrange(desc(`No. Players >= 90th Percentile for Surplus Value`)) %>%
  write_csv("rank-90th.csv")

ggplot(rank_90th_percentile, aes(reorder(Team, `No. Players >= 90th Percentile for Surplus Value`), `No. Players >= 90th Percentile for Surplus Value`)) + geom_bar(stat = "identity") + coord_flip()
