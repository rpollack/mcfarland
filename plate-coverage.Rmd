...
fg_info <- fg_db %>%
  tbl("player_info") %>%
  dplyr::select(PlayerId, FirstName, LastName, BISPosition) %>%
  collect(n=Inf)
...
high_achievers <- plate_coverage_raw %>%
  filter(wrc_plus >= 160,
         game_year < 2020) %>%
  ungroup() %>%
  dplyr::select(-zCoverage) %>%
  mutate(wrc_plus = round(wrc_plus, 0)) %>%
  summarize(mean_dist_from_optimal = mean(dist_from_optimal))

normies <- plate_coverage_raw %>%
  filter(wrc_plus < 160,
         game_year < 2020) %>%
  ungroup() %>%
  dplyr::select(-zCoverage) %>%
...
```{r}
top_10_zone_2019 <- plate_coverage_raw %>%
  filter(game_year == 2019) %>%
  dplyr::select(Name, `Plate Coverage (sq. in)` = total_coverage) %>%
  arrange(desc(`Plate Coverage (sq. in)`))

ggplot(top_10_zone_2019 %>% head(10), aes(reorder(Name, `Plate Coverage (sq. in)`), `Plate Coverage (sq. in)`)) + geom_bar(stat="identity") + coord_flip()  + labs(x="") + ggtitle("Largest Plate Coverage, 2019") + fgt
```
...
plate_coverage_raw %>%
  ...
  arrange(desc(`Plate Coverage (sq. in)`)) %>%
  tail(10) %>%
  ggplot(aes(reorder(Name, `Plate Coverage (sq. in)`), -`Plate Coverage (sq. in)`)) + geom_bar(stat="identity") + coord_flip()  + labs(x="") + ggtitle("Smallest Plate Coverage, 2016") + fgt
```
...
ggplot(plate_coverage_2016, aes(total_coverage, wrc_plus)) + geom_point() + 
  ...
  ggtitle("Plate Coverage vs. wRC+, 2015 - 2019") + labs(y="wRC+", x="Plate Coverage (sq. in)") + fgt
...
peak_2016 <- plate_coverage_raw %>%
  filter(game_year == 2019) %>%
  ungroup() %>%
  dplyr::select(Name, dist_from_optimal, wrc_plus) %>%
  mutate(wrc_plus = round(wrc_plus, 0)) %>%
...
plate_coverage_change_best <- plate_coverage_change %>%
  dplyr::select(Name, optim_diff, wrc_diff) %>%
  arrange(optim_diff) 
...
peraza_yoy <- savant_data_plate_coverage %>%
  filter(game_year >= 2016,
         Name == "Jose Peraza") %>%
  dplyr::select(Name, game_year, px_in, pz_in)
...
```{r}
ggplot(plate_coverage_raw %>% filter(game_year == 2019), aes(total_coverage, wrc_plus)) + geom_point() + geom_smooth(method="loess")  + 
  ggtitle("Plate Coverage vs. wRC+, 2019") + labs(y="wRC+", x="Plate Coverage (sq. in)")
```
