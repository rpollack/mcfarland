...
player_ranks <-
  ...
  filter(!is.na(TeamId)) %>%    # remove guys who changed teams in a year
...
what we want is, for training data, fetch historical data.

for current-season data, fetch projected data. 

where can i get projected team finishes?

but we'll need to do the same transforms on each and same kinds of fetches for each. 

So, perhaps just one fetch, distinguished by Type (0 or Steamer Update), and use
...
mvp_predictions %>%
  ...
  ggtitle(glue("Predicted MVP Voting, {year_to_predict}"),
    subtitle = glue("Based on stats through {Sys.Date()-days(1)} and projected Steamer full-season WAR")
  )
```
