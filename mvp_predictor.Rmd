...
master_df <-
  ...
  dplyr::select(playerid, player_name, Season, League, team, contains("rank"), contains("mvp"))
...
```{r}
master_df %>%
  ggplot(aes(league_war_rank, mvp_points)) +
  geom_point() +
  facet_wrap(vars(Season, League))
```
...
mvp_model_test <-
  ...
  # remove variables not used in model
