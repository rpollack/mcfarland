...
player_data <-
  ...
  mutate(proj_war = if_else(Type == 0 & Season == year_to_predict, lead(WAR), -1),
         proj_avg = if_else(Type == 0 & Season == year_to_predict, lead(batting_average), -1),
         proj_hr = if_else(Type == 0 & Season == year_to_predict, lead(home_runs), -1L),
         proj_rbi = if_else(Type == 0 & Season == year_to_predict, lead(rbi), -1L),
         proj_ops = if_else(Type == 0 & Season == year_to_predict, lead(ops), -1),
         proj_sb = if_else(Type == 0 & Season == year_to_predict, lead(stolen_bases), -1L)) %>%
  filter(Type == 0) %>%
  dplyr::select(-Type)
...
# TODO: non-pitcher names to handle: AJ Pollock, Carlos Ruiz, Dee Strange-Gordon, Hyun Jin Ryu, Michael Morse?
...
# TODO: handle guys who change teams in a year. put them on their final team. 

# TODO: update to use pre-season STEAMER projections (Type = -105. after the season is over, there are no steamer in-season projections (Type = 1103). 
...
player_ranks <-
  ...
  # for the current year, use Steamer-projected ranks
  mutate(
    league_ops_rank = if_else(Season == year_to_predict, min_rank(-proj_ops), min_rank(-ops)),
    league_hr_rank = if_else(Season == year_to_predict, min_rank(-proj_hr), min_rank(-home_runs)),
    league_sb_rank = if_else(Season == year_to_predict, min_rank(-proj_sb), min_rank(-stolen_bases)),
    league_avg_rank = if_else(Season == year_to_predict, min_rank(-proj_avg), min_rank(-batting_average)),
    league_rbi_rank = if_else(Season == year_to_predict, min_rank(-proj_rbi), min_rank(-rbi)),
...
# TODO: need to evluate the model's predictions here. 
```
...
mvp_predictions %>%
  ...
  ggtitle(glue("Predicted MVP Voting, {year_to_predict}"),
    subtitle = glue("Based on current division standings, 
                    and projected Steamer full-season stats as of {Sys.Date()-days(1)}")
  )
```
