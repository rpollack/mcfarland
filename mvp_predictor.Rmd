...
master_df <-
  ...
  mutate(league_ops_rank = min_rank(-ops),
  ...
  ungroup() %>%
  dplyr::select(playerid, player_name, Season, team, contains("rank"), is_mvp)
...
mvp_split <-
  ...
  dplyr::select(-playerid, -player_name, -team) %>%
...
au_roc <-
  hof_results %>%
  roc_auc(truth = is_mvp, .pred_class) %>%
