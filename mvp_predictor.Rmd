...
filtered_data <-
  ...
  select(
    -playerid, -teamid, -team_games, -ops, -home_runs, -rbi, -stolen_bases, -batting_average,
    -defense, -games_played, -games_in_field, -WAR, -Position
  )
...
train_df <-
  ...
  select(-Season, -player_name) # remove Season and players' names factors. we leave them in the test DF, so I can correctly predict MVP's
...
mvp_model_test <-
  test_df %>%
  select(-Season, -player_name) %>% # remove variables not used in model
  ...
  select(player_name, Season, League, is_mvp, pred_mvp, No, Yes, everything())
...
predict_df <-
  ...
  filter(Season == year_to_predict) %>%
  select(
    -playerid, -player_name, -teamid, -team_games, -ops, -home_runs, -rbi, -batting_average, -is_mvp,
    -defense, -games_played, -games_in_field, -Position
  )
