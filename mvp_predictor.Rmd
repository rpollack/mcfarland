...
master_df <- inner_join(player_stats, team_info, by=c("Season", "teamid")) %>%
  ...
  filter(plate_appearances >= 3.1 * (team_games))  %>% # examine qualified players only
...
# TODO: remove guys who are PA-qualified but are, say, below 10th in OPS ranking. or something like that
# there is no value in predicting these guys as they realistically have a 0% shot at MVP. they're getting small amounts
# of consideration in the model, which is just not realistic.
# perhaps cut off at 40th rank. 33rd is the max rank that anyone has ever had who won mvp ... Maury Wills, 1962
# this can be seen with: train_df %>% filter(is_mvp == "Yes") %>% arrange(desc(ops_rank_league)) %>% View()
# cutting off after 40th will remove 2,847 players
...
train_df <- master_df %>%
  filter(Season != 2018) %>% # train & test on data prior to 2018
  select(-playerid, -player_name, -teamid, -team_games, -ops, -home_runs, -rbi, -stolen_bases, -batting_average,
         -defense, -plate_appearances, -League) %>%
  mutate(is_mvp = if_else(is_mvp == 1, "Yes", "No"))
...
# distribution of mvp-worthy talent in each league
ggplot(mvp_predictions, aes(x=1,y = prob_mvp)) + geom_boxplot() + facet_wrap(~League) +
  coord_flip() + fgt
...
bind_cols(row.names(varImp(mvp_predictor)$importance) %>% as_tibble(), 
          varImp(mvp_predictor)$importance %>% as_tibble()) %>%
