...
ggplot(sb_info_seasons, aes(age_bracket, sb_perc, group = age_bracket)) + geom_boxplot() +
  fgt
...
fit <- gamlss(cbind(SB, attempts - SB) ~ ns(Season, 5) * log(attempts) * Age,
...
henderson_coleman <- eb_sb %>%
  filter(batter_name %in% c("Vince Coleman", "Rickey Henderson"))
...
plot_gamlss_fit <- function(f) {
  sb_info_seasons %>%
    ...
    ggtitle("SB% Prior Expectations, 50 Steal Attempts @ 27 Years Old") +
...
ggplot() + geom_histogram(binwidth = 0.009, aes(y=..density..)) +
  stat_function(fun = dbeta,
   ...
   stat_function(fun = dbeta,
                ...
                aes(color="Rickey Henderson")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 1796.56876, shape2 = 580.01119),
                lwd=1.5,
                aes(color = "Tim Raines")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 1123.084, shape2 = 401.8446),
                lwd=1.5,
                aes(color="Carlos Beltran")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 1225.087, shape2 = 474.1915),
                lwd=1.5,
                aes(color="Maury Wills")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 1818.548, shape2 = 590.381),
                lwd=1.5,
                aes(color="Lou Brock")) + 
