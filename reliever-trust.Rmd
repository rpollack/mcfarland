...
gmli <- fg_db %>%
  ...
  select(PlayerId, Name, Season, Age, GS, G, TBF, k_minus_bb_relief, ValueW, gmLI)
...
reliever_trust_changes <- gmli %>%
  ...
  mutate(gmli_diff = gmLI - lag(gmLI),
         ...
         to_year = Season,
         display = sprintf("%s %s – %s", Name, from_year, to_year),
  ...
  filter(!is.na(gmli_diff),
         to_year - from_year == 1) %>% 
  select(PlayerId, Name, from_year, to_year, Age, TBF, k_minus_bb_relief, 
         proj_k_bb, gmli_diff, project_kbb_minus_kbb) %>%
...
ggplot(reliever_trust_changes, aes(gmli_diff, project_kbb_minus_kbb)) + geom_point() + geom_smooth(method = "lm") + fgt +
