...
pitcher_info <- fg_db %>%
  ...
  filter(Type == 0,
         Season >= 2015) %>%
  select(PlayerId, Season, G, GS, IP, TBF) %>%
  collect(Inf) %>%
...
kershaw_results <- savant_data %>%
  ...
  summarize(mean_hit_speed = mean(launch_speed, na.rm = TRUE),
            ...
            mad_hit_speed = mad(launch_speed, na.rm = TRUE),
            mad_launch_angle = mad(launch_angle, na.rm = TRUE))
...
Increase in FIP- from 45 to 72

Even if you strip out the (supposedly) bad luck in HR/FB, Kershaw's xFIP- still has increased from 56 to 64.
