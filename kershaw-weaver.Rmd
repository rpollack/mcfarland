...
ggplot(xwoba_gainers_pitchers %>% filter(game_year == 2017, mean_xwoba_plus >= 92, mean_xwoba_plus <= 94) %>% arrange(mean_xwoba_plus), aes(reorder(player_name, -mean_xwoba_plus) , mean_xwoba_plus)) + geom_bar(stat="identity", fill ="#50ae26") + coord_flip() + ggtitle("xwOBA+ Allowed, 2017, xwOBA between 92 and 94") + 
  labs(x="", y="xwOBA+ Allowed (smaller is better)") + fgt
```
...
ggplot(foo %>% filter(Name == "Clayton Kershaw"), aes(as.character(Season), value, group = 1)) + geom_point() + geom_line() +
  facet_wrap(~stat) + ggtitle("The Slight Decline of Clayton Kershaw, 2015 - 2017") + labs(y="", x="") + fgt
