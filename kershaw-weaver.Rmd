...
ggplot(foo3 %>% filter(Name == "Clayton Kershaw"), aes(as.character(Season), value, group = 1)) + geom_point() + geom_line() +
  facet_wrap(~stat) + ggtitle("The Slight Decline of Clayton Kershaw, 2015 - 2017") + labs(y="Value (lower is better)", x="")
...
```{r}
kershaw_pitch_types <- starters_for_comparision %>%
  select(Name, Season, `Curveball` = w_cu_c, `Fastball` = w_fb_c, `Slider` = w_sl_c)
...
ggplot(foo4 %>% filter(Name == "Clayton Kershaw"), aes(as.character(Season), value, group = 1)) + geom_point() + geom_line() +
  facet_wrap(~stat) + ggtitle("Linear Weights for Kershaw's Pitches, 2015 - 2017") + labs(y="Runs Above Average per 100 Pitches\n(higher is better)", x="")        
...
ggplot(xwoba_gainers_pitchers %>% filter(game_year == 2016) %>% arrange(mean_xwoba_plus) %>% head(10), aes(reorder(player_name, -mean_xwoba_plus) , mean_xwoba_plus)) + geom_bar(stat="identity") + coord_flip() + ggtitle("xwOBA+ Allowed Leaders, 2016, Starting Pitchers >= 200 Batters") + 
...
ggplot(xwoba_gainers_pitchers %>% filter(game_year == 2017, mean_xwoba_plus >= 93, mean_xwoba_plus <= 95) %>% arrange(mean_xwoba_plus), aes(reorder(player_name, -mean_xwoba_plus) , mean_xwoba_plus)) + geom_bar(stat="identity") + coord_flip() + ggtitle("xwOBA+ Allowed, 2017, xwOBA between 93 and 95") + 
  labs(x="", y="xwOBA+ Allowed (smaller is better)")
```
...
ggplot(xwoba_gainers_pitchers %>% filter(game_year == 2017) %>% head(10), aes(reorder(player_name, xwoba_diff), xwoba_diff)) +
  geom_bar(stat="identity") + coord_flip() + ggtitle("Largest Increases in xwOBA+ Allowed, 2016 - 2017") +
  labs(x="",y="Change in xwOBA+ Allowed (%, smaller is better)")
```
...
```{r}
kershaw_stats <- starters_for_comparision %>%
  group_by(Season) %>%
