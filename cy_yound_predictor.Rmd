...
# TODO: relievers aren't judged against starters in wins and losses ... are they?
# TODO: BB and SO should also be handled differently for relievers .. and HR too.
# maybe eliminate all counting stats? maybe just eliminate them ... and keep the rate stats?
...
master_df <- inner_join(player_stats, team_info, by=c("Season", "teamid")) %>%
  ...
  filter((is_reliever == 0 & IP >= 170) |
           is_reliever == 1 & IP >= 70) %>%
  group_by(Season, League) %>%
  mutate(win_rank = min_rank(-W),
         loss_rank = min_rank(L),
         era_rank = min_rank(era),
         fip_rank = min_rank(fip),
         so_rank = min_rank(-SO),
         bb_rank = min_rank(BB),
         hr_rank = min_rank(HR),
         fip_war_rank = min_rank(-fip_war),
         so_rate_rank = min_rank(-so_rate),
         bb_rate_rank = min_rank(bb_rate),
         k_minus_bb_rank = min_rank(-k_minus_bb),
         hr_per_9_rank = min_rank(hr_per_9)) %>%
  select(playerid, player_name, Season, League, is_reliever, team_PCT, IP, is_cy, contains("_rank"))
...
cy_predictions %>%
  ...
  select(player_name, is_reliever, team_PCT, IP, contains("rank"), prob_cy)
