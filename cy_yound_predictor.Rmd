...
cy_list <- read_csv("~/Downloads/mlb season awards - Cy_yong.csv") %>%
  ...
  mutate(is_cy = "Yes",
         League = factor(League),
         is_cy = factor(is_cy, levels = c("Yes", "No"))) # define levels so prsummary function looks for the right thing
...
master_df <- inner_join(player_stats, team_info, by=c("Season", "teamid")) %>%
  ...
  filter(IP >= 150) %>%      # filter this low to catch chris sale in 2018. 
  group_by(Season, League) %>%
  mutate(zWins = (W - mean(W)) / sd(W),
         zLosses = (L - mean(L)) / sd(L),
         zERA = (era - mean(era)) / sd(era),
         zK = (SO - mean(SO)) / sd(SO),
         zfWAR = (fip_war - mean(fip_war)) / sd(fip_war),
         `zK%` = (so_rate - mean(so_rate)) / sd(so_rate),
         zIP = (IP - mean(IP) / sd(IP))) %>% 
  select(playerid, player_name, team_PCT, is_cy, contains("z"))
...
```{r}
# remove uninformative variables from master dataset, as well as info from the year we want to predict
partition_df <- master_df %>%
  filter(Season != year_to_predict) %>%
  select(-playerid, -player_name)

# identify rows used to create test data partition
# createDataPartition is stratified by default which helps ensure occurrence of is_cy matches reality
test_df_rows <- createDataPartition(partition_df$is_cy, p = 0.25) %>%
  as_tibble()

# create test dataset to hold out of training
test_df <- 
  partition_df %>%
  filter(row_number() %in% test_df_rows$Resample1)

# create training dataset to be used for cross validation
train_df <- 
  partition_df %>%
  filter(!row_number() %in% test_df_rows$Resample1)
...
# use upsampling to help resolve class imbalance
...
# unused now that parameter search is set to 'random' above
# parameter_grid <- expand.grid(
#   nrounds = c(20, 30, 40, 50), 
#   eta = c(0, .01, 0.1),
#   max_depth = c(25, 30, 40, 50),
#   gamma = c(0, 1, 2, 5),
...
evaluate model on the test dataset using "most likely Cy season" as "predicted CY". 

favor using precision/recall/F1 over sens/spec/AUC due to strong class imbalance.
...
cy_predictions %>%
  ...
  select(player_name, team_PCT, contains("z"), prob_cy)
...
master_df %>%
  ...
  filter(Season != year_to_predict) %>%
  mutate(`Team Winning %` = round(team_PCT, 3)) %>%
  ungroup() %>%
  select(-team_PCT, -is_cy) %>%
  sample_n(5) %>%
