...
master_df <- inner_join(player_stats, team_info, by=c("Season", "teamid")) %>%
  ...
  filter(IP >= 150) %>%      # filter this low to catch blake snell in 2018. lowest prior was Kershaw in 2014
  group_by(Season, League) %>%
  mutate(zWins = (W - mean(W)) / sd(W),
         zLosses = (L - mean(L)) / sd(L),
         zERA = (era - mean(era)) / sd(era),
         zK = (SO - mean(SO)) / sd(SO),
         zBB = (BB - mean(BB)) / sd(BB),
         zHR = (HR - mean(HR)) / sd(HR),
         zfWAR = (fip_war - mean(fip_war)) / sd(fip_war),
         `zK%` = (so_rate - mean(so_rate)) / sd(so_rate),
         `zBB%` = (bb_rate - mean(bb_rate)) / sd(bb_rate),
         `zK-BB%` = (k_minus_bb - mean(k_minus_bb)) / sd(k_minus_bb),
         `zHR/9` = (hr_per_9 - mean(hr_per_9)) / sd(hr_per_9)) %>%
  ...
  select(playerid, player_name, Season, League, team_PCT, is_cy, contains("z")) %>%
  mutate(is_cy = factor(is_cy, levels = c("Yes", "No"))) # define levels so prsummary function looks for the right thing
