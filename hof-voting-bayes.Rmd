...
est_hof_repeat_players %>% 
  ...
  filter(Name %in% c("Larry Walker", "Barry Bonds", "Edgar Martinez")) %>%
...
estimated HOF voting for first-year relievers

```{r}
relievers_historical <-
  master_bayes %>% 
  filter(vote_year != 2019,
         position_class == "Relief pitcher") %>%
  mutate(vote_share = num_votes_received / total_ballots)

reliever_mdl <- lm(relievers_historical$vote_share ~ relievers_historical$bj_hof_monitor + relievers_historical$SV)

summary(reliever_mdl)
```
...
est_hof_all %>%
  mutate(`Estimated Vote Share (%)` = round(est_hof_vote_share, 3) * 100,
         `Chance of Induction (%)` = round(1 - pbeta(0.75, alpha1, beta1), 3) * 100,
         `Chance of Elimination (%)` = round(pbeta(0.05, alpha1, beta1), 3) * 100)  %>%
