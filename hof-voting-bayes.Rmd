...
current_voting_data <-
  read_csv("~/Documents/2022 BBHOF Tracker-2/Summary-Table 1.csv")
...
master_bayes <-
  ...
  dplyr::select(
    Name, vote_year, year_on_ballot, num_votes_received, total_ballots, prev_year_vote_share, bj_hof_monitor,
    SV, position_class
  ) %>%
  ungroup() %>%
  mutate(log_bill_james = log(bj_hof_monitor),
         is_scandal = if_else(Name %in% c("Curt Schilling", "Barry Bonds", "Roger Clemens", "Alex Rodriguez", "Manny Ramirez"), 1, 0)) %>%
  replace_na(list(
    prev_year_vote_share = 0,
    log_bill_james = 0
  ))
```
...
master_bayes_no_sv %>%
  ...
  mutate(
    ...
    estimate = (alpha1 / (alpha1 + beta1) * 100),
    credible_low = qbeta(.025, alpha1, beta1) * 100,
    credible_high = qbeta(.975, alpha1, beta1) * 100
  ) %>%
  filter(vote_year == 2022) %>%
  ggplot(aes(estimate, reorder(Name, estimate))) +
  geom_point() +
  geom_errorbarh(aes(xmin = credible_low, xmax = credible_high)) +
  labs(x = "HOF Vote Share w/ 95% credible interval", y = "") +
  geom_vline(xintercept = 75, lty = "dashed") +
...
fit <- gamlss(cbind(num_votes_received, total_ballots - num_votes_received) ~ prev_year_vote_share + vote_year,
  data = repeat_players,
  family = BB(mu.link = "identity")
)
...
est_hof_repeat_players %>%
  ...
  filter(Name %in% c("Curt Schilling", "Barry Bonds", "Todd Helton", "Omar Vizquel", "Scott Rolen", "Roger Clemens")) %>%
...
est_hof_first_year_starters %>%
  dplyr::select(Name, alpha0, beta0, alpha1, beta1, est_hof_vote_share) %>%
  filter(Name %in% c("David Ortiz", "Jimmy Rollins", "Alex Rodriguez")) %>%
  pmap(plot_prior_posterior)
```
...
est_hof_first_year_relievers %>%
  dplyr::select(Name, alpha0, beta0, alpha1, beta1, est_hof_vote_share) %>%
  filter(Name %in% c("Joe Nathan", "Jonathan Papelbon")) %>%
  pmap(plot_prior_posterior)
```
...
est_hof_all %>%
  ...
  arrange(desc(`Chance of Induction (%)`))
```
