...
plot_prior_posterior <- function(Name, alpha0, beta0, alpha1, beta1, est_hof_vote_share) {
  ...
  ggtitle(glue("{Name} HOF Voting 2019"),
...
current_voting_data <-
  read_csv("2019 BBHOF Tracker-4/Summary-Table 1.csv")
...
master_bayes <- 
  ...
  dplyr::select(Name, num_votes_received, year_on_ballot, vote_year, total_ballots, bj_hof_monitor, 
                narrative_score, SV, position_class, prev_year_vote_share) %>%
  ungroup() 
...
est_hof_first_year_starters %>% 
  dplyr::select(Name, alpha0, beta0, alpha1, beta1, est_hof_vote_share) %>% 
  filter(Name %in% c("Roy Halladay", "Roy Oswalt", "Todd Helton")) %>%
  pmap(plot_prior_posterior)
```
...
ggplot() + 
  geom_histogram(binwidth = 0.1, aes(y=..density..)) +
  stat_function(fun = dbeta,
                args=list(shape1 = 34.59546782, shape2 = 162.2300195),
                aes(color="Scott Rolen")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 142.79091942, shape2 = 54.0345679),
                aes(color = "Roger Clemens")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 63.63772263, shape2 = 133.1877647),
                aes(color = "Fred McGriff")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 14.96972, shape2 = 181.8558),
                aes(color = "Andruw Jones")) +
  stat_function(fun = dbeta,
                args=list(shape1 = 157.8521, shape2 = 38.97335),
                aes(color = "Mike Mussina")) +
  stat_function(fun = dbeta,
                args = list(shape1 = 116.6489, shape2 = 80.17663),
                aes(color ="Larry Walker")) +
  xlim(0, 1) + labs(x="Estimated HOF Vote Share",
                    color = "Player",
                    caption = "Sources:\nPast voting: Baseball Reference\n2019 voting: bbhoftracker.com") + 
  ggtitle(glue("HOF Voting 2019: Example Posterior Distributions")) + fgt +
  geom_vline(xintercept = 0.75, linetype = "dashed", color = "green", lwd = 1.1) +
  geom_vline(xintercept = .05, linetype = "dashed", color = "red", lwd = 1.1) +
  ggsave("repeat_posteriors.png")
...
ggplot(est_hof_all, aes(est_hof_vote_share, reorder(Name, est_hof_vote_share))) +
  ...
  labs(y="", x="Estimated HOF Vote Share with 95% Credible Interval", 
       caption = "Sources:\nPast voting: Baseball Reference\n2019 voting: bbhoftracker.com") +
  ggtitle("Projected 2019 HOF Voting",
          subtitle = glue("After {known_votes} ballots tallied")) + fgt +
   geom_vline(xintercept = 0.75, linetype = "dashed", color = "green", lwd = 1.1) +
  geom_vline(xintercept = .05, linetype = "dashed", color = "red", lwd = 1.1) +
  ggsave("est_hof_voting_2019.png")

# condense to just a table of point estimates, chances of induction, and chances of elimination
est_hof_all %>%
  mutate(`Estimated Vote Share (%)` = round(est_hof_vote_share, 3) * 100,
...
# rivera chance of being unanimous
1 - pbeta(1, 182.3906, 0.4832773)

# rivera chance of setting vote share record
1 - pbeta(0.994, 182.3906, 0.4832773)
