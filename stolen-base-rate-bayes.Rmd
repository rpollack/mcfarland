...
Tim Raines vs. Rickey Henderson vs. Carlos Beltran
...
A typical approach would set minimum thresholds for playing time or simply the number of attempts. This is a decent approach but leaves all sorts of information on the cutting room floor. If you set your threshold at, say, 1,000 attempts, you get to choose between only Henderson and Brock, as the other guys have fewer than 1,000 attempts. Want to reduce the threshold to 750? Then you get everyone except Beltran. But what if he's your guy?

Going by successs rate has another flaw. Modern baseball analysis incorporates the idea that what a player does on the field is only an approximation of his true talent. To get closer at that true talent, we have to regress our observations to the mean. Raw success rate is informative but leaves out influential effects such as luck and measurement error.
...
sb_info_seasons %>%
  ...
  ggtitle("Stolen Base Success Rates over Time") +
  labs(x="Decade", y="SB Success Rate") + fgt
```

This sample includes every player-season season with at least one attempt. So Luis Sojo's one attempt in 2001 (which succeeded, by the way) is right alongside Rickey Henderson's 172 attempts in 1982. Ditto Ty Cobb's 134 attempts in 1915. They're all here.

We can see that the 1950's was a low water mark for success rate. Managers and coaches seemingly ran without regard for success, resulting in a median success rate of about 50%. Contrast that to the 2010's where the success rates are much higher. This means that managers, coaches, and players are more cautious about whether to run and are more successful in deciding whether to swipe a bag. 
...
Each point on the curve represents the probability density (y) of the player having a particular true-talent stolen base success rate (x). The peak of the curve is the players' most likely rate. 

Look at the red curve that belongs to Tim Raines. Despite an observed career success rate of 84.6%, the model thinks his true talent success rate is about 78%. Why? Regression to the mean. We do it for modern metrics like BABIP and HR/FB. We must also do it for classic metrics like stolen base success rate. The principles are the same: on-field observations are a mixture of true talent, luck, and measurement error. 
...
Remember our original question. We have a runner on first and we need a bag stolen, and every player since 1900 is on our bench. Whom do we sent in to pinch run?

Just looking at the graph above makes it hard to decide. Raines seems to have a higher success rate -- his curve peaks higher on the x-axis. But we have much less information about Raines than we do about Henderson. Additionally, the two curves seem to overlap an awful lot. Whom do we choose? Or in Bayesian terms -- given the curves above, what is the probability that Raines' success rate is higher than Henderson's?
...
there's our answer. We take Raines off the bench and put him in the game, leaving Henderson to watch. The Man of Steal has the gaudy raw numbers, true, but we've shown that Rock is more likely to give us the stolen base we need.
