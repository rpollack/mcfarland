...
oob_season_team <-
  ...
  mutate(player = str_extract(des, "(?<=\\.).*(?=out at)")) # try to get player's name

#TODO: extract the base thrown out at. 

  count(game_year, batting_team, name="oob") %>%
  complete(game_year, batting_team, fill = list(oob = 0)) %>%
  mutate(oob_per_game = case_when(
    game_year %in% c(2015, 2016, 2017, 2018, 2019) ~ round(oob / 162, 3),
    game_year == 2020 ~ round(oob/60, 3),
    game_year == 2021 ~ round(oob/150, 3)
  ))

# TODO: get the name of the player who's been thrown out! (could be more than one)
...
oob_changes <-
  ...
  filter(!is.na(oob_change) & !is.na(oob_rate_change)) %>%
  ungroup()
...
oob_changes %>%
  slice_max(order_by = oob_rate_change, n = 25) 

oob_changes %>%
  slice_min(order_by = oob_rate_change, n = 25)
