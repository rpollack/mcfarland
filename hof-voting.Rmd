...
train_and_assess_model <- function(algorithm, train_df, assess_df, eval_metric) {
  ...
  # save model to global environment so it can be used later
  model_name <- str_c("hof_voting_model", algorithm, sep = "_")
...
batter_data_clean_processed <- predict(batter_preprocess_params, batter_data_pre) %>%
  bind_cols(
    batter_data %>% select(vote_share),
    batter_data %>% select(ped_scandal),
    batter_data %>% select(year_on_ballot),
    batter_data %>% select(vote_year)
  )
```
...
batter_model_evaluation <-
  algorithms %>%
  map_dfc(train_and_assess_model, train_df = batter_hof_train, assess_df = batter_hof_test, eval_metric = "RMSE") %>%
  as_tibble() %>%
  bind_cols(batter_hof_test) %>%
  select(vote_share, contains("predict_")) %>%
  gather(model_type, predicted_vote_share, contains("predict_")) %>%
  select(model_type, predicted_vote_share, vote_share) %>%
...
batter_model_metrics <-
  ...
  select(`Model Type` = model_type, `R^2` = r_squared, RMSE, MAE, `zR^2` = zr_squared, zRMSE, zMAE, label)
...
voting_trends <-
  ...
  select(Name, prev_year_vote_share, prev_three_year_vote_share_trend)
...
predictions_processed <- predict(preprocess_params_2, predictions_pre) %>%
  bind_cols(
    batter_predictions %>% select(ped_scandal),
...
predict(hof_voting_model_xgbLinear, newdata = predictions_processed) %>%
  ...
  left_join(voting_trends, by = "Name") %>%
  select(Name, year_on_ballot, predicted_vote_share, previous_year_vote_share = prev_year_vote_share.y) %>%
  mutate(change = predicted_vote_share - previous_year_vote_share)



# predict(hof_voting_model_xgbLinear_tuned, newdata = predictions_processed) %>%
#   as_tibble() %>%
#   select(predicted_vote_share = value) %>%
#   bind_cols(predictions_processed) %>%
#   select(Name, predicted_vote_share, everything()) %>%
...
in order to do a discrete bayesian inference i would have to manually generate a probability density curve for all 1,000 data points (since HOF voting goes to 3 digits)

in order to do continous inference (at least for empirical bayes) i would have to do something like getting the success/fail data for each player from the past 3 years to get a prior. for players w/o at least 3 years of data I'd have to impute the missing data somehow. for barrel rates there was no missing data as I used players who'd had at least 3 PA in the big leagues

I think ML is still the best approach (at least until I learn the latter, or until I find a good way to generate a realistic continuous probability distribution for HOF voting totals)

```{r}
p_hof <- seq(0, 1, by = .01)
prior_edgar <- 1 / 5 + 0 * p_hof

plot(p_hof, prior_edgar, type = "h")

# add info from HOF Ballot Tracker: http://www.bbhoftracker.com/

total_ballots_observed <- 46
edgar_percent <- .913
edgar_success <- total_ballots_observed * edgar_percent
edgar_fail <- total_ballots_observed - edgar_success
edgar_votes <- c(edgar_success, edgar_fail)

posterior_edgar <- pdisc(p_hof, prior_edgar, edgar_votes)
edgar_95_credible <- discint(cbind(p_hof, posterior_edgar), 0.95)

ggplot(posterior_edgar %>% as_tibble(), aes(p_hof, value)) + geom_col() +
  labs(x = "HOF Vote Share", y = "Probability") +
  ggtitle("Edgar Martinez HOF Vote Share Probability",
    subtitle = glue("{round(max(posterior_edgar), 3) * 100}% chance that vote share is exactly {edgar_percent * 100}%
                          {round(edgar_95_credible$prob, 3) * 100}% chance that HOF vote share is between {min(edgar_95_credible$set)*100}% and {max(edgar_95_credible$set)*100}%")
  )

# halladay
prior_roy <- 1 / 2 + 0 * p_hof

plot(p_hof, prior_halladay, type = "h")

# add info from HOF Ballot Tracker: http://www.bbhoftracker.com/

roy_percent <- .893
roy_success <- total_ballots_observed * roy_percent
roy_fail <- total_ballots_observed - roy_success
roy_votes <- c(roy_success, roy_fail)

posterior_roy <- pdisc(p_hof, prior_roy, roy_votes)
roy_95_credible <- discint(cbind(p_hof, posterior_roy), 0.95)

ggplot(posterior_roy %>% as_tibble(), aes(p_hof, value)) + geom_col() +
  labs(x = "HOF Vote Share", y = "Probability") +
  ggtitle("Roy Halladay HOF Vote Share Probability",
    subtitle = glue("{round(max(posterior_roy), 3) * 100}% chance that vote share is exactly {roy_percent * 100}%
                          {round(roy_95_credible$prob, 3) * 100}% chance that HOF vote share is between {min(roy_95_credible$set)*100}% and {max(roy_95_credible$set)*100}%")
  )



# plot(p_hof, edgar_posterior, type="h")
```
...
xgb_predictions <-
  ...
  select(predicted_vote_share = value) %>%
  bind_cols(batter_hof_test) %>%
  select(vote_share, predicted_vote_share) %>%
  mutate(residual = vote_share - predicted_vote_share)
...
predict(batter_hof_xgb_mdl, newdata = data_2019) %>%
  as_tibble() %>%
  select(predicted_vote_share = value) %>%
  bind_cols(predictions_processed %>% select(Name, year_on_ballot)) %>%
  select(Name, predicted_vote_share, everything()) %>%
  mutate(predicted_vote_share = round(predicted_vote_share, 1)) %>%
  arrange(desc(predicted_vote_share)) %>%
  left_join(voting_trends, by = "Name") %>%
  select(Name, year_on_ballot, predicted_vote_share, previous_year_vote_share = prev_year_vote_share) %>%
  mutate(change = predicted_vote_share - previous_year_vote_share)
```
...
starter_data_clean_processed <-
  ...
  bind_cols(
    starter_hof_data %>% select(vote_share),
    starter_hof_data %>% select(ped_scandal),
    starter_hof_data %>% select(year_on_ballot),
    starter_hof_data %>% select(vote_year)
  )
...
# remove pitchers from the main dataset. we do this with the to_predict column
...
starter_predictions_pre <-
  bound_data %>%
  select(-ped_scandal, -year_on_ballot, -vote_year, -to_predict)

preprocess_params_3 <- preProcess(starter_predictions_pre, method = pre_methods)

starter_predictions_processed <- predict(preprocess_params_3, starter_predictions_pre) %>%
  bind_cols(
    bound_data %>% select(ped_scandal),
    bound_data %>% select(year_on_ballot),
    bound_data %>% select(vote_year),
    bound_data %>% select(to_predict)
  ) %>%
  filter(to_predict == 1) %>%
  select(-to_predict)

predict(hof_voting_model_xgbTree, newdata = starter_predictions_processed) %>%
  as_tibble() %>%
  select(predicted_vote_share = value) %>%
...
reliever_data_clean_processed <-
  ...
  bind_cols(
    reliever_hof_data %>% select(vote_share),
    reliever_hof_data %>% select(ped_scandal),
    reliever_hof_data %>% select(year_on_ballot),
    reliever_hof_data %>% select(vote_year)
  )
