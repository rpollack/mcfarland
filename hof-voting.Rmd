...
batter_model_metrics <-
  ...
  select(`Model Type` = model_type, `R^2` = r_squared, RMSE, `zR^2` = zr_squared, zRMSE, MAE, zMAE)
...
ggplot(batter_model_metrics, aes(`zR^2`, zMAE)) + geom_point() + fgt +
  ...
  xlim(-3, 3) + ylim(-3, 3) + 
  geom_label(aes(label=str_c(str_replace(`Model Type`, "predict_", ""), ": R^2 = ", round(`R^2`, 2), ", MAE = ", round(MAE, 1)))) +
