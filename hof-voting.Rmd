...
data_clean <- 
  ...
  mutate(vote_share_change = vote_share - lag(vote_share),
         vote_share_change = replace_na(vote_share_change, 0)) %>%
...
# i would also have to add the most recent vote-share change in the 2019 data. I can't compute vote share change from 2018 to 2019, because 2019 hasn't happened yet.

# add data: WAR rank on ballot? to account for the fact that ballots get crowded with good players?
...
hof_predictions <-
  ...
  # how do we do vote_share_change here??? we don't know the voting share change. so maybe it will have to be 2018's value.
  # i think i need to draw this out. maybe should just be 'most recent vote share change'. or something like 'vote share each year for the past 3 yrs to establish trend"

batter_predictions <-
  hof_predictions %>%
  filter(position != "1") %>%
  select(-W, -L, -ERA, -ERA_plus, -WHIP, -G_pitch, -GS, -SV, -IP, -H_allowed, 
         -HR_allowed, -BB_allowed, -SO, -position)
