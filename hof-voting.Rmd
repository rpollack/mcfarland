...
batter_data_preprocessed <- predict(batter_preprocess_params, batter_data_pre) %>%
  bind_cols(
    batter_data %>% dplyr::select(vote_share, ped_scandal, year_on_ballot, vote_year)
  )
...
batter_voting_future_pre <- batter_predictions %>%
...
batter_voting_future_preprocessed <- predict(batter_preprocess_params, batter_voting_future_pre) %>%
  bind_cols(
    batter_predictions %>% dplyr::select(ped_scandal, year_on_ballot, vote_year)
  )
...
starter_data_preprocessed <-
  ...
  bind_cols(
    starter_hof_data %>% dplyr::select(vote_share, ped_scandal, year_on_ballot, vote_year)
  )
...
starter_voting_future_preprocessed <- predict(starter_preprocess_params, starter_predictions_pre) %>%
  bind_cols(
    starter_data_padded %>% dplyr::select(ped_scandal, year_on_ballot, vote_year, to_predict)
  ) %>%
  filter(to_predict == 1) %>%
...
reliever_data_clean_processed <-
  ...
  bind_cols(
    reliever_hof_data %>% dplyr::select(vote_share, ped_scandal, year_on_ballot, vote_year)
  )
