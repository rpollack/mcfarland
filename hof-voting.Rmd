...
get and clean data for the year we want to predict
only bother with the predictors we know we'll use
...
predict(position_player_hof_voting_model_xgbTree, newdata = predictions_processed) %>%
  as_tibble() %>%
...
predict(starter_hof_voting_model_xgbTree, newdata = starter_predictions_processed) %>%
  as_tibble() %>%
  dplyr::select(predicted_vote_share = value) %>%
...
predict(reliever_hof_voting_model_xgbLinear, newdata = reliever_predictions_processed) %>%
  as_tibble() %>%
  dplyr::select(predicted_vote_share = value) %>%
