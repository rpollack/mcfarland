...
hof_voting_data_historical_clean %>%
  ...
  geom_point() + facet_wrap(vars(metric), scales = "free_y")


# show simple model: if all we know is the previous year's results, their j_hof monitor and the player's year on the ballot, how close can we get to the predicted vote share?
hof_voting_data_historical_clean %>%
  ...
  # dplyr::select(position_class, vote_share, prev_year_vote_share, year_on_ballot, bj_hof_monitor) %>%
  split(.$position_class) %>%
  map(~ lm(vote_share ~ bj_hof_monitor + year_on_ballot, data = .)) %>%
  map(summary) %>%
  map_dbl("r.squared") %>%
  tidy() %>%
  mutate(x = round(x, 2)) %>%
  dplyr::select(`Player type` = names, `R^2` = x)

# TODO: show scatterplots of the above

# mimic first-year players: same as above, but 
hof_voting_data_historical_clean %>%
  # mutate(position_class = case_when(
  #   position == 1 & GS / G_pitch >= 0.5 ~ "Starting pitcher",
  #   position == 1 & GS / G_pitch <= 0.5 ~ "Relief pitcher",
  ...
  # )) %>%
  split(.$position_class) %>%
  map(~ lm(vote_share ~ prev_year_vote_share + year_on_ballot + bj_hof_monitor, data = .)) %>%
  map(summary) %>%
  map_dbl("r.squared") %>%
  tidy() %>%
  mutate(x = round(x, 2)) %>%
...
#     TRUE ~ "Position player"
#   )) %>%
#   nest(vote_share, year_on_ballot, bj_hof_monitor) %>%
#   split(.$position_class) %>%
#   mutate(fit = map(~ lm(vote_share ~ year_on_ballot + bj_hof_monitor, data = .)),
#          predicted_vote_share = map(fit, augment)) %>%
#   
#   map(summary) %>%
#   map_dbl("r.squared") %>%
#   tidy() %>%
...
# much simpler model: the plot below is very nice
ggplot(hof_voting_data_historical_clean, aes(prev_year_vote_share, vote_share)) + geom_point() + geom_smooth(method="lm")
...
first_years %>%
  ...
  map_dbl("r.squared") %>%
  tidy()
# position players r^2 is 0.66
# reliever r^2 is 0.50
# starting pitcher r^2 is 0.80. quite a shock you can get that high with so little information  

fit_first_year <- lm(vote_share ~ bj_hof_monitor,
                     data = first_years)

  
# model vote_share for players who have already been on the ballot once
# a simple linear model works amazingly well for players who've been on the ballot once already
