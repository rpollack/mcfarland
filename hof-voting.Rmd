...
train_and_assess_model <- function(algorithm, train_df, assess_df){
...
# TODO: select and pre-process data in one function, using different columns depending on whether the request is a batter, starter, or reliever
...
# best tune: XGBlinear, R^S of 0.934, RMSE Of 6.13, MAE of 3.6
...
data_clean <- 
  ...
  select(-W, -L, -ERA, -ERA_plus, -WHIP, -G_pitch, -GS, -SV, -IP, -H_allowed, 
         ...
         -OBP, -SLG, -R, -OPS, -RBI, -HR, -OPS_plus, -jaws, -jpos)
...
batter_model_evaluation <-
  algorithms %>%
  map_dfc(train_and_assess_model, train_df = batter_hof_train, assess_df = batter_hof_test) %>%
  as_tibble() %>%
