...
data_clean <- 
  hof_voting_data %>%
  select(-X1, -X4, Name = X2, year_on_ballot = X3, vote_share = X5, bj_hof_monitor = X6, 
...
pitcher_hof <- data_clean %>%
  ...
  select(-position) %>%
  remove_empty("cols") %>%
  select(ERA, WHIP, G_pitch, SV, BB_allowed, vote_share)
...
pitcher_predictions <-
  ...
  filter(position == 1) %>%
  remove_empty("cols") %>%
  select(Name, ERA, WHIP, G_pitch, SV, BB_allowed)
         
predict(hof_voting_model_xgbLinear, newdata = pitcher_predictions) %>%
  as_tibble() %>%
  select(predicted_voting_percentage = value) %>%
