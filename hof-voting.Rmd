...
first_years  %>%
  split(.$position_class) %>%
  map(~ lm(vote_share ~ bj_hof_monitor + narrative_score , data = .)) %>%
  map(summary) %>%
  map_dbl("r.squared") %>%
  tidy()
...
fit_second_year <- lm(vote_share ~ prev_year_vote_share + year_on_ballot,
...
ggplot(non_first_years_fitted, aes(vote_share, predicted_vote_share)) +
  geom_point() + geom_smooth(method = "lm") +
  ggtitle("Non-first-year Players' HOF Voting Results vs. Predicted",
          subtitle = glue("R^2: {non_first_year_r_sq}")) +
...
no_hitters <-
  ...
  #   Type == "" ~ "num_no_hitters",
  #   Type == "Perfect Game" ~ "num_perfect_games",
  #   Type == "Perfect Game, Postseason" ~ "num_postseason_perfect_games",
  #   Type == "Postseason" ~ "num_postseason_no_hitters",
  #   TRUE ~ NA_character_
  # )) %>% 
  count(Name) %>%
