...
batter_data <-
  ...
  filter(position != 1) %>%
  ungroup() %>%
  dplyr::select(Name, year_on_ballot, vote_year, prev_year_vote_share, prev_three_year_vote_share_trend, ped_scandal, bj_hof_monitor, vote_share, position) 
...
batter_data_pre <-
  batter_data %>%
  dplyr::select(-vote_share, -ped_scandal, -year_on_ballot, -vote_year, -position)
...
batter_data_clean_processed <- predict(batter_preprocess_params, batter_data_pre) %>%
  bind_cols(
    ...
    batter_data %>% dplyr::select(position)
  )
```
...
batter_predictions <-
  ...
  dplyr::select(Name, year_on_ballot, vote_year, prev_year_vote_share, prev_three_year_vote_share_trend, ped_scandal, bj_hof_monitor, position)

# preprocess 2019 HOF voting data using the same parameters identified earlier
predictions_pre <- batter_predictions %>%
  dplyr::select(-ped_scandal, -year_on_ballot, -vote_year, -position)
predictions_processed <- predict(batter_preprocess_params, predictions_pre) %>%
  bind_cols(
    ...
    batter_predictions %>% dplyr::select(position)
  )
...
data_2019 <- xgb.DMatrix(predictions_processed %>%
                           dplyr::select(
                             prev_year_vote_share, prev_three_year_vote_share_trend, bj_hof_monitor, ped_scandal, year_on_ballot, vote_year, position
                           ) %>%
                           mutate(position = as.numeric(position)) %>%
                           as.matrix())
