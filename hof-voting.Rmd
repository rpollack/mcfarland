...
removing highly correlated data helped here. 
...
pitcher_predictions <-
  ...
  select(Name, year_on_ballot, bj_hof_monitor, bj_hof_standards, career_rWAR, best_war_years, jaws,
         W, L, ERA, ERA_plus, WHIP, IP, SO, num_cy_youngs, k_bb_ratio, dist_from_300_w, dist_from_3000k)
         
predict(hof_voting_model_xgbTree, newdata = pitcher_predictions) %>%
  as_tibble() %>%
  select(predicted_voting_percentage = value) %>%
