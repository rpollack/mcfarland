...
# TODO: figure out better way to impute NA values for prev_three_year_vote_trends. leaving as 0 is not great.
...
# best tune: XGBlinear, R^S of 0.94, RMSE Of 7.1
...
data_clean <- 
  ...
  select(Name, year_on_ballot, vote_share, prev_three_year_vote_share_trend, everything()) %>%
  ungroup()


# impute_model <- preProcess(data_clean[,c('prev_three_year_vote_share_trend')], "knnImpute")
# data_clean_imputed <- predict(impute_model, data_clean)

# vote_share_trans <-  preProcess(data_clean[,3], method = "YeoJohnson")
# pp_vote_share <- predict(vote_share_trans, data_clean[,3]) %>%
#   select(vote_share_pp = vote_share) %>%
#   bind_cols(data_clean) %>%
#   select(-vote_share)
# 
# 
# impute_model <- preProcess(data_clean[,c('prev_three_year_vote_share_trend')], "knnImpute")
# data_clean_imputed <- predict(impute_model, data_clean[,c('prev_three_year_vote_share_trend')])
...
batter_hof <- data_clean %>%
  filter(position != 1) %>%
  select(-W, -L, -ERA, -ERA_plus, -WHIP, -G_pitch, -GS, -SV, -IP, -H_allowed, -HR_allowed, -BB_allowed, -SO, -position,
         -career_length, -vote_year, -BB, -AB, -OBP, -SLG, -R, -OPS, -RBI, -HR, -OPS_plus)
...
predict(hof_voting_model_xgbLinear, newdata = batter_predictions) %>%
  as_tibble() %>%
  select(predicted_vote_share = value) %>%
  bind_cols(batter_predictions) %>%
  select(Name, predicted_vote_share, everything()) %>%
  mutate(predicted_vote_share = round(predicted_vote_share, 1)) %>%
  select(Name, year_on_ballot, predicted_vote_share, prev_three_year_vote_share_trend, everything()) %>%
