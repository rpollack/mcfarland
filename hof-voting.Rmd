...
seed <- 11
# best seed so far is 11. xgblinear with R^2 of 0.9 and RMSE of 7.5. Good batter predictions, too.
...
I think ML is still the best approach (at least until I learn the latter)
...
cy_young_winner_count <-
  read_csv("~/Downloads/mlb season awards - Cy_yong.csv") %>%
...
predict(hof_voting_model_xgbLinear, newdata = starter_predictions_processed) %>%
  as_tibble() %>%
  select(predicted_vote_share = value) %>%
