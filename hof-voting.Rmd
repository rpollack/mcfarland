...
# add data: previous three year's voting shares? postseason career numbers?
# modif

# the year we want to predict has a different table form. (it doesn't have vote total or vote share columns, obvs). will have to grab that one separately.
...
train_model <- function(train_df, algorithm, control_parameters){
...
# algorithms <- c("nnet", "xgbTree", "svmRadial", "xgbLinear")
...
train_model(batter_hof_train, "glm", control_parameters)
train_model(batter_hof_train, "svmRadial", control_parameters)
train_model(batter_hof_train, "xgbTree", control_parameters)
train_model(batter_hof_train, "xgbLinear", control_parameters)
train_model(batter_hof_train, "knn", control_parameters)
# neural networks always give me 'missing values in resampled performance measures ...'

send_message("HOF voting models are finished training!")


# TODO: I can do all this work inside of a function, instead of outside it. the output of the function should be the model's predictions, not the model itself. then I can just use map_df() to combine the predictions into a tibble.
