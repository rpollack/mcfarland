...
data_clean <- 
  ...
  mutate(prev_year_vote_share = lag(vote_share),
...
batter_data_clean_processed <- predict(batter_preprocess_params, batter_data_pre) %>%
  bind_cols(batter_data %>% select(vote_share), 
...
voting_trends <-
  ...
  mutate(prev_year_vote_share = vote_share,
...
starter_preprocess_params <-
  preProcess(starter_data_pre, method = c("center", "scale", "knnImpute"))
starter_data_clean_processed <-
  predict(preprocess_params, starter_data_pre) %>%
  bind_cols(
...
starter_model_evaluation <-
  ...
  map_dfc(
    ...
    train_df = starter_hof_train,
    assess_df = starter_hof_test,
    eval_metric = "RMSE"
  ) %>%
  as_tibble() %>%
...
preprocess_params_3 <- preProcess(starter_predictions_pre, method = c("center", "scale", "knnImpute"))
starter_predictions_processed <- predict(preprocess_params_3, bound_data) %>%
  bind_cols(bound_data %>% select(ped_scandal),
