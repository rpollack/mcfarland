...
data_clean <-
  hof_voting_data %>%
  dplyr::select(Name = X2, year_on_ballot = X3, vote_share = X5,
...
Position Players: extract data from main dataset, pre-processs, and create training and test datasets
...
# select predictors for position players only
batter_data <-
  data_clean %>%
  filter(position != 1) %>%
  ungroup() %>%
  dplyr::select(Name, year_on_ballot, vote_year, prev_year_vote_share, prev_three_year_vote_share_trend, ped_scandal, bj_hof_monitor, vote_share) 
...
batter_test_df_rows <- createDataPartition(batter_data_clean_processed$vote_share, p = 0.25, list = FALSE)
...
batter_predictions <-
  ...
  dplyr::select(Name, year_on_ballot, vote_year, prev_year_vote_share, prev_three_year_vote_share_trend, ped_scandal, bj_hof_monitor)

# preprocess 2019 HOF voting data using the same parameters identified earlier
batter_predictions_pre <- batter_predictions %>%
  dplyr::select(-ped_scandal, -year_on_ballot, -vote_year)
batter_predictions_processed <- predict(batter_preprocess_params, predictions_pre) %>%
  bind_cols(
...
batter_model_evaluation <-
  algorithms %>%
  map_dfc(train_and_assess_model, 
          batter_hof_train, 
          batter_hof_test,
          "RMSE",
          "position_player") %>%
...
predict(position_player_hof_voting_model_gbm, newdata = predictions_processed) %>%
  as_tibble() %>%
...
data_2019 <- xgb.DMatrix(batter_predictions_processed %>%
                           dplyr::select(
                             prev_year_vote_share, prev_three_year_vote_share_trend, bj_hof_monitor, 
                             ped_scandal, year_on_ballot, vote_year
                           ) %>%
                           as.matrix())

predict(batter_hof_xgb_mdl, newdata = data_2019) %>%
  as_tibble() %>%
  dplyr::select(predicted_vote_share = value) %>%mutate(predicted_vote_share = round(predicted_vote_share, 1)) %>%
...
predict(starter_hof_voting_model_lm, newdata = starter_predictions_processed) %>%
  as_tibble() %>%
  dplyr::select(predicted_vote_share = value) %>%
...
# pre-process data. in addition to centering, scaling, and imputing, also remove zero-variance predictors (ped_scandal, in this case)
reliever_data_pre <-
  reliever_hof_data %>%
  dplyr::select(-vote_share, -ped_scandal, -year_on_ballot, -vote_year)
pre_methods <- c("center", "scale", "knnImpute", "zv", "nzv")
...
reliever_predictions_processed <- predict(reliever_preprocess_params, reliever_predictions_pre) %>%
  bind_cols(
    reliever_hof_predictions %>% dplyr::select(ped_scandal),
    reliever_hof_predictions %>% dplyr::select(year_on_ballot),
    reliever_hof_predictions %>% dplyr::select(vote_year)
  ) 
