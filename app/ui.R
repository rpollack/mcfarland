...
")),
    tags$script(HTML("\n      function updateQueryString(player, vibe) {\n        var params = new URLSearchParams(window.location.search);\n        if (player) { params.set('player', player); } else { params.delete('player'); }\n        if (vibe) { params.set('vibe', vibe); } else { params.delete('vibe'); }\n        var newUrl = window.location.pathname + (params.toString() ? '?' + params.toString() : '');\n        window.history.replaceState(null, '', newUrl);\n      }\n      Shiny.addCustomMessageHandler('update-querystring', function(msg) {\n        updateQueryString(msg.player, msg.vibe);\n      });\n      Shiny.addCustomMessageHandler('copy-link', function(url) {\n        navigator.clipboard.writeText(url);\n      });\n      Shiny.addCustomMessageHandler('open-x-share', function(msg) {\n        var url = 'https://twitter.com/intent/tweet?text=' + encodeURIComponent(msg.text) + '&url=' + encodeURIComponent(msg.url);\n        window.open(url, '_blank');\n      });\n      Shiny.addCustomMessageHandler('open-threads-share', function(msg) {\n        var url = 'https://www.threads.net/intent/post?text=' + encodeURIComponent(msg.text) + '&url=' + encodeURIComponent(msg.url);\n        window.open(url, '_blank');\n      });\n      Shiny.addCustomMessageHandler('open-email-share', function(msg) {\n        var url = 'mailto:?subject=' + encodeURIComponent(msg.text) + '&body=' + encodeURIComponent(msg.text + ' ' + msg.url);\n        window.location.href = url;\n      });\n    "))
  ),
  nav_panel(
    title = "Analysis",
