...
total_pitches_to_each_side_cutoff <- 25
...
fit <- gamlss(cbind(num_good_pitches, total_pitches - num_good_pitches) ~ log(total_pitches),
              data = by_pitcher,
...
# best pitchers
ggplot(good_pitches_est %>% head(25), aes(reorder(player_name, est_good_pitch_rate), est_good_pitch_rate)) + coord_flip() +
...
# worst pitchers
ggplot(good_pitches_est %>% tail(25), aes(reorder(player_name, est_good_pitch_rate), est_good_pitch_rate)) + coord_flip() +
  geom_point() + geom_errorbar(aes(ymin = low_credible, ymax = high_credible)) +
  labs(x = "", y = "Estimated Good-Pitch Rate with 95% Credible Interval") +
