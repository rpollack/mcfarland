...
data <-
  ...
  select(player_name, game_year, p_throws, pitch_type, balls, strikes, zone, stand, description, xwoba) %>%
  collect(n = Inf)
```
...
# worst pitchers
ggplot(good_pitches_est %>% tail(25), aes(reorder(player_name, -st_good_pitch_percentage), est_good_pitch_percentage)) + coord_flip() +
  geom_point() + geom_errorbar(aes(ymin = low_credible, ymax = high_credible)) +
  labs(x = "")
