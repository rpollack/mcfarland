...
crossing(Season = c("2018", "2019"),
  ...
  pivot_longer(xWOBA:`Whiff%`, names_to = "metric", values_to = "value", values_drop_na = TRUE, values_ptypes = list(value= double())) %>%
  mutate(weighted_val = freq * value) %>%
  ggplot(aes(Season, weighted_val, color = pitch_type, group = pitch_type)) + facet_wrap(vars(metric), scales = "free_y") + geom_point() + geom_line() +
  labs(y="Value (Weighted by Pitch Frequency)")
