...
crossing(Season = c("2018", "2019"),
  ...
  mutate(
    ...
    `Whiff%` = c(.342, .161, NA_character_, .279, .145, .205, .091, .374),
    `% Horiz. Break vs. Average (in)` = c(11, -24, NA_character_, -25, 3, -5, -6, -4),
    `% Vert. Drop vs. Average (in)` = c(2, 12, NA_character_, -6, -6, 11, -14, -26)) %>%
  pivot_longer(xWOBA:`% Vert. Drop vs. Average (in)`, names_to = "metric", values_to = "value", values_drop_na = TRUE, values_ptypes = list(value= double())) %>%
...
hernandez_pitch_data %>%
  ...
  labs(y="Value", caption = "Source: Statcast") +
  scale_y_continuous(labels = function(x) (sprintf("%.3f", x))) + expand_limits(y=0) +
  ggtitle("Season vs. Horizontal and Vertcal Movement vs. Average Pitch", subtitle = "Elieser Hernandez")
...
plt <-
  ...
  filter(row_number() <= 10) %>%
  ggplot(aes(reorder(player_name, est_good_pitch_rate), est_good_pitch_rate)) + coord_flip() +
  geom_point(size = 0.7) + geom_errorbar(aes(ymin = low_credible, ymax = high_credible)) + facet_grid(vars(position), scales = "free_y") +
  labs(x = "", y = "Estimated Good-Pitch Rate with 95% Credible Interval") +
