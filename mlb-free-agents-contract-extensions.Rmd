...
get_international_signings <- function(year) {
  ...
  read_html(glue("https://www.spotrac.com/mlb/international/{year}")) %>%
    ...
    as_tibble() %>%
    filter(Amount != "") %>%
    dplyr::select(name = 1, Team = Team, `Contract Amount` = Amount) %>%
    mutate(
      Offseason = year,
...
fa_signings <-
  ...
  ungroup() %>%
  dplyr::select(name = Name, Offseason, Team = `To Team`, age = Age, sign_date = Date, three_year_war = `WAR3`, AB, G = `G...10`, GS, IP) %>%
  mutate(`Contract Type` = "Domestic Free Agent") %>%
  arrange(sign_date)
...
unsigned_batters <-
  ...
  mutate(
    `Contract Type` = "Unsigned Domestic Free Agent",
...
fa_salaries <-
  ...
  mutate(name = str_replace_all(name, c(
    ...
    "Yoenis Cespedes" = "Yoenis Céspedes",
...
extensions <-
  ...
  filter(!is.na(Date)) %>%
  dplyr::select(name, Offseason, sign_month, sign_date = Date, Team, `Contract Amount`, sign_years) %>%
  mutate(`Contract Type` = "Contract Extension",
         Offseason = case_when(
...
team_performance  <-
  dbFetch(q_team_pct) %>%
  as_tibble() %>%
  mutate(AbbName = if_else(AbbName == "FLA", "MIA", AbbName)) %>%
  group_by(AbbName) %>%
...
all_free_agents <-
 ...
 # str_replace_all(Team, c("Arizona Diamondbacks" = "ARI"))
  mutate(Team = case_when(
    ...
    str_detect(Team, "Los Angeles Angels") ~ "LAA",
...
Oh, and I built this with R (particularly the [tidyverse](https://www.tidyverse.org) and the graphs are done with [Plotly](https://plotly.com/r/). 
...
fa_signing_plt <-
  all_free_agents %>%
  filter(
    `Contract Type` %in% c("Domestic Free Agent", "International Free Agent"), !is.na(`Contract Amount`), `Contract Amount` != 0,
    Offseason >= 2010
  ) %>%
  mutate(
  ...
  group_by(Offseason, Team) %>%
  arrange(desc(`Contract Amount`)) %>%
  summarize(
    `Number of Players Signed` = n(),
    `Total Salary` = round(sum(`Contract Amount`, na.rm = TRUE), 1),
    Players = paste(player_salary, collapse = "\n")
  ) %>%
  ungroup() %>%
  complete(Offseason, Team, fill = list(`Number of Players Signed` = 0, `Total Salary` = 0, Players = "No one")) %>%
  ggplot(aes(Offseason, `Total Salary`, group = 1, label = Players, fill = Team, color = Team)) +
  geom_point() +
  geom_line() +
  facet_wrap(vars(Team)) +
  scale_x_discrete(breaks = c("2010", "2015", "2019")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  ggtitle("MLB Free Agent Spending, 2010–2019 Offseasons") +
...
ggplotly(fa_signing_plt, tooltip = c("Offseason", "Total Salary", "Players"))
```
...
fa_signing_plot_total <-
  all_free_agents %>%
  filter(`Contract Type` %in% c("Domestic Free Agent", "International Free Agent"), !is.na(`Contract Amount`), `Contract Amount` != 0) %>%
  mutate(
    player_salary = glue("{name} - ${round(`Contract Amount`,1)}m"),
    Offseason = as.character(Offseason)
  ) %>%
  ...
  ggplot(aes(reorder(Team, `Total Salary`), `Total Salary`, fill = Team, label = `Number of Players Signed`)) +
  geom_col() +
  theme(legend.position = "none") +
  coord_flip() +
  scale_color_manual(values = team_colors) +
  scale_fill_manual(values = team_colors) +
  fgt +
  labs(x = "", y = "Total FA Contracts Signed (Millions USD)") +
  ggtitle("Cumulative MLB Free Agent Spending by Team, 2010-2019 Offseasons")

ggplotly(fa_signing_plot_total, tooltip = c("Team", "Number of Players Signed", "Total Salary"))
```
...
top_signings_plt <-
  all_free_agents %>%
  filter(`Contract Type` %in% c("Domestic Free Agent", "International Free Agent"), !is.na(`Contract Amount`), `Contract Amount` != 0) %>%
  mutate(Info = glue("{name} - {Offseason} - {Team}")) %>%
  top_n(25, wt = `Contract Amount`) %>%
  ggplot(aes(reorder(Info, `Contract Amount`), `Contract Amount`, label = Info)) +
  geom_col() +
  coord_flip() +
  labs(x = "", y = "Contract Salary (Millions USD)") +
  ggtitle("MLB Top 25 Free Agent Contracts, 2010-2019", subtitle = "Adjusted to 2020 dollars") +
  fgt

ggplotly(top_signings_plt, tooltip = c("Info", "Contract Amount"))
```
...
fa_team_plt <-
  ...
  ggplot(aes(reorder_within(sign_label, `Contract Amount`, Team), `Contract Amount`, fill = Team)) +
  geom_col() +
  facet_wrap(vars(Team), scales = "free_y", ncol = 2) +
  coord_flip() +
  ...
  labs(x = "", y = "Contract Amount (Millions USD)") +
...
all_free_agents %>%
  group_by(Offseason) %>%
  summarize(`Median Days to Sign` = median(days_to_sign, na.rm = TRUE)) %>%
  ggplot(aes(Offseason, `Median Days to Sign`)) +
  geom_point() +
  geom_line() +
  expand_limits(y = 0) +
  fgt +
...
sign_time_plt_talent <-
  ...
  arrange(desc(name)) %>%
  summarize(
    `Median Days to Sign` = median(days_to_sign, na.rm = TRUE),
    player_names = paste(name, collapse = "\n")
  ) %>%
  complete(Offseason, player_type) %>%
  ggplot(aes(Offseason, `Median Days to Sign`, text = player_names, group = 1)) +
...
age_type_plt <-
  ...
  filter(
    !is.na(age_bucket),
    Offseason >= 2010,
    `Contract Type` %in% c("Domestic Free Agent", "International Free Agent")
  ) %>%
...
```{r echo=FALSE}
all_free_agents %>%
  filter(`Contract Type` == "Domestic Free Agent", player_type != "Scrub", days_to_sign >= 0) %>%
  distinct() %>%
...
extension_plt <-
  ...
  group_by(Team, Offseason) %>%
  arrange(desc(`Contract Amount`)) %>%
  summarize(
    `Total Salary` = round(sum(`Contract Amount`), 1),
    players = paste(label, collapse = "\n")
  ) %>%
  mutate(Offseason = as.character(Offseason)) %>%
  ungroup() %>%
  complete(Team, Offseason, fill = list(`Total Salary` = 0, players = "No one")) %>%
  ggplot(aes(Offseason, `Total Salary`, group = 1, label = players, color = Team)) +
  geom_point() +
  geom_line() +
...
ggplotly(extension_plt, tooltip = c("Offseason", "Total Salary", "players"))
```
...
ggplotly(top_extensions_plt, tooltip = c("Info", "Contract Amount"))
```
...
top_5_extension_plt <-
  ...
  group_by(Team) %>%
  top_n(5, wt = `Contract Amount`) %>%
  ggplot(aes(reorder_within(sign_label, `Contract Amount`, Team), `Contract Amount`, fill = Team, label = age)) +
  geom_col() +
  facet_wrap(vars(Team), scales = "free_y", ncol = 2) +
  coord_flip() +
  scale_x_discrete(labels = function(x) gsub("__.+$", "", x)) +
...
cumulative_spend_plt <-
  all_free_agents %>%
  filter(
    `Contract Type` %in% c("Domestic Free Agent", "Contract Extension"),
    !is.na(Team),
    Team != ""
  ) %>%
  group_by(Team, `Contract Type`) %>%
  summarize(total = sum(`Contract Amount`, na.rm = TRUE)) %>%
  pivot_wider(names_from = `Contract Type`, values_from = total) %>%
  inner_join(team_performance, by = c("Team" = "AbbName")) %>%
  mutate(Information = glue("Team: {Team}
  ...
  labs(x = "Free Agent Spending (Millions USD)", y = "Contract Extension Spending (Millions USD)",
       caption = "Winning% is for the 2010 - 2019 seasons") +
  ggtitle("Free Agent vs. Contract Extension Spending, MLB Teams 2010-2019 Seasons") +
  fgt +
  scale_color_gradient2(low = "darkred", high = "darkgreen", mid = "yellow4", midpoint = 0.5) +
...
cume_plt <-
  all_free_agents %>%
  filter(`Contract Type` %in% c("Domestic Free Agent", "International Free Agent", "Contract Extension"),
         Offseason >= 2010, Offseason <= 2019,
         !is.na(Team),
         Team != "") %>%
  group_by(Offseason, Team, `Contract Type`) %>%
  arrange(name) %>%
  summarize(
  ...
  ungroup() %>%
  complete(Offseason, Team, `Contract Type`, fill = list(total_spending = 0)) %>% 
  ggplot(aes(as.character(Offseason), total_spending, group = `Contract Type`, fill = `Contract Type`, label = players)) +
  geom_area() +
  facet_wrap(vars(Team)) +
  scale_x_discrete(breaks = c("2010", "2015", "2019")) + fgt +
  ggtitle("Offseason vs. Total Contracts Given by Type, by Team 2010-2019") +
  labs(x="Offseason", y="Total Spending (Millions USD)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
If you're interested in more baseball nerdery, [follow me on Twitter](https://twitter.com/ryry9379/) and [get updated when I write new things](http://www.ryanpollack.com/get-updates/).
