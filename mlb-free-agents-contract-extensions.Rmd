...
fa_signing_plt <-
  ...
  ungroup() %>%
  complete(offseason, Team, fill = list(num_players_signed = 0, total_salary = 0, players_signed = "No one")) %>%
  ggplot(aes(offseason, total_salary, group = 1, label = players_signed, fill = Team, color = Team)) +
  geom_point() +
  geom_line() +
...
fa_signing_plot_total <-
  ...
  ggplot(aes(reorder(Team, total_salary), total_salary, fill = Team, label = num_players_signed)) +
  geom_col() +
  theme(legend.position = "none") +
  coord_flip() +
  scale_color_manual(values = team_colors) +
...
ggplotly(fa_signing_plot_total, tooltip = c("Team", "num_players_signed", "total_salary"))
```
...
extension_plt <-
  ...
  ungroup() %>%
  complete(Team, offseason, fill = list(total_salary = 0, players = "No one")) %>%
  ggplot(aes(offseason, total_salary, group = 1, label = players, color = Team)) +
  geom_point() +
  geom_line() +
