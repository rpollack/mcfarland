...
In this example we get all leaderboards for the years 2012-2016.
...
```{r}
fg_grouped <- fg_data %>%
  select(Name, Season, PA, GB_per_FB) %>%
  group_by(Name) %>%
  arrange(Name, Season) %>%
  do(lag(., -1))
...
#streamgraph(fg_grouped, "Name", "offense_p", "Season") %>%
#  sg_axis_x(tick_interval=1) %>%
#  sg_legend(TRUE, "Player")
