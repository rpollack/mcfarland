...
```{r}
fg_data <- scrape_player_batting_data("all", 2012, 2016)
...
fg_grouped <- fg_data %>%
  filter(Team == "Orioles") %>%
  group_by(Season) %>%
  mutate(offensive_WAR_perc = (WAR / sum(WAR)) * 100) %>%
  arrange(desc(Season), desc(offensive_WAR_perc))
  

streamgraph(fg_grouped, "Name", "offensive_WAR_perc", "Season") %>% sg_axis_x(tick_interval=1) %>% sg_legend(TRUE, "Player")
