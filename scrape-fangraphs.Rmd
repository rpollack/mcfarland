...
fg_batters <- inner_join(fg_players, fg_bat, by=c("PlayerId" = "playerid")) %>%
  ...
  select(PlayerId, Name, Season, PA, -BirthDate, ValueW, oswing = `pfxO-Swing%`, zswing = `pfxZ-Swing%`, `wRC+`) %>%
  filter(PA >= 200) %>%
  ...
  #since we're throwing out sub-300 PA seasons, throw out non-consec. seasons also
```
...
team_value_plt <- ggplot(data=head(player_value_to_team, 20), aes(x=reorder(name_plus_team, hitter_war_perc), y=hitter_war_perc)) + geom_bar(stat="identity") + coord_flip() + ggtitle("Good Hitters on Bad Teams (Except for Manny Machado!)") + labs(y="% of Team's Non-Pitcher WAR", x="Player")
