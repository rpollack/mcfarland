...
discipline_changes <- fg_data %>%
  mutate(z_minus_o_swing = Z_Swing_perc - O_Swing_perc) %>%
  group_by(Name) %>%
  arrange(Season) %>%
  filter(G > 100) %>%
  select(Name, Season,PA, z_minus_o_swing, Z_Swing_perc, O_Swing_perc) %>%
  mutate(change = z_minus_o_swing - lag(z_minus_o_swing))
...
```{r}
players_per_non_pitcher_win <- player_value_to_team %>%
  group_by(Team) %>%
  summarize(hitters_per_win = n() / sum(WAR)) %>%
  arrange(desc(hitters_per_win))
```
