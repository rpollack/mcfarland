...
poff_chances <-
  ...
  select(team_name, poff_chance_davenport, poff_chance_fg, poff_chance_numberfire, poff_chance_teamrankings = poff_chance) %>%
  inner_join(poff_chance_pecota, by = "team_name") %>%
  select(team_name, poff_chance_davenport, poff_chance_fg, poff_chance_numberfire, poff_chance_teamrankings, poff_chance_pecota = poff_chance) %>%
...
poff_chances_clean |>
  ...
  geom_col() +
  coord_flip() +
  labs(y = "Playoff Chance (%)", x = "", caption = "Playoff chances: median of FG, BPro, Clay Davenport, Numberfire, and TeamRankings") +
  geom_hline(yintercept = 50, lty = "dashed") +
  facet_wrap(vars(league), scales = "free_y") +
  ggtitle("MLB Team Playoff Chances", subtitle = glue("Prior to Games Played on {Sys.Date()}"))
...
poff_chance_change |>
  ...
  ggtitle("Playoff Chances & Changes",
    subtitle = glue("From 2024-03-28 to {Sys.Date()}")
  ) +
  labs(
