...
poff_chances_clean |>
  ...
  geom_col() +
  coord_flip() +
  labs(y = "Playoff Chance (%)", x = "", caption = "Playoff chances: median of FG, B-Ref, BPro, Clay Davenport, Numberfire, TeamRankings, and 538 (until deprecation)") +
  geom_hline(yintercept = 50, lty = "dashed") +
  facet_wrap(vars(league), scales = "free_y") +
  ggtitle("MLB Team Playoff Chances", subtitle = glue("Prior to Games Played on {Sys.Date()}"))
...
read_csv("~/Downloads/fangraphs-leaderboards-3.csv") |>
  filter(!Season %in% c(1994, 1995, 2020)) |>
  select(Season, PlayerId, Name, PA, G, Off) |>
  filter(PA <= 520, Off >= 30) |>
  arrange(desc(Off))
```
