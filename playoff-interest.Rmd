...
read_html("https://www.teamrankings.com/mlb/") %>%
  ...
  pluck(1) %>%
  html_table() %>%
  mutate(team_name = str_replace_all(Team, c("LA Dodgers" = "Los Angeles Dodgers",
...
poff_chance_numberfire <-
  ...
  bind_cols(
    ...
    mutate(team_name = str_extract(Team, ".*[a-z]"),
           poff_chance = as.numeric(str_extract(Playoffs, "[:digit:]{1,3}\\.[:digit:]{1}"))) %>%
  select(team_name,poff_chance)
...
poff_chances <-
  ...
  mutate(
    avg_poff_chance = round((poff_chance_538 + poff_chance_bbref + poff_chance_davenport + poff_chance_fg + poff_chance_numberfire + poff_chance_teamrankings + poff_chance_pecota) / 7, 0),
...
poff_chances %>%
  pivot_longer(2:8, names_to = "site") %>%
