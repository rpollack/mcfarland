...
tweet <- glue("Good morning! The MLB playoffs start in {difftime(playoff_start, Sys.Date(), units = 'days') %>% floor() %>% as.numeric()} days.

The current playoff picture is attached.

Most impactful game today: {away_handle} ({away_pct}% chance) at {home_handle} ({home_pct}% chance).

Sources: @fangraphs @baseball_ref @baseballpro Clay Davenport @FiveThirtyEight @numberFire @teamRankings")
