...
playoff_davenport <- read_html("http://www.claydavenport.com/stats/ps_odds.html") %>%
  html_elements("pre") %>%
  html_text2() %>%
  str_split("\n")
...
poff_chances_clean |>
  ...
  geom_col() +
  coord_flip() +
  labs(y = "Playoff Chance (%)", x = "", caption = "Playoff chances: median of FG, B-Ref, BPro, Clay Davenport, Numberfire, TeamRankings") +
  geom_hline(yintercept = 50, lty = "dashed") +
  facet_wrap(vars(league), scales = "free_y") +
  ggtitle("MLB Team Playoff Chances", subtitle = glue("Prior to Games Played on {Sys.Date()}"))
...
# compute playoff chances change between the start of the 2024 season and the current date

poff_chance_change <-
  read_csv("playoff_chances.csv") |>
  filter(prediction_date >= "2024-03-28") |>
...
poff_chance_change |>
  ...
  labs(
    ...
    caption = "Playoff chances: median of FG, B-Ref, BPro, Clay Davenport, Numberfire, TeamRankings, and 538 (until deprecation)"
  ) +
