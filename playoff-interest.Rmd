...
# append playoff chance data to disk
poff_chances_clean |>
  mutate(prediction_date = Sys.Date()) |>
  select(prediction_date, everything(), -median_poff_chance) |>
  write_csv(file = "playoff_chances_2025.csv", append = TRUE)

# do the same for daily interest game schedule
sched |>
  mutate(game_date = Sys.Date()) |>
  write_csv("interest_schedule.csv", append = TRUE)
...
max_change <-
  change_graph |>
  slice_max(order_by = change, n = 1, with_ties = FALSE) |>
  select(change) |>
  as.numeric() |>
  pluck()
...
limit <- max(abs(max_change), abs(min_change))
...
change_graph |>
  ...
  labs(
    x = "Change Since March 28th, 2025 (percentage points)", y = "Current Playoff Chance (%)",
    caption = "Playoff chances: median of FG, BPro, Clay Davenport, and TeamRankings"
  ) +
...
# top 10 interesting/relevant games so far in the season
# read_csv("interest_schedule.csv") |>
#   slice_max(n=10, order_by = interest_game)
```
...
rdif <- sum(data$runs_scored) - sum(data$runs_allowed)
...
data |>
  ...
  ggtitle(glue("{abbrev} {season} Season ({num_games} Games)"),
    subtitle = glue("7 game moving average of runs scored/game, runs allowed/game, and run differential
              Overall: {rpg} runs scored per game, {rag} runs allowed per game, {rdif} run differential")
  ) +
  expand_limits(y = 0) +
...
# monthly R/RA/G graph
data |>
  mutate(
    month = month(date, label = TRUE),
    month = if_else(month %in% c("Mar", "Apr"), "Mar/Apr", month),
    month = factor(month, levels = c("Mar/Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct"))
  ) |>
  group_by(month) |>
  summarize(
    runs_per_game = round(sum(runs_scored) / n(), 1),
    runs_allowed_per_game = round(sum(runs_allowed) / n(), 1)
  ) |>
  pivot_longer(-month, names_to = "metric") |>
  ggplot(aes(month, value, group = metric, color = metric, label = value)) +
  geom_point(size = 4) +
  geom_path(linewidth = 2) +
  expand_limits(y = 0) +
  labs(x = "") +
  geom_label(aes(y = value), vjust = 1.5) +
  ggtitle(glue("{abbrev} {season} Season"),
    subtitle = "Runs Scored & Allowed per Game, by Month"
  ) +
  scale_colour_manual(values = cbPalette)
```

rolling x-day mean of Immaculate Grid scores

```{r}
# how many days to capture in the 'mean'
window <- 30

ig <- read_sheet("https://docs.google.com/spreadsheets/d/1Ojo7gi7hiwELpxX8ctjWT9-UGhht4-oHpQdCdDPMyWM/edit?gid=0#gid=0") |>
  drop_na(RARITY)

num_games <- nrow(ig)
all_time_rarity <- floor(sum(ig$RARITY) / num_games)

ig |>
  mutate(rolling_7d_mean = slide_dbl(RARITY, mean, .before = window - 1)) |>
  ggplot(aes(DATE, rolling_7d_mean)) +
  geom_point(size = 0.2) +
  geom_path() +
  labs(x = "", y = glue("Rolling {window}-day Rarity")) +
  ggtitle("Immaculate Grid Rarity",
    subtitle = glue("Average rarity: {all_time_rarity} in {num_games} games")
  ) +
  expand_limits(y = 0)
```
