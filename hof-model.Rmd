...
1. Cano, Manny, A-Rod etc were all busted for PEDs TWICE. Encode this fact. 
1. scale_pos_weight
...
max_franchise_games |> count(PlayerId, age) |> filter(n>1)
...
bat_data_full <-
  ...
  summarize(
    ...
    season_HR = sum(HR),
    season_SB = sum(SB),
    season_num_asg = sum(asg),
    season_num_mvp_points = sum(num_mvp_points)
  ) |>
...
bat_data_full |> count(PlayerId, age) |> filter(n > 1)
```
...
hof_split <-
  ...
  select(-PlayerId, -Name, -num_seasons, -mlb_played_last) |>
...
player_predictions <-
  ...
  select(Name, age, pred_hof, cumulative_war, war_per_year, cumulative_h, cumulative_hr, cumulative_mvp_pts, scandal_type, career_midpoint_year, max_cume_frnch_g, cumulative_sb, cumulative_asg) |>
...
player_predictions |>
  ...
  select(Name, pred_hof, age, cumulative_war, cumulative_hr, cumulative_h, cumulative_sb, cumulative_mvp_pts, cumulative_asg, scandal_type, war_per_year) |>
  arrange(desc(pred_hof), desc(war_per_year)) |>
  kbl() |>
  kable_styling()
...
c("Robinson Cano", "Josh Donaldson", "Shohei Ohtani") |> 
