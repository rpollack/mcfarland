...
master_list <-
  ...
  mutate( # use model from The Point of Pittsburgh to project future arb salaries
    ...
    Salary = as.numeric(Salary) * ((1-0.215)^(Season - next_year)), # inflate actual salary each year by the average of the last 3 years' Feb-Feb YoY rates:             
...
# team WAR per season AL Central
team_war_per_season %>%
  filter(Division == "AL Central") %>%
  ggplot(aes(Season, controlled_war, color = Team, fill = Team)) + geom_point() + geom_line() +
  labs(y = "WAR Under Control", caption = "Sources: Baseball Reference, FanGraphs, & Tom Tango") +
  ggtitle(glue("Team Controlled WAR Per Season, {next_year} - {next_year + 4}"),
          subtitle = "AL Central Division") +
  geom_label_repel(aes(label = label),
    na.rm = TRUE,
    show.legend = FALSE,
    color = "white",
...
warcels %>%
  ...
  summarize(
    ...
    controlled_salary_per_year = sum(salary_under_control)
  ) %>%
