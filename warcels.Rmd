...
all_teams_payroll <- team_info %>% 
  ...
  dplyr::select(Team = team_name, Name, Age, Year, salary) %>%
  group_by(Name, Team) %>%
...
master_list <-
  ...
  mutate(
    ...
    Salary = as.numeric(Salary) * (1.0238^(Season - next_year)), # inflate salary by 2.38% annually
...
warcels <- 
  master_list %>%
  group_by(PlayerId, Name, Team, Division) %>%
  summarize(
    controlled_years = n(),
