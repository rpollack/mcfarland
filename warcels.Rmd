...
warcels_team_2018 %>%
  ...
  ggtitle("Controlled Surplus Value, 2018 - 2022")
...
team_war_per_season %>%
  ...
  labs(y="WAR Under Control") + facet_wrap(~Division, ncol = 2) +
...
warcels %>%
  ...
  scale_color_manual(values = team_colors) +
  scale_fill_manual(values = team_colors) +
  labs(x="Controlled Years / Player", y="Controlled Projected WAR / Year", 
...
```{r}
#player surplsu value changes 2017 - 2017
player_surplus_value_changes <-
  ...
  mutate(surplus_value_change = ifelse(Name == "Jose Ramirez" & surplus_value_change == 47.5, 0, surplus_value_change)) %>%
...
player_surplus_value_changes %>%
  ...
  labs(x="", y="Surplus Value Change (Millions USD)") + 
...
team_surplus_value_changes %>%
  ...
  labs(x="", y="Surplus Value Change (Millions USD)") + ggtitle("MLB Team Surplus Value Changes, 2017-2021 Period to 2018-2022 Period")
