...
getPayrolls <- function(Tm) {
  ...
  tms_melt <- tms_melt %>%
    ...
    mutate(value = ifelse(value == "Arb-1" | value == "Arb", 2.2, value), #$2.2M is the avg Arb-1 salary in 2016, per MLBTR's tracker
...
# now need to find fWAR prior to current year (so for 2015 players, WAR for all years prior to 2015)
all_arb <- all_arb %>%
  mutate(war_at_signing = lapply())
