...
Project WAR for the 5 years going forward; I wrote the code the way I did because 
...
projected_war <-
  ...
  filter(Season == 2018) %>% # this row has all the relevant WAR projections in it
...
# get payroll information for all teams, keep only the next five years of information for each player, and change some players' names to match
...
all_teams_payroll <- team_info %>%
  ...
  ))) # incorrect name but still ensures a dataset match


# TODO: change more names to match those in FG. to find names that need changing, anti_join all_teams_payroll with projected_war
```
...
master_list <-
  ...
  mutate( # handle type 1
    ...
    super_two = ifelse(Name %in% c(
      ...
      "Trea Turner", "Matthew Boyd", "Carl Edwards Jr."
    ),
    TRUE, FALSE
    )
  ) %>%
...
team_war_per_season <-
  ...
  mutate(label = ifelse(Team %in% c("NYY", "CLE", "HOU", "LAD", "MIL", "WSN") &
    Season == min(Season),
  Team,
  NA_character_
  ))
...
warcels %>%
  ...
  ggtitle(glue("MLB Team Outlook, {next_year} – {next_year + 4}")) +
  scale_color_manual(values = team_colors) + fgt +
  scale_fill_manual(values = team_colors) +
  labs(
