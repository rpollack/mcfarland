...
# also add the team's primary color for graphing purposes
warcels <- 
  master_list %>%
...
warcels_team_2018 %>%
  ...
  labs(x="", y="Controlled Surplus Value (USD Millions)") +
...
team_war_per_season %>%
  ggplot(aes(Season, controlled_war, group=Team, fill=Team)) + geom_point() + geom_line() + fgt +
  labs(y="Surplus Value Under Control (USD Millions)") + facet_wrap(~Division, ncol = 2) +
  scale_fill_manual(values = team_colors) +
...
warcels %>%
  ...
  summarize(num_players = n(),
            ...
            controlled_war_per_player_per_year = sum(war_under_control)/sum(controlled_years)/num_players) %>%
  ggplot(aes(controlled_years_per_player, controlled_war_per_player_per_year, color=Team, fill=Team)) + 
  geom_point() + fgt + 
  geom_label_repel(aes(label=Team),
                   ...
                   color="white",
                   segment.color = "black") +
  ggtitle("MLB Team Control over Talent, 2018 - 2022") + scale_color_manual(values = team_colors) +
  scale_fill_manual(values = team_colors) +
  labs(x="Controlled Years per Player", y="Controlled WAR per Player per Year", 
...
warcels_2017_2018 <- read_csv("warcels-2017.csv") %>%
  dplyr::select(Name, Team, war_under_control, salary_under_control, surplus_value_under_control = controlled_surplus_value) %>%
...
player_surplus_value_changes %>%
  ...
  labs(x="", y="Surplus Value Change (USD Millions)") + 
...
team_surplus_value_changes %>%
  ...
  labs(x="", y="Surplus Value Change (USD Millions)") + ggtitle("MLB Team Surplus Value Changes, 2017-2021 Period to 2018-2022 Period")
