...
team_colors <- teamAbbr$team_color
names(team_colors) <- factor(teamAbbr$Tm)
head(team_colors)
...
warcels_team_2018 %>%
  ggplot(aes(reorder(Team, total_surplus_value_under_control), total_surplus_value_under_control), fill=Team) +
  geom_col() + 
  coord_flip() +
  fgt +
...
team_war_per_season <-
  ...
  # mutate(label = ifelse(Team %in% c("NYY", "CLE", "HOU", "LAD", "CHC") &
  #                         Season == max(Season),
  #                       Team,
  #                       NA_character_))

team_war_per_season %>%
  ggplot(aes(Season, controlled_war, group=Team, color=Team)) + geom_point() + geom_line() + fgt +
  ...
  labs(y="Surplus Value Under Control (USD Millions)") + facet_wrap(~Division, ncol = 1)
...
warcels %>%
  ...
  ggplot(aes(controlled_years_per_player, controlled_war_per_player_per_year, color=Team)) + geom_point() + fgt +
  geom_label_repel(aes(label=Team),
                       na.rm = TRUE) +
  ggtitle("MLB Team Control over Talent, 2018 - 2022") +
  labs(x="Controlled Years per Player", y="Controlled WAR per Player per Year", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango")
