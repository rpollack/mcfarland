...
INSTRUCTIONS ON UPDATING THESE CALCULATIONS EACH YEAR:
1. 
...
projected_war <-
  ...
  mutate(
    ...
    # TODO: there is a bug where players without a 2018 season are not getting projected: Yasmany, Tulo, Jacoby, etc.
    #       how do I find players who missed 2018 but whom are expected to play in 2019? perhaps join on bb-ref dataset
    #       sooner rather than later.
    #       I can probably use complete() to ensure there are values for all the prior seasons used to project. 
    #       I also think it makes sense to give players 0 WAR for missed seasons. This will also affect rookies but hey, you weren't in the bigs
    #       so you shouldn't get credit for playing in them.
  ...
  mutate(
    ...
    # TODO when updating for next season: update the following 5 columns to be the 5 years we want to project for
    # TODO: when updating for next season: update each column to calculate off the previous year's column

    `2019` = round(ifelse(Position == "SP" | Position == "RP",
      step2 + ((26 - age_next_year) * 0.1), 
      step2 + ((30 - age_next_year) * 0.1)
    ), 1),
    `2020` = round(ifelse(Position == "SP" | Position == "RP",
      `2019` - 0.4,
      `2019` - 0.4 + ((30 - age_in_two_years) * .08)
    ), 1),
    `2021` = round(ifelse(Position == "SP" | Position == "RP",
      `2020` - 0.4,
      `2020` - 0.4 + ((30 - age_in_three_years) * .03)
    ), 1),
    `2022` = round(ifelse(Position == "SP" | Position == "RP",
      `2021` - 0.4,
      `2021` - 0.4 + ((30 - age_in_four_years) * .03)
    ), 1),
    `2023` = round(ifelse(Position == "SP" | Position == "RP",
      `2022` - 0.4,
      `2022` - 0.4 + ((30 - age_in_five_years) * .03)
    ), 1)
  ) %>%
...
# get payroll information for all teams, keep only the next five years of information for each player, and change players' names to match
# how they are formatted in the FanGraphs database, so information from the two datasets can be joined together later.
...
all_teams_payroll <- team_info %>%
  ...
  ungroup() %>%
  mutate(Name = str_replace_all(Name, c("Ronald Acuna" = "Ronald Acuna Jr.",
                                        "Danny Coulombe" = "Daniel Coulombe",
                                        "Daniel Ponce de Leon" = "Daniel Poncedeleon",
                                        "Gio Urshela" = "Giovanny Urshela",
                                        "Zack Britton" = "Zach Britton"))) # incorrect name but still ensures a dataset match 


# TODO: change Names to match those in FG. 
# TODO: figure out why Yasmany Tomas, Jacoby Ellsbury, etc.  dropped from projected_war despite being present in fg_batters ...
#          is it because they are missing 2018 seasons????
# the following code finds players who are in baseball-reference but not in FG projections: anti_join(all_teams_payroll, projected_war, by=c("Name", "Team")) %>% distinct(Name)
```
...
master_list <-
  ...
  mutate( # use model from The Point of Pittsburgh to project future arb salaries
    ...
    Salary = as.numeric(Salary) * (1.022^(Season - next_year)), # inflate actual salary by 2.38% annually
