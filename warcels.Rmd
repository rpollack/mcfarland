...
# I put Toronto at 3202930 because it's the average of chicago & philly markets.  
...
team_info <- tibble(
  ...
  tv_households = c(1890100, 2412730, 1119480, 2424240, 3463060,
                    3463060, 863800, 1498960, 1630380, 1853030,
                    2450800, 919020, 5476830, 5476830, 1696330,
                    895700, 1742530, 7348620, 7348620, 2488090,
                    2942800, 1160220, 1065700, 1808530, 2488090,
                    1215570, 1908590, 2713380, 3202930, 2476680)
  )
...
#retain players with more than 0 WAR under control or whose AAV is over $5 mil.
...
team_war_per_season %>%
  ggplot(aes(Season, controlled_war, color=Team, fill=Team)) + geom_point() + geom_line() + fgt +
...
warcels %>%
  ...
  summarize(num_players = n(),
            ...
            controlled_war_per_year = sum(war_under_control)/sum(controlled_years)) %>%
  ...
  geom_label_repel(aes(label=Team),
                   ...
                   segment.color = "black",
                   nudge_y = .1) +
  ggtitle("MLB Team Contention Windows, 2018 - 2022") + 
  scale_color_manual(values = team_colors) +
  scale_fill_manual(values = team_colors) +
  labs(x="Controlled Years / Player", y="Controlled WAR / Year (Projected)", 
       caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") +
