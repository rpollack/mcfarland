...
test run to use proper SQL / mariadb init
```{r}
con <- dbConnect(RMariaDB::MariaDB(), group = "fg_db")
q_bat <- dbSendQuery(con, "select distinct sbm.Team, Batting, BaseRunning from stats_batting_master sbm 
            inner join player_info pi on sbm.playerid - pi.PlayerId
            and sbm.Season = 2020
            and sbm.team != \'- - -\'
            and sbm.Type = 0")
res_bat <-
...
# get team offense. this matches FG's number they display so I think it's the right method
res_bat %>% 
  group_by(Team) %>%
  mutate(Off = Batting + BaseRunning) %>% 
  summarize(team_off = round(sum(Off), 1)) %>%
  arrange(desc(team_off))
...
team_info <- tibble(
  ...
  team_color = c(
    ...
    "#BA0C2F", "#FDB829", "#2F241D", "#005C5C", "#F2552C",
...
millions_per_war <- 9
# source: https://blogs.fangraphs.com/an-update-to-prospect-valuation/
...
warcels_full %>%
  ...
  ggtitle(glue("Top 25 Controlled WAR, {next_year} – {next_year + 4}"))
...
warcels_full %>%
  ...
  ggtitle(glue("Bottom 25 Controlled WAR, {next_year} – {next_year + 4}"))
...
warcels %>%
  ...
  scale_fill_manual(values = team_colors) +
  theme(legend.position = "None") +
  xlim(-1000000, 7500000)

warcels %>%
  group_by(Team) %>%
  summarize(total_controlled_surplus_value = sum(surplus_value_under_control)) %>%
  inner_join(team_info, by = c("Team" = "team_name")) %>%
  mutate(csv_per_tv_household = total_controlled_surplus_value / tv_households * 1000000) %>%
  ungroup () %>%
  ggplot(aes(reorder(Team, csv_per_tv_household), csv_per_tv_household)) +
  geom_col() + coord_flip() + fgt +
  scale_y_continuous(labels = comma) + labs(y="Controlled Surplus Value per TV Household ($)", x="")
