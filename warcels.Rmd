...
all_players <-
  ...
  complete(nesting(PlayerId, Name, birth_year, Team, Position), Season = c(
    ...
    next_year, next_year + 1, next_year + 2, next_year + 3,
    next_year + 4
  ), fill = list(ValueW = 0)) %>%
  ...
  arrange(Season) %>%
  mutate(
    step1 = (0.6 * lag(ValueW)) + (0.3 * lag(ValueW, 2)) + (0.1 * lag(ValueW, 3)),
    step2 = 0.8 * step1,
    ValueW = case_when(
      Season == next_year ~ 
        case_when(Position %in% c("SP", "RP") ~ step2 + ((26 - age) * 0.1),
                  TRUE ~ step2 + ((30 - age) * 0.1)),
      TRUE ~ ValueW
    ))
      
# TODO: fill in each of ths missing cases when Season == next-year + 1, etc.
...
projected_war <-
  ...
  mutate(
    ...
    war_two_years_ago = lag(ValueW),
    war_three_years_ago = lag(ValueW, 2)) %>%
  
  #retain the last row, which has all the info we want
