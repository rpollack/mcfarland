...
* for pre-arb players, minimum salary is used.
* for arb players, MLBTR projections are used. arb-1s are increased by XX%, 
...
warcels <- all_players %>%
  ...
  mutate(age_now = this_year - as.integer(birth_year),
         ...
         Position = ifelse(Position == "SP", "Rotation",
                           ifelse(Position == "RP", "Bullpen", "Position")),
...
ggplot(data=team_war %>% filter(Position == "Position"), aes(years_of_control, war_under_control, label=Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="avg. years of control (position players only; 5 max)") + ggtitle("Avg. Length of Control vs. How Much WAR they Control, Position Players") + xlim(3,5)

ggplot(data=team_war %>% filter(Position == "Rotation"), aes(years_of_control, war_under_control, label=Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="avg. years of control (rotation only; 5 max)") + ggtitle("Avg. Length of Control vs. How Much WAR they Control, Rotation") + xlim(3,5)

ggplot(data=team_war %>% filter(Position == "Bullpen"), aes(years_of_control, war_under_control, label=Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="avg. years of control (bullpen only; 5 max)") + ggtitle("Avg. Length of Control vs. How Much WAR they Control, Bullpen") + xlim(3,5)
