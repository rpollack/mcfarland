...
warcels <- all_players %>%
  ...
  mutate(age_now = this_year - as.integer(birth_year),
         ...
         war_per_year_of_control = war_under_control / years_of_control) %>%
  filter(Season == max(Season)) %>%    #get only the latest projections
  select(FullName, Position, Team, years_of_control, war_under_control, war_per_year_of_control, salary_under_control, dollars_per_win_under_control) %>%
...
ggplot(data=team_war %>% filter(Position == "Hitter"), aes(years_of_control, war_under_control_per_year, label=Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="avg. years of control (position players only; 5 max)") + ggtitle("Avg. Length of Control vs. How Much WAR they Control, Position Players") + xlim(2.5,5)

ggplot(data=team_war %>% filter(Position == "Rotation"), aes(years_of_control, war_under_control_per_year, label=Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="avg. years of control (rotation only; 5 max)") + ggtitle("Avg. Length of Control vs. How Much WAR they Control, Rotation") + xlim(2.5,5)

ggplot(data=team_war %>% filter(Position == "Bullpen"), aes(years_of_control, war_under_control_per_year, label=Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="avg. years of control (bullpen only; 5 max)") + ggtitle("Avg. Length of Control vs. How Much WAR they Control, Bullpen") + xlim(2.5,5)
