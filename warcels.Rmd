...
team_info <- tibble(
  team_name = c("ARI", "ATL", "BAL", "BOS", "CHC",
                "CHW", "CIN", "CLE", "COL", "DET",
                "HOU", "KCR", "LAA", "LAD", "MIA",
                "MIL", "MIN", "NYM", "NYY", "OAK",
                "PHI", "PIT", "SDP", "SEA", "SFG",
                "STL", "TBR", "TEX", "TOR", "WSN"),
  team_url = c("arizona-diamondbacks", "atlanta-braves", "baltimore-orioles", "boston-red-sox", "chicago-cubs",
  ...
  team_color = c("#A71930", "#002F5F", "#ED4C09", "#C60C30", "#003279",
  ...
  tv_households = c(1890100, 2412730, 1119480, 2424240, 3463060/2,
  ...
  payroll_opening_day_2018 = c(131565116, 118284851, 148574615, 233200429, 183156139,
...
To update this this graph for the coming season, increment all year numbers by 1.

```{r}

# project five-year WAR for all players, then gather into tidy DF
# use WARcels methodology, detailed here: http://tangotiger.com/index.php/site/comments/war-marcels-warcels
...
projected_war <- 
  ...
  mutate(age_now = this_year - as.integer(birth_year),
         ...
         age_in_five_years = age_now + 5,
         
         # retain the last 3 years of a player's WAR for the purposes of projecting forward
         `2018` = ifelse(is.na(ValueW) == FALSE, ValueW, 1),  #players w/o appearances in a season get 1 WAR for that year
         `2017` = ifelse(is.na(lag(ValueW)) == FALSE, lag(ValueW), 1),
         `2016` = ifelse(is.na(lag(ValueW, 2)) == FALSE, lag(ValueW, 2), 1)) %>%
  filter(Season == this_year) %>%  # the row with the player's most recent season now contains information needed to project WAR for the next 5 years, which is all we want
  mutate(step1 = (0.6 * `2018`) + (0.3 * `2017`) + (0.1 * `2016`),
         step2 = 0.8 * step1,
         `2019` = round(ifelse(Position == "SP" | Position == "RP", 
                               step2 + ((26 - age_next_year) * 0.1),  # 2019 - birth_year.   age_now = 2018-birth_year
                               step2 + ((30 - age_next_year) * 0.1)), 1),
         `2020` = round(ifelse(Position == "SP" | Position == "RP", 
                               `2018` - 0.4,
                               `2018` - 0.4 + ((30-age_in_two_years) * .08)), 1),
         `2021` = round(ifelse(Position == "SP" | Position == "RP", 
                               `2019` - 0.4,
                               `2019` - 0.4 + ((30-age_in_three_years) * .03)), 1),
         `2022` = round(ifelse(Position == "SP" | Position == "RP",
                               `2020` - 0.4,
                               `2020` - 0.4 + ((30-age_in_four_years) * .03)), 1),
         `2023` = round(ifelse(Position == "SP" | Position == "RP", 
                               `2021` - 0.4,
                               `2021` - 0.4 + ((30-age_in_five_years) * .03)), 1)) %>% 
  dplyr::select(PlayerId, Name, Team, birth_year, Position, `2019`, `2020`, `2021`, `2022`, `2023`) %>%
  gather(Season, ValueW, contains("20")) %>%
  mutate(Season = as.integer(Season)) %>%
  dplyr::select(PlayerId, Name, Season, Team, ValueW, birth_year, Position)
...
master_list <-
  ...
  dplyr::select(PlayerId, Name, Team, Acquired, Position, Season, WAR = ValueW, Salary = salary) %>%
  group_by(PlayerId, Name) %>%
  ...
  mutate(
    ...
    Salary = as.numeric(Salary) * (1.022^(Season - next_year)), # inflate salary by 2.38% annually
    surplus_value = ((millions_per_war * (1.05^(Season - next_year))) * WAR) - Salary) %>% # inflate $/WAR by 5% annually
  ungroup() %>%
...
### plot controlled surplus value by team

```{r}
warcels_team_2019 <-
  warcels %>%
  group_by(Team) %>%
  summarize(total_surplus_value_under_control = sum(surplus_value_under_control, na.rm = TRUE))
...
warcels_team_2019 %>%
  ...
  ggtitle("Controlled Surplus Value, MLB Teams 2019 - 2023")

```

### Team WAR per Season thru 2022
...
team_war_per_season <-
  ...
  summarize(controlled_war = sum(WAR),
            controlled_surplus_value = sum(surplus_value, na.rm = TRUE)) %>%
  mutate(label = ifelse(Team %in% c("NYY", "CLE", "HOU", "LAD", "CHC", "WSN") &
                          Season == min(Season),
                        Team,
                        NA_character_))
...
team_war_per_season %>%
  ...
  ggtitle("Team Controlled WAR Per Season, 2019 - 2023") + 
...
team_war_per_season %>%
  ...
  labs(y="Surplus Value Under Control (Millions USD)", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + 
  facet_wrap(~Division, ncol = 2, scales = "free_y") +
  ggtitle("Team Controlled Surplus Value Per Season, 2019 - 2023") + 
  geom_label_repel(aes(label=label),
                   na.rm = TRUE,
                   show.legend = FALSE,
                   color="white",
...
### WAR graphs for a couple divisions

```{r}
team_war_per_season %>%
  filter(Division == "NL Central") %>%
  ggplot(aes(Season, controlled_war, color=Team, fill=Team)) + geom_point() + geom_line() +
  labs(y="WAR Under Control", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + 
  ggtitle("NL Central Controlled WAR Per Season, 2018 - 2022") + 
  geom_label_repel(aes(label=label),
                   na.rm = TRUE,
                   show.legend = FALSE,
                   color="white",
...
team_war_per_season %>%
  ...
  ggplot(aes(Season, controlled_war, color=Team, fill=Team)) + geom_point() + geom_line() +
  labs(y="WAR Under Control", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + 
  ggtitle("AL Central Controlled WAR Per Season, 2018 - 2022") + 
  geom_label_repel(aes(label=label),
                   na.rm = TRUE,
                   show.legend = FALSE,
                   color="white",
...
### team controlled surplus value per season through 2022


### MLB Team Control over Talent
...
warcels %>%
  ...
  summarize(num_players = n(),
            controlled_years_per_player = sum(controlled_years) / num_players,
            controlled_war_per_year = mean(war_per_year_of_control)) %>%
  ggplot(aes(controlled_years_per_player, controlled_war_per_year, color=Team, fill=Team)) + 
  geom_point() + 
  geom_label_repel(aes(label=Team),
                   ...
                   segment.color = "black",
                   nudge_y = .1) +
  ggtitle("MLB Team Outlook, 2019 – 2023") + 
  scale_color_manual(values = team_colors) + fgt +
  scale_fill_manual(values = team_colors) +
  labs(x="Control of Average Player (Years)", y="Proj. WAR per Year of Average Player", 
...
surplus_value_change_plt %>%
  ...
  ggtitle("Largest Reductions in Controlled Surplus Value, 2018-2019") + fgt
...
surplus_value_change_plt %>%
  ...
  geom_col() + coord_flip() +
  labs(x="", y="Change in Controlled Surplus Value (Millions USD)") +
  ggtitle("Largest Gains in Controlled Surplus Value, 2018-2019") + fgt
...
```{r}
#player surplus value changes 2017 - 2017
player_surplus_value_changes <-
  warcels_2018_2018 %>%
  mutate(surplus_value_change = round(surplus_value_under_control_2018 - surplus_value_under_control_2017, 1)) %>%
  dplyr::select(Name, war_under_control_2017, war_under_control_2018, salary_under_control_2017, salary_under_control_2018,
         surplus_value_under_control_2017, surplus_value_under_control_2018, surplus_value_change) %>%
  
  # jose ramirez has two rows in here; need to zero out one 
  mutate(surplus_value_change = ifelse(Name == "Jose Ramirez" & war_under_control_2017 == 2.0, 0, surplus_value_change)) %>%
  arrange(desc(surplus_value_change))

#largest surplus value increases
player_surplus_value_changes %>%
  head(10) %>%
  ggplot(aes(reorder(Name, surplus_value_change), surplus_value_change)) + geom_col() + coord_flip() + fgt +
  labs(x="", y="Surplus Value Change (Millions USD)", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + 
  ggtitle("Largest Increases in Controlled Surplus Value\n2017-2021 Period to 2018-2022 Period")

#largest surplus value decreases
player_surplus_value_changes %>%
  tail(10) %>%
  ggplot(aes(reorder(Name, -surplus_value_change), surplus_value_change)) + geom_col() + coord_flip() + 
  labs(x="", y="Surplus Value Change (Millions USD)", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + 
  ggtitle("Largest Decreases in Controlled Surplus Value\n2017-2021 Period to 2018-2022 Period")

```

### surplus value changes by team

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
team_surplus_value_changes <-
  read_csv("warcels-2018.csv") %>%
  group_by(Team) %>%
  summarize(total_surplus_value_under_control = sum(controlled_surplus_value)) %>%
  inner_join(warcels_team_2018, by="Team", suffix=c("_2018", "_2019")) %>%
  mutate(surplus_value_change = total_surplus_value_under_control_2018 - total_surplus_value_under_control_2017)

team_surplus_value_changes %>%
  ggplot(aes(reorder(Team, surplus_value_change), surplus_value_change)) + geom_col() + fgt + coord_flip() + 
  labs(x="", y="Surplus Value Change (Millions USD)", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + ggtitle("MLB Team Surplus Value Changes, 2017-2021 Period to 2018-2022 Period")

team_surplus_value_changes %>%
  arrange(desc(surplus_value_change)) %>%
  tail(10) %>%
  ggplot(aes(reorder(Team, -surplus_value_change), surplus_value_change)) + geom_col() + coord_flip() + 
  labs(x="", y="Surplus Value Change (Millions USD)", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + ggtitle("10 Largest Drops in Controlled Surplus Value\n2017-2021 Period to 2018-2022 Period")



```

### 2018 controlled surplus value dollar per TV household
...
warcels_team_2018 %>%
  ...
  scale_fill_manual(values = team_colors) + 
  labs(x="", y="Controlled Surplus Value (Millions USD)", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") +
  ggtitle("Controlled Surplus Value, MLB Teams 2018 - 2022")


inner_join(warcels_team_2018, team_info, by=c("Team" = "team_name")) %>%
  mutate(surplus_value_per_tv_household = total_surplus_value_under_control / (tv_households / 100000)) %>%
  ggplot(aes(reorder(Team, surplus_value_per_tv_household), surplus_value_per_tv_household)) + geom_col() +
  coord_flip() + labs(x="", y="Controlled Surplus Value per 100k TV Households in DMA (Millions USD)")
...
inner_join(warcels_team_2018, team_info, by=c("Team" = "team_name")) %>%
  mutate(tv_households_per_csv_dollar = tv_households / total_surplus_value_under_control) %>%
  ggplot(aes(reorder(Team, tv_households_per_csv_dollar), tv_households_per_csv_dollar)) + geom_col() +
  coord_flip() + labs(x="", y="TV Households per CSV Dollar") + scale_y_continuous(labels=comma)

#THIS graph shows that BAL has a large population to be saddled with so little surplus value. this tell us the team is spending disproportionately to its market size

# negative numbers don't make sense though. you can't have negative TV households. how to fix?



ggplot(team_info, aes(reorder(team_name, tv_households), tv_households)) + geom_col() + coord_flip() +
  labs(x="", y="TV Households in DMA") + scale_y_continuous(labels = comma) +
  ggtitle("TV-Viewing Households in MLB Team Markets, 2016")


# what am i trying to demonstrate here? 
...
ggplot(team_data, aes(tv_households, total_surplus_value_under_control, color=Team, fill = Team)) + geom_point() + 
  ...
  labs(x="TV-viewing Households in Designated Market Area", y="Controlled Surplus Value (Millions USD)",
       caption="* 2016 - 2017, per Nielsen\n** 2018 - 2022") +
  theme(legend.position = "None")


ggplot(team_data, aes(tv_households, payroll_opening_day_2018, color=Team,
                      fill = Team)) + geom_point() + 
  geom_label_repel(aes(label=Team),
                   na.rm = TRUE,
                   show.legend = FALSE,
                   color="white",
  ...
  labs(x="TV-viewing Households in Designated Market Area", y="2018 Opening Day Payroll (Millions USD) **",
