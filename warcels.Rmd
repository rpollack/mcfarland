...
all_teams_payroll <- all_teams_payroll %>%
  mutate(total_contract = str_extract(`Contract Status`, "\\$.*M"),
         ...
         aav = round(total_contract / contract_length, 2))
...
fg_batters_project <- fg_batters %>%
  ...
  mutate(age_now = this_year - as.integer(birth_year),
         ...
         dollars_per_win_under_control = round(salary_under_control / war_under_control, 2)) %>%
  filter(Season == max(Season)) %>%    #get only the latest projections
  select(FullName, Team.y, years_of_control, war_under_control, salary_under_control, dollars_per_win_under_control) %>%
