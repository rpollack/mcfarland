...
master_list <-
  ...
  mutate( # handle type 3
    Salary = ifelse(Salary == "Arb-1", 1.36881 + (.26896 * (cumsum(WAR))), Salary)
  ) %>% # use linear model i computed elsewhere in this doc
  ungroup() %>%
  ...
  mutate(
    ...
    surplus_value = ((millions_per_war * (1.05^(Season - next_year))) * WAR) - Salary
  ) %>% # inflate $/WAR by 5% annually
  ungroup() %>%
...
warcels_full %>%
  ...
  ggtitle(glue("Change in MLB Team Controlled Surplus Value, {this_year} – {next_year}"))
...
# and it is the result of not just players changing teams, but players getting better.
# still i think there are weird holes. Oakland's salary change is high - probably bc Chapman got way better. But Semien having $40m in controlled salary
# doesn't look right. think there may be a bug there.
warcels_full %>%
  group_by(year_analyzed, Team) %>%
  summarize(
  ...
  ggtitle(glue("MLB Team Offseason Results"), subtitle = glue("{this_year} – {next_year}")) +
  labs(x = "Change in Controlled Salary (Millions USD)", y = "Change in Controlled WAR")

# team control over talent vs. the talent under control
warcels %>%
  group_by(Team) %>%
  summarize(
    num_players = n(),
  ...
  scale_fill_manual(values = team_colors) +
  labs(
    x = "Control of Average Player (Years)", y = "Proj. WAR per Year of Average Player",
...
player_surplus_value_changes <-
  ...
  dplyr::select(
    Name, war_under_control_2017, war_under_control_2018, salary_under_control_2017, salary_under_control_2018,
