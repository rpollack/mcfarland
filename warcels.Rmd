...
warcels <- master %>%
  ...
  filter(Year == max(Year),
         ...
         #attempt to filter out scrubs who won't play much while still penalizing Pujols, Wright, Choo, etc.
         (salary_under_control > 10.0)) %>%
...
ggplot(data=team_war, aes(avg_years_of_control, avg_war_under_control_pear_year, label=Team)) + geom_point() + geom_text(aes(label=Team),hjust=0, vjust=0) + labs(x="avg. years of control (entire team; 5 max)") + ggtitle("Avg. Length of Control vs. How Much WAR they Control, Entire Team")
