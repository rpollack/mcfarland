...
projected_war <- all_players %>%
  ...
  dplyr::select(PlayerId, Name, Team, birth_year, Position, `2018`, `2019`, `2020`, `2021`, `2022`) %>%
...
write_csv(warcels, "warcels-2018.csv")
```
...
team_war_per_season %>%
  ...
  labs(y="WAR Under Control", caption="Sources: Baseball Reference, FanGraphs, & Tom Tango") + facet_wrap(~Division, ncol = 2) +
  ggtitle("Team Controlled WAR Per Season, 2018 - 2022") + 
...
team_war_per_season %>%
  ...
  ggtitle("Team Controlled Surplus Value Per Season, 2018 - 2022") + 
  geom_label_repel(aes(label=label),
                   na.rm = TRUE,
                   show.legend = FALSE,
                   color="white",
...
warcels %>%
  ...
  geom_label_repel(aes(label=Team),
                   ...
                   segment.color = "black",
                   nudge_y = .1) +
  ggtitle("MLB Team Outlook, 2018 – 2022") + 
...
warcels_2017_2018 <- read_csv("warcels-2017.csv") %>%
  dplyr::select(Name, Team, war_under_control, salary_under_control, 
                surplus_value_under_control = controlled_surplus_value) %>%
  inner_join(warcels, by="Name", suffix=c("_2017", "_2018"))
...
team_surplus_value_changes <-
  read_csv("warcels-2017.csv") %>%
  group_by(Team) %>%
  summarize(total_surplus_value_under_control = sum(controlled_surplus_value)) %>%
  inner_join(warcels_team_2018, by="Team", suffix=c("_2017", "_2018")) %>%
...
team_data %>%
  ...
  ggplot(aes(tv_households, payroll_opening_day_2018, fill = Team, color=Team,
             size=total_surplus_value_under_control)) +
  geom_point() +
  scale_color_manual(values = team_colors) +
  scale_fill_manual(values = team_colors) +
  scale_x_continuous(labels = comma) +
