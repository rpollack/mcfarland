...
pitcher_stats <-
  ...
  ungroup() %>%
  dplyr::select(-G, -GS, -FIP_WAR, -`RA9-Wins`, franchise)
...
one_team_percentage_pit <-
  pitcher_stats %>%
  mutate(WAR = if_else(WAR < 0, 0, WAR)) %>% # consider negative WAR as zero so the math works
  group_by(PlayerId, franchise) %>%
  summarize(franchise_war = sum(WAR)) %>%
  mutate(one_team_pct = round(franchise_war / sum(franchise_war), 4)) %>%
  group_by(PlayerId) %>%
  filter(one_team_pct == max(one_team_pct)) %>%
  dplyr::select(PlayerId, one_team_franchise = franchise, one_team_pct) %>%
  ungroup()
...
pitchers_full <-
  ...
  inner_join(one_team_percentage_pit, by = "PlayerId")


batter_stats_q <-
  dbSendQuery(con, "select sb.playerid AS PlayerId, ValueW as WAR, Age, st.DBFranchId as franchise
...
batters_full <-
  ...
  ungroup() %>%
  distinct(PlayerId, Age, WAR) %>%
  inner_join(one_team_percentage_bat, by = "PlayerId")
...
war_through_age <-
  ...
  dplyr::select(playerid = PlayerId, name = Name, position, last_year, mean_year, scandal_type, age = Age, cumulative_war, one_team_franchise, one_team_pct, considered_hof, hof) %>%
...
hof_split <-
  ...
  filter(considered_hof) %>%
  dplyr::select(-playerid, -name, -considered_hof, -last_year, -one_team_franchise) %>%
