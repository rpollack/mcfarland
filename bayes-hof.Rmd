...
### different analysis
at each position, given player X has had a season between Y and Z WAR, what is the probability of HOF?
...
prob_num_seasons_in_range <- function(position, war_floor, war_ceiling, num_seasons){
  ...
  # debugging
  # print(sprintf("position: %s, war floor: %s, war ceiling: %s, number of seasons: %s", 
  #         position, war_floor, war_ceiling, num_seasons))
...
# debugging the function above

```{r}
hof_eligible_players_seasonal %>%
      filter(Position == position) %>%
      mutate(season_in_war_range = if_else(WAR >= 1 & WAR <= 3, 1, 0)) %>%
      group_by(PlayerId, Name, Position, HOF) %>%
      summarize(num_seasons_in_range = sum(season_in_war_range)) %>%
      filter(num_seasons_in_range == 10) %>%
      #print() %>%  #show us the players who meet the criteria
      ungroup() %>%
      count(HOF) %>% 
      complete(HOF = c("Yes", "No"), fill = list(n=0)) %>% # account for situations zero HOF'ers or zero non HOF'ers
