...
predictions %>%
  ...
  geom_path() +
  geom_hline(yintercept = 0.11, size = 1.1, color = "red", lty = "dashed") +
...
war_split <-
  ...
  filter(!active,
         cumulative_war >= 0.5) %>%
  dplyr::select(position, mean_year, age, cumulative_war) %>%
  initial_split(strata = cumulative_war)
...
war_recipe <-
  recipe(cumulative_war ~ ., data = war_train) %>%
  step_dummy(position)
...
# find the optimal values of tuning parameters using 10-fold cross validation
war_folds <- vfold_cv(war_train, strata = cumulative_war)
...
war_predictions <-
  war_res %>%
  collect_predictions() %>%
  mutate(residual = .pred - cumulative_war)

war_predictions %>%
  ggplot(aes(cumulative_war, .pred)) +
  geom_point() + geom_smooth(method = "lm")

war_predictions %>%
  ggplot(aes(cumulative_war, residual)) +
