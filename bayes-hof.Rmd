...
seasonal_war <- fg_db %>%
  ...
  dplyr::select(PlayerId, Name, last_year) %>%
...
prob_war_thru_age <- function(age, war){
  #number of players (all-time HOF eligible and HOF inductees) who, at age X or younger, have accumulated Y +/- 2 WAR
...
war_age_combinations <- war_through_age %>%
  ...
  separate(num_players, c("num_all_players_who_meet_criteria", 
                          "num_hof_players_who_meet_criteria"), sep = " ") %>%
  mutate(num_all_players_who_meet_criteria = as.numeric(num_all_players_who_meet_criteria),
...
bayes_df <- inner_join(war_through_age, war_age_combinations, by=c("Age", "cumulative_war")) %>%
  ...
  filter(Age == max(Age)) %>%
  mutate(p_e_b = num_hof_players_who_meet_criteria / num_players_in_hof,
         p_e = num_all_players_who_meet_criteria / num_hof_eligible_players,
         prob_hof = round(((p_hof * p_e_b) / p_e) * 100, 1)) %>%
