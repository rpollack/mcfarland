...
war_through_age <-
  ...
  mutate(
    ...
    considered_hof = if_else((last_year < 2022 - 5 & num_seasons >= 10) | Name == "Addie Joss", TRUE, FALSE),
  ) %>%
  ungroup() %>%
  distinct() %>%
  ...
  # TODO: remove temp code to patch in Ortiz as the 2022 HOF inductee
  mutate(hof = if_else(name == "David Ortiz", "elected", hof)) %>%
...
xgb_spec <-
  ...
  set_engine("xgboost", scale_pos_weight = 6.2) %>%
