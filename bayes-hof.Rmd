...
pitchers <- seasonal_war %>%
  filter(str_detect(Position, "P")) %>%
  unite(Name, FirstName, LastName, sep = " ") %>%
  mutate(last_year = year(ymd(LastGame)),
         Position = str_extract(Position, "^P"),    #extract primary position from dataset
         birth_year = year(ymd(BirthDate))) %>%   
  dplyr::select(PlayerId, Name, birth_year, last_year, Position) %>%
  inner_join(pitcher_info, by="PlayerId") 
...
war_through_age_batters <- batters %>%
...
bayes_hof_hitting <- inner_join(war_through_age_batters, war_age_combinations, by=c("Age", "cumulative_war", "Position")) %>%
