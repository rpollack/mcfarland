...
area_under_curve <- 
  final_res %>% 
    collect_predictions() %>% 
    mutate(pred_hof = as.numeric(.pred_class)) %>% 
    roc_auc(hof, pred_hof, event_level="second") %>%
...
final_res %>% 
  collect_predictions() %>% 
  mutate(pred_hof = as.numeric(.pred_class)) %>% 
  roc_curve(hof, pred_hof, event_level="second") %>% 
  ggplot(aes(1 - specificity, sensitivity)) +
  geom_abline(lty = "dashed", color = "gray80", size = 1.5) +
  geom_path() +
  ggtitle("ROC Curve, HOF Prediction Model",
