...
get_all_star_rosters <- function(year) {
  ...
  read_html(glue("https://www.baseball-reference.com/allstar/{year}-allstar-game.shtml")) %>%
    ...
    html_text() %>%
    str_extract_all("[:alpha:]{7}[:digit:].*(?=\\.shtml)") %>%
...
all_star_data <-
  bind_rows(all_star_data_first, all_star_data_second, all_star_data_third) %>%
  count(brefid, name = "num_asgs", sort = TRUE)
...
war_mvp_data <-
  ...
  left_join(all_star_data, by=c("refid" = "brefid")) %>%
