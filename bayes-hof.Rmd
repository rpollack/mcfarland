...
one_team_percentage_pit <-
  ...
  summarize(franchise_g = sum(G)) %>%
  group_by(PlayerId) %>%
  filter(franchise_g == max(franchise_g)) %>%
  dplyr::select(PlayerId, one_team_franchise = franchise, max_franchise_g = franchise_g) %>%
  ungroup()
...
war_through_age <-
  ...
  dplyr::select(playerid = PlayerId, name = Name, position, last_year, mean_year, scandal_type, age = Age, cumulative_war, one_team_franchise, max_franchise_g, considered_hof, hof) %>%
