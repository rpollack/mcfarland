...
#   )) %>%
  ...
  mutate(is_scandal = case_when(
    name %in% c("Barry Bonds", "Roger Clemens", "Joe Jackson", "Robinson Cano", "Pete Rose", "Alex Rodriguez", "Manny Ramirez", "Mark McGwire", "Sammy Sosa", "Jose Canseco", "Rafael Palmeiro", "Jason Giambi", "Gary Sheffield", "Miguel Tejada", "Nelson Cruz", "Andy Pettite", "Ryan Braun", "Carlos Beltran", "Jose Altuve", "Alex Bregman", "Melky Cabrera") ~ 1,
    TRUE ~ 0
  )) %>%
  inner_join(positional_info, by=c("playerid" = "PlayerId")) %>%
  select(playerid, name, position = Position, last_year, is_scandal, age, cumulative_war, considered_hof, hof)
...
hof_split <-
  ...
  # train model only on players who have
  filter(considered_hof) %>%
  dplyr::select(-playerid, -name, -considered_hof, -last_year) %>%
  mutate(hof = factor(hof)) %>%
...
nnet_fit <-
  mlp(epochs = 100, hidden_units = 25, dropout = 0.1) %>%
...
hof_results <-
  hof_test %>%
  bind_cols(predict(nnet_fit, new_data = .)) %>%
  mutate(pred_hof = if_else(.pred_class == "elected", 1, 0))

au_prc <-
  hof_results %>%
  pr_auc(truth = hof, pred_hof) %>%
  select(.estimate) %>%
  as.numeric() %>%
  round(2)
...
hof_results %>%
  pr_curve(truth = hof, pred_hof) %>%
  ggplot(aes(recall, precision)) +
  geom_path() +
  coord_equal() +
  ggtitle("Precision-Recall Curve, HOF Model",
          subtitle = glue("Area under the curve: {au_prc}"))
...
```{r}
# predict HOF for everyone 

year_cutoff <- 1970
num_players <- 10

war_through_age %>%
  filter(hof == "not_elected",
         last_year >= year_cutoff) %>%
  group_by(playerid) %>%
  filter(age == max(age)) %>%
  select(playerid, name, age, position,cumulative_war, is_scandal) %>%
  ungroup() %>%
  bind_cols(predict(nnet_fit, new_data = .),
            predict(nnet_fit, new_data = ., type = "prob")) %>%
  ...
  ggtitle(glue("Top {num_players} players not in HOF, by probability of election"),
          subtitle = glue("Retired {year_cutoff} or later, or still active"))
