...
war_through_age <-
  ...
  replace_na(list(HOF = FALSE)) %>%
  inner_join(positional_info, by="PlayerId") %>%
   group_by(PlayerId, Name, last_year, Position) %>%
  arrange(Age) %>%
  mutate(
    cumulative_war = round(cumsum(WAR), 1),
    considered_hof = if_else(last_year <= current_year - 6, TRUE, FALSE)
  ) %>%
  dplyr::select(playerid = PlayerId, name = Name, position = Position, last_year, age = Age,
                cumulative_war, considered_hof, hof = HOF) %>%
  ungroup() %>%
  filter(!is.na(age)) %>%
  distinct() %>%
  
  # add in a unique identifier for each row. pivot_wider requires this
  mutate(row_num = row_number()) %>%
  pivot_wider(names_from = "age", values_from = "cumulative_war",
              names_glue = "war_{age}") %>%
  dplyr::select(-row_num) %>%
  group_by(playerid, name, position, last_year, considered_hof, hof) %>%
  summarize(across(contains("war_")), total = sum(., na.rm = TRUE))

# TODO: the above code should give me one row per player. instead it gives me n rows 
# where n is the number of seasons, one cell filled in per season.

## ohh what i think is happening is that there are no unique ID's outside of Age and cumue_war
