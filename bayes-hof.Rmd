...
bayes_hof <- war_through_age %>%
  ...
  dplyr::select(PlayerId, Name, prob_hof, cumulative_war, Age, Position, last_year, everything(), -WAR, 
...
# wrapped in a tryCatch because sometimes no one is in that range.
prob_num_seasons_in_range <- function(position, war_floor, war_ceiling, num_seasons){
  
  # need to get players' names out of it, too
  tryCatch(
...
prob_num_seasons_in_range <- Vectorize(prob_num_seasons_in_range)
...
pos = c("1B", "2B", "3B", "SS", "OF", "C", "P")
wl <- c(1, 3, 5, 7, 9, 11)
wh <- c(3, 5, 7, 9, 11, 13)
n = c(seq(1:25))

df <- expand.grid(position = pos,
                  war_floor = wl, 
                  war_ceiling = wh, 
                  num_seasons_to_analyze = n) %>% 
  filter(war_ceiling - war_floor == 2) %>% #retain only valid WAR ranges
  as_tibble()
# 
# 
# data_initial <- 
...
#          war_floor = war_lo,
#          war_ceiling = war_hi,
#          num_seasons_to_analyze = c(seq(1:25)))

# TODO: find a way to generate a single tibble with multiple WAR ranges inside of it that each look for
# a match with 1 season all the way thru 25 seasons. the tibble above has only 1 WAR range which is limiting
# then the graph would be: x-axis is number of seasons in range, y-axis is probability, and color or size
# of bubble is the WAR range. something like that.

# TODO: find a way to correctly convey when p_hof = is NaN. can't graph this but need to convey it somehow
...
df %>%
  rowwise() %>%
  mutate(non_hof_who_meet_criteria = prob_num_seasons_in_range(position, war_floor, 
                                                               war_ceiling, num_seasons_to_analyze)[1],
         hof_who_meet_criteria = prob_num_seasons_in_range(position, war_floor, 
                                                               war_ceiling, num_seasons_to_analyze)[2],
         total_who_meet_criteria = non_hof_who_meet_criteria + hof_who_meet_criteria) %>%
  inner_join(base_hof_prob, by="position") %>%
  mutate(p_b = p_hof_at_position,
  ...
  dplyr::select(position, war_floor, war_ceiling, num_seasons_to_analyze, p_hof, everything()) %>%
  View()
