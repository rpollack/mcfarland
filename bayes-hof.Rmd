...
```{r}
# regex for finding bb-ref player ID's. between 3 and 7 alpha chars followed by exactly 2 digits. 
bref_id_regex <- "[:alpha:]{3,7}[:digit:]{2}"
...
```{r}
awards_html <- read_html("https://www.baseball-reference.com/awards/awards_2021.shtml#NL_CYA_voting_link") %>%
...
# TODO: let's start by doing from 1967-on and going from there. 
# TODO: then handle from 1956 to 1966 when voting was league-wide
...
bind_cols(cya_ids, cya_points)

# data-stat="points_won" >3.0
...
war_through_age <-
  ...
  mutate(
    ...
    considered_hof = if_else((last_year <= most_recent_year - 6 & num_seasons >= 10) | Name == "Addie Joss", TRUE, FALSE),
  ) %>%
  ungroup() %>%
  distinct() %>%
  ...
  dplyr::select(playerid = PlayerId, name = Name, position, last_year, mean_year, scandal_type, age = Age, cumulative_war, max_franchise_g, cumulative_mvp_points, cumulative_asg, considered_hof, hof) %>%
