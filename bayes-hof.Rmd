...
hof_players <-
  ...
  mutate(
    ...
    # correct some discrepancies between the datasets
...
#TODO: how to avoid including batting WAR for pitchers? this causes joel peralta, mariano rivera, fernando rodney, etc. to show up and muck up the dataset.
...
war_through_age <-
  ...
  # left_join(batter_stats, by = "PlayerId") %>% 
  # left_join(pitcher_stats, by = c("PlayerId", "Age") %>% View()
  # replace_na(list(WAR_bat = 0, WAR_pitch = 0)) %>%
  inner_join(positional_info, by = "PlayerId") %>% 
 # mutate(WAR = if_else(Position == "P", WAR_pitch, WAR_bat)) %>% View()
  ...
  mutate(
    ...
    considered_hof = if_else((last_year <= current_year - 6 & num_seasons >= 10) | Name %in% c("Addie Joss", "Satchel Paige", "Monte Irvin") , TRUE, FALSE),
  ) %>%
  ungroup() %>%
  distinct() %>%
  ...
  dplyr::select(playerid = PlayerId, name = Name, position = Position, last_year, mean_year, scandal_type, age = Age, cumulative_war, considered_hof, hof = HOF) %>%
  ...
  distinct() %>%
  filter(!name %in% c("Satchel Paige", "Monte Irvin")) %>%
  na.omit()
...
war_through_age %>%
  ...
  filter(hof == "elected") %>%
  ggplot(aes(decade, prop)) + facet_wrap(vars(position), scales = "free_y") +
...
predictions %>%
  ...
  geom_path() +
  geom_hline(yintercept = 0.079, size = 1.1, color = "red", lty = "dashed") +
...
war_through_age %>%
  ...
  ggtitle(glue("Probability of HOF election"), subtitle = "Active or retired but not voted on yet. 25% = Predicted Hall of Famer")
...
war_through_age %>%
  group_by(playerid) %>%
  filter(age == max(age)) %>%
  ggplot(aes(cumulative_war)) + geom_histogram()
...
war_shares_data <-
  ...
  filter(age == max(age),
         position != "P",
         mean_year >= 1931) # played most of their career after 1931 when the award was first given
...
war_shares_data %>% 
  ungroup() %>%
  filter(mvp_shares > 0) %>% 
  arrange(cumulative_war) %>%
  filter(!playerid %in% c(1009178, 1011218, 1012976, 1013976, 15103, 6141, 1000379)) %>%  # get rid of 'impostor' players
  mutate(shares_per_war = round(mvp_shares/cumulative_war, 3)) %>%
  arrange(shares_per_war) %>%
  slice_head(n=25) %>%
  dplyr::select(playerid, name, cumulative_war, mvp_shares, shares_per_war)
