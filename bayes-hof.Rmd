...
war_mvp_data <-
  mvp_voting_data %>%
  mutate(
    player_name = str_replace_all(player_name, "\\s", " "), # fix non-breaking spaces
    player_name =
      str_replace_all(player_name, c(
        "Cal Ripken Jr." = "Cal Ripken",
        "Henry Aaron" = "Hank Aaron"
      ))
  ) %>%
  group_by(player_name) %>%
  summarize(total_mvp_points = sum(mvp_points)) %>%
  right_join(war_through_age %>%
    group_by(playerid) %>%
    filter(age == max(age)),
  ...
  dplyr::select(playerid, player_name, cumulative_war, total_mvp_points, max_franchise_g) %>%
