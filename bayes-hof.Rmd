...
pitcher_stats_q <-
  dbSendQuery(con, "select spmp.PlayerId, ValueW as FIP_WAR, \`RA9-Wins\`, Age, spmp.GS, spmp.G, st.DBFranchId as franchise
...
one_team_percentage_pit <-
  ...
  dplyr::select(-G, -franchise, -franchise_g)
...
batter_stats_q <-
  dbSendQuery(con, "select sb.playerid AS PlayerId, ValueW as WAR, Age, sb.G, st.DBFranchId as franchise
...
batters_full <-
  ...
  ungroup() %>%
  distinct(PlayerId, Age, WAR) %>%
  inner_join(one_team_percentage_bat, by = c("PlayerId", "Age", "WAR"))
...
batter_stats %>% 
  ...
  # TODO: this doesn't handle Sheffield's case in MIL where his WAR total decreased his last year.
  # his franchise WAR for MIL should be 1.25 in his time there. so instead of taking the max, take the last value with the franchise.

  # TODO: test with players who return to a franchise, like Ichiro.
  
  # taking the max always works with G because they incrase. but WAR can decrease.
...
# sure this WORKS, but ... what value to keep?

# at each age, keep the most number of games played for a team TO THAT POINT. so for Trea it'll be WSN up until he surpasses 637 G with LA (or another franchise.)

# for sheffield, it should change: it'll be MIL until he is 27, then change to FLA 383 at age 27, then get to FLA 558 at age 29, then stay that way forever. 

# for molina, ripken jr -- it'll be constant throughout. 
```
...
war_through_age <-
  ...
  dplyr::select(playerid = PlayerId, name = Name, position, last_year, mean_year, scandal_type, age = Age, cumulative_war, max_franchise_g, considered_hof, hof) %>%
...
```{r}
# build a specification. set scale_pos_weight to 6, this is num(not_elected) / num(elected)
xgb_spec <-
  boost_tree(
    trees = 1000,
  ...
  set_engine("xgboost", scale_pos_weight = 6.18) %>%
...
xgb_recipe <-
  ...
  step_log(max_franchise_g) # log-transform this very skewed predictor
...
war_through_age %>%
  ...
  ggtitle(glue("Probability of HOF election"), subtitle = glue("Active or retired but not voted on yet. {optimal_threshold}% = Predicted Hall of Famer"))
...
# TODO: there are multiple players names Randy Johnson, Jose Ramirez, Bob Gibson, Chris Carpenter, Pete Rose, Frank Thomas, Joe Morgan, Billy Williams, Boog Powell, earl averill, Tony Gwynn, Frank Thomas, Randy Johnson, Luis Gonzalez, Reggie Sanders, Bill Doran, Frank Thomas, John Mayberry, Bernie Williams,   ...going to have to join by player ID's here!

# TODO: how to find duplicate players: war_mvp_data %>% arrange(player_name) %>% filter(player_name == dplyr::lag(player_name)) %>% View()

# get mvp voting by year
...
mvp_voting_data <-
  ...
  count(value, wt = mvp_points, name = "total_mvp_points", sort = TRUE)
...
playerid_q <- dbSendQuery(con, "select playerid, refid from playerid_lookup")

player_ids <-
  dbFetch(playerid_q) %>%
  as_tibble()

# TODO: not sure if i can fix this at all, but a lot of guys like mookie betts, mike trout, etc do not have ref ID's -- so they will show up as having zero MVP points. 

dbClearResult(playerid_q)
...
more_player_ids <-
  ...
  bind_rows(player_ids) %>%
  filter(!is.na(refid)) %>%
  distinct()

# combine datasets for non-pitchers who played most of their careers 1931 and later
war_mvp_data <-
  war_through_age %>%
  filter(position != "P",
         mean_year >= 1931) %>%
  group_by(playerid) %>%
  filter(age == max(age)) %>%
  dplyr::select(playerid, name, position, cumulative_war) %>%
  inner_join(more_player_ids, by = "playerid") %>%
  left_join(mvp_voting_data, by=c("refid" = "value")) %>%
  replace_na(list(total_mvp_points = 0)) %>%
  mutate(points_per_war = total_mvp_points / cumulative_war)
```
...
war_mvp_data %>%
  ggplot(aes(cumulative_war, total_mvp_points)) +
  geom_point() +
  geom_smooth(method = "lm")

# best players to get no MVP points

war_mvp_data %>%
  filter(total_mvp_points == 0) %>%
