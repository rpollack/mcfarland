...
# also set the war floor & ceiling appropriately. this matters because it controls which players are compared
# to each other. starting with a 5-WAR range
...
war_through_age <-
  ...
  mutate(
    ...
    num_seasons = seq(n()),
    war_ceiling = cumulative_war + (1.1 * num_seasons),
    war_floor = cumulative_war - (1.1 * num_seasons)
  )
    ...
    hof_at_position = ifelse(Position == "1B", 24,
      ifelse(Position == "2B", 21,
        ifelse(Position == "3B", 17,
          ifelse(Position == "SS", 26,
            ifelse(Position == "C", 19,
              ifelse(Position == "OF", 72,
                ifelse(Position == "DH", 2,
...
# compute HOF probabilities for all players
bayes_hof <- 
  war_through_age %>%
  filter(HOF == "No") %>%
  group_by(PlayerId) %>%
