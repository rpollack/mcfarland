...
# create model specification with placeholders for tuned parameer values
nnet_spec <-
...
new_data <-
  ...
  group_by(playerid) %>%
  filter(age == max(age)) %>%
  dplyr::select(playerid, name, age, position,cumulative_war, is_scandal) %>%
  ungroup() %>%
  
  
  bind_cols(predict(final_wf, new_data = .),
            predict(final_wf, new_data = ., type = "prob")) %>%
