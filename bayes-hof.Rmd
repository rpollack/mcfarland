...
war_through_age <-
  ...
  left_join(mvp_voting_seasonal, by=c("refid" = "value",
                                      "Season" = "season")) %>%
  left_join(all_star_seasonal, by=c("refid" = "brefid",
                                     "Season")) %>%
  ...
  mutate(
    ...
    cume_mvp_points = cumsum(mvp_points),
    cume_asg = cumsum(asg),
  ...
  dplyr::select(playerid = PlayerId, name = Name, position, last_year, mean_year, scandal_type, age = Age, cumulative_war, max_franchise_g, cume_mvp_points, cume_asg, considered_hof, hof) %>%
