...
war_through_age <-
  ...
  mutate(
    ...
    considered_hof = if_else((last_year <= 2022 - 6 & num_seasons >= 10) | Name == "Addie Joss", TRUE, FALSE),
  ) %>%
  ungroup() %>%
  distinct() %>%
  ...
  # temp code to patch in Ortiz as the 2022 HOF inductee
  mutate(hof = if_else(name == "David Ortiz", "elected", "not_elected"))
```
...
xgb_recipe <-
  ...
  step_log(max_franchise_g, contains("cumulative"), offset = .1) # log-transform skewed predictors
