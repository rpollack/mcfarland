...
seasonal_war <- fg_db %>%
  tbl("player_info") %>%
  dplyr::select(PlayerId, FirstName, LastName, LastGame, Position) %>%
  collect(n=Inf) %>%
  filter(!str_detect(Position, "P")) %>%
  unite(Name, FirstName, LastName, sep = " ") %>%
  mutate(last_year = year(ymd(LastGame)),
         Position = str_extract(Position, "^.{1,2}")) %>%   #extract primary position from dataset
  dplyr::select(PlayerId, Name, last_year, Position) %>%
...
#compute HOF probabilities at each position
...
bayes_hof <- inner_join(war_through_age, war_age_combinations, by=c("Age", "cumulative_war", "Position")) %>%
  ...
  mutate(p_e_b = num_hof_players_who_meet_criteria / num_players_in_hof,
         p_e = num_eligible_players_who_meet_criteria / num_hof_eligible_players,
