...
# create model specification with placeholders for tuned parameter values
nnet_spec <-
...
nnet_params <-
  grid_latin_hypercube(
    epochs(),
    hidden_units(),
    dropout(range(c(0.1,0.9)))
  )

nnet_folds <- vfold_cv(hof_train, strata = hof)
...
# execute cross-validation and tuning parameter combinations
nnet_res <-
  nnet_wf %>%
  tune_grid(resamples = hof_folds,
            grid = nnet_params,
            metrics = metric_set(pr_auc))

# pull the best parameters for the model
best_params <-
  nnet_res %>%
  select_best("pr_auc")

#finalize the model using these best parameters
final_wf <-
  nnet_wf %>%
...
# make predictions on the test set so we can evaluate the model
eval_predictions <-
  final_fit %>%
  collect_predictions() %>%
  mutate(pred_hof = if_else(.pred_class == "elected", 1, 0))

# evaluation: show confusion matrix
eval_conf_matrix <-
  eval_predictions %>% 
...
# show conf matrix metrics
eval_conf_matrix %>%
  summary() %>%
  ggplot(aes(.metric, .estimate)) +
  geom_col() + coord_flip() +
  ggtitle("Confusion Matrix Measurements")
...
# fit the final workflow on the training dataset so it can be used to predict.
hof_mdl <-
  final_wf %>%
  fit(hof_train)


# TODO: I think I need to 'extract the workflow' and then 'extract the fit'
# in order to get something i can use on my new data
...
unseen <-
  ...
  filter(age == max(age)) %>%
  ungroup() %>%
  dplyr::select(-playerid, -name, -considered_hof, -last_year, -num_seasons) %>%
  
  bind_cols(predict(hof_mdl, new_data = .),
