...
war_through_age <-
  ...
  dplyr::select(playerid = PlayerId, name = Name, position = Position, last_year, mean_year, scandal_type, age = Age, cumulative_war, considered_hof, hof) %>%
  ...
  mutate(
    position = case_when(
      ...
      name %in% c(
        ...
        "Jesse Burkett", "Joe Medwick", "Zack Wheat", "Jim O'Rourke", "Joe Kelley", "Heinie Manush"
      ) ~ "LF",
      name %in% c(
...
war_through_age %>%
  ...
  geom_hline(yintercept = 13.5, lty = "dashed") +
  ggtitle(glue("Probability of HOF election"), subtitle = "Active or retired but not voted on yet. 13.5% = Predicted Hall of Famer")
