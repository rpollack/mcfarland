...
data_grouped <-
  full_swinging_data %>%
  count(game_date, pitcher_id, pitcher, p_throws, is_swinging_strike, name="num_pitches") %>%
  group_by(game_date, pitcher_id, pitcher, p_throws) %>%
...
#  count(pitcher_id, pitcher, p_throws, percentage) %>%
  ggplot(aes(x=percentage, y = pitcher, height = stat(density))) + geom_density_ridges(stat = "binline", bins = 15) +
  labs(y="", x="Swinging Strike Rate on Breaking Pitches")
