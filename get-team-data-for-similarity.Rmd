...
ggplot(data=stats_raw, aes(x=Stat, y=Value, fill=Team)) + geom_bar(stat="identity", position="dodge") + scale_fill_manual(values=c("#79BDEE", "#002F6C")) + coord_flip() + labs(x="") + ggtitle("2016 Cubs vs. 2012 Rays")
...
ggplot(data=team_similarity_plus, aes(x=Stat, y=Value, fill=Team)) + geom_bar(stat="identity", position="dodge") + scale_fill_manual(values=c("#79BDEE", "#002F6C")) + coord_flip() + labs(x="", y = "Value (100 = League Average)") + ggtitle("2016 Cubs vs. 2012 Rays, Relative to League Average")
...
You'll note some real powerhouses and several World Series champions on this list. Teams that perform like the Cubs do, relative to their competition, typically win their divison (or league, as the case may be). The average winning percentage of this group is .589, or between 95-96 wins in today's game. And in other news ... water is wet.
