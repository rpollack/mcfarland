...
ggplot(data=stats_raw, aes(x=Stat, y=Value, fill=Team)) + geom_bar(stat="identity", position="dodge") + scale_fill_manual(values=c("#79BDEE", "#002F6C")) + coord_flip() + labs(x="") + ggtitle("2016 Cubs vs. 2012 Rays") + fgt

ggsave("cubs-rays-raw.png", last_plot())


tht_tbl <- spread(stats_raw, key = Team, value = Value)


write_csv(tht_tbl, "cubs-rays-tbl.csv")

```


  The large x-axis scale makes it difficult to see, but similarities are there. The teams' offenses struck out a similar number of times and hit the same number of triples. Their pitching staffs allowed a similar number of runs. Plus, they both employed Ben Zobrist.
...
ggplot(data=team_similarity_plus, aes(x=Stat, y=Value, fill=Team)) + geom_bar(stat="identity", position="dodge") + scale_fill_manual(values=c("#79BDEE", "#002F6C")) + coord_flip() + labs(x="", y = "Value (100 = League Average)") + ggtitle("2016 Cubs vs. 2012 Rays, Relative to their Leagues") + fgt

ggsave("cubs-rays-relative.png", last_plot())


tht_tbl_relative <- spread(team_similarity_plus, key = Team, value = Value)

write_csv(tht_tbl_relative, "cubs-rays-tbl-rel.csv")
...
similar_teams <- similar_teams %>%
  ...
  write_csv("similar_teams.csv")
similar_teams
```

Some real powerhouses, and a few World Series champions, make this list. The average winning percentage of this group is .589, or between 95-96 wins in today's game. Teams that perform like the Cubs do, relative to their competition, typically win their divison (or league, as the case may be). 

In other news ... water is wet. Good teams win lots of games. 
