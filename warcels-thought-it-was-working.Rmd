...
# TODO: merge in super_two guys so we know to put their last Arb year as Arb 4, not Arb 3
...
master_list <-
  ...
  group_by(Name) %>%
  arrange(Year) %>% 
  mutate(  ## bb-ref sometimes puts in "Arb". figure out which arb year it is, based on proximity to FA year)
      salary = ifelse(salary == "Arb" & lead(salary) == "FA",
                      "Arb-3",
                      salary),
      salary = ifelse(salary == "Arb" & lead(salary, 2) == "FA",
                      "Arb-2",
                      salary),
      salary = ifelse(salary == "Arb" & lead(salary, 3) == "FA",
                      "Arb-1",
                      salary)) %>%
...
# TODO: use WAR to project arb salaries
