...
getPayrolls <- function(Tm, team_name) {
  ...
  tms_melt <- tms_melt %>%
    ...
    filter(!salary %in% c("", "FA")) %>%           #remove years where players aren't on a payroll                                   
    mutate(Team = Tm)
}
```
...
control_information <- all_teams_payroll %>%
  filter(salary != "FA") %>%
  group_by(Name, Team) %>%
  mutate(years_before_arb_eligible = ifelse(salary == "Arb-1", as.integer(Year) - next_year, 0)) %>%
...
#bb-ref sometimes puts 'Arb' (w/o the year). figure out which arb it is based on proximity to FA year.
#a rare minority of players sign away arbitration.
all_teams_payroll <- all_teams_payroll %>%
  group_by(Name, Team) %>%
  arrange(Name, Year) %>%
  mutate(salary = ifelse(salary == "Arb", 
                        ifelse(lead(salary) == "FA", "Arb-3", salary),
                        salary),
         salary = ifelse(salary == "Arb", 
                        ifelse(lead(salary, 2) == "FA", "Arb-2", salary),
                        salary),
         salary = ifelse(salary == "Arb",
                        ifelse(lead(salary, 3) == "FA", "Arb-1", salary),
                        salary)) %>%
  filter(salary != "FA")
```
...
master <- inner_join(all_teams_payroll, control_information, by=c("Name", "Team")) %>%
  inner_join(projected_war, by=c("Name", "Team")) %>%
...
payroll_info <- master %>%
  ...
  mutate(salary = ifelse(Name == "Kevin Gausman" & salary == "signed", "$3.45M", salary),
         salary = ifelse(Name == "Cesar Hernandez" & salary == "signed", "$2.55M", salary),
         salary = ifelse(Name == "Collin McHugh" & salary == "signed", "$3.85M", salary),
         salary = ifelse(Name == "David Phelps" & salary == "Arb-2", "$4.6M", salary),
         salary = ifelse(Name == "Marcus Stroman" & salary == "signed", "$3.4M", salary),
         salary = ifelse(Name == "Marwin Gonzalez" & salary == "signed", "$3.725M", salary),
         salary = ifelse(Name == "Michael Wacha" & salary == "signed", "$2.775M", salary),
         salary = ifelse(Name == "Taijuan Walker" & salary == "signed", "$2.25M", salary),
         salary = ifelse(Name == "Wilmer Flores" & salary == "signed", "$2.2M", salary),
         salary = ifelse(Name == "Jake Odorizzi" & salary == "signed", "$4.1M", salary),
         
         #Texas is paying only $9M annually for Prince
         salary = ifelse(Name == "Prince Fielder", "$9M", salary)) %>%
  
  #get any explicit salaries, as well as pre-arb ones
  mutate(salary = ifelse(grepl("M", salary), as.double(str_extract(salary, "(?<=\\$)[:digit:]+\\.*[:digit:]*")), salary),
         salary = ifelse(grepl("k", salary), as.double(str_extract(salary, "(?<=\\$)[:digit:]+")) / 1000, salary),
         salary = ifelse(grepl("Pre-Arb", salary), 0.535, salary)) %>%
  
  
  #project arb-1 salaries, then use that to project future arb years
  mutate(salary = ifelse(salary == "Arb-1" & (years_before_arb_eligible == 1 | years_before_arb_eligible == 0), 1.36881 + (.26896 * (currentWAR + war_next)), 
                        ifelse(salary == "Arb-1" & years_before_arb_eligible == 2, 1.36881 + (.26896 * (currentWAR + war_next + war_in_two_years)),
                               ifelse(salary == "Arb-1" & years_before_arb_eligible == 3, 
                                      1.36881 + (.26896 * (currentWAR + war_next + war_in_two_years + war_in_three_years)), salary)))) %>%
  arrange(Name, Team, Year) %>%
  mutate(salary = ifelse(salary == "Arb-2", (as.double(lag(salary)) / .25) * .4, salary),
         salary = ifelse(salary == "Arb-3", (as.double(lag(salary)) / .4) * .6, salary),
         salary = ifelse(salary == "Arb-4", (as.double(lag(salary)) / .6) * .7, salary),
         salary = as.numeric(salary)) %>%
  group_by(Name, Team) %>%
  summarize(salary_under_control = round(sum(salary), 2))
```
...
### Look at surplus value per team for 2016-2017 FAs

```{r}
fa_warcels <- warcels %>%
  filter(Name %in% c("Edwin Encarnacion", "Jose Bautista", "Kenley Jansen", "Justin Turner", "Yoenis Cespedes",
                     "Aroldis Chapman", "Dexter Fowler", "Rich Hill", "Neil Walker", "Mark Melancon", 
                     "Jeremy Hellickson", "Ivan Nova", "Ian Desmond", "Carlos Gomez", "Josh Reddick", 
                     "Matt Holliday", "Mark Trumbo", "Edinson Volquez", "Carlos Beltran", "Matt Wieters",
                     "Brett Cecil", "Michael Saunders", "Mike Napoli", "Jason Hammel", "Andrew Cashner", 
                     "Wilson Ramos", "Greg Holland", "Luis Valbuena", "Kendrys Morales", "Brad Ziegler",
                     "Sergio Romo", "Kohi Uehara", "Derek Holland", "Bartolo Colon", "Rajai Davis", "Joe Blanton",
                     "Matt Joyce", "Colby Rasmus", "Brandon Moss", "Santiago Casilla", "Angel Pagan",
                     "Chase Utley", "Doug Fister", "R.A. Dickey", "Steve Pearce", "Colby Lewis", "Jon Jay",
                     "Pedro Alvarez", "Jonathan Papelbon", "Erick Aybar")) %>%
  mutate(millions_per_win = salary_under_control / war_under_control)

team_dollars_per_win <- fa_warcels %>%
  group_by(Team) %>%
  summarize(mean_millions_per_win = mean(millions_per_win))

ggplot(team_dollars_per_win, aes(reorder(Team, mean_millions_per_win), mean_millions_per_win)) + geom_bar(stat="identity") + coord_flip() + labs(x="Team", y="Cost per Controlled Free-Agent Win, 2016-2017 offseason")
