...
all_teams_payroll <- all_teams_payroll %>%
  group_by(Name, Team) %>%
  arrange(Name, Year) %>%
  mutate(value = ifelse(value == "Arb", 
                        ifelse(lead(value) == "FA", "Arb-3", value),
                        value),
         value = ifelse(value == "Arb", 
                        ifelse(lead(value, 2) == "FA", "Arb-2", value),
                        value),
         value = ifelse(value == "Arb",
...
payroll_info <- master %>%
  ...
  mutate(value = ifelse(Name == "Kevin Gausman" & value == "signed", "$3.45M", value),
         ...
         value = ifelse(Name == "Prince Fielder", "$9M", value)) %>%
...
warcels %>%
  select(Name, Team, years_of_control, Position, war_under_control, salary_under_control,
...
war_timeline <- master_full %>%
  ungroup() %>%
  select(Name, Team, `2018` = war_next, `2019` = war_in_two_years, `2020` = war_in_three_years, 
