...
# Why A's Fans Should be Optimistic about Sean Manaea
...
hr_fb_model <- lm(athletics_hr_fb$`HR/FB Year N+1` ~ athletics_hr_fb$`HR/FB Year N`)

#since 2002

```

I looked at only A's starters here to somewhat adjust for park, which influences home run rate. Regardless, very little correlation is (r = 0.12). Don't expect Manaea to repeat his dinger-friendly ways in 2017.
...
ggplot(iso_by_location, aes(x=Year, y=`Avg. ISO on Swing`, color=`Pitch Location`)) + geom_line(aes(group=`Pitch Location`)) + geom_point() + scale_y_continuous(labels= function(x){
  sprintf("%.3f", x)
}) + fgt + ggtitle("Hitters Shouldn't Chase Pitches Out of the Zone") + scale_color_manual(values=c("#336699", "#50ae26"))
```
...
Don't get me wrong: Manaea isn't a world-beter. He isn't even the best pitcher on the A's staff. That honor still belongs to Sonny Gray. Manaea isn't even a #2; our Depth Chart projections [http://www.fangraphs.com/depthcharts.aspx?position=ALL&teamid=10#SP] put Jharel Cotton in that spot.
...
Scenes like these will provide A's fans for hope for the future.
