...
plate_discipline_deciles <- plate_discipline_overall %>%
  ungroup() %>%
  mutate(decile = ntile(discipline_sq_in, 10)) %>%
  group_by(decile) %>%
  summarize(from_disc = min(discipline_sq_in),
            to_disc = max(discipline_sq_in),
...
ggplot(plate_discipline_deciles, aes(reorder(as.character(decile), decile), mean_wrc_plus)) +
  geom_bar(stat="identity") + labs(x="Decile", y="Mean wRC+")
