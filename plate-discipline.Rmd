...
- VISUALIZING HITTERS' PLATE DISCIPLINE
- 
- 
- 
...
Here's another pair of hitters: Joey Gallo 2015 (AF of 33.7) and Joey Gallo 2017 (AF of 7.8):

```{r}
ggplot(savant_data %>% 
         filter(game_year %in% c(2015, 2017),
                batter_name == "Joey Gallo"), 
       aes(px_in, pz_in)) + geom_point(color="#8e001c") + facet_grid(~game_year) + 
  ggtitle("Swings from Select Hitters, 2017") + xlim(-20, 20) +ylim(0, 60) + fgt +
  labs(x="Horizontal Pitch Location (in.)", y="Vertical Pitch Location (in.)")
```

Per swing, Gallo is much less aggressive in 2017 than he was in 2015. While he still swings at eye-level pitches, now that he's playing everyday he complements that approach with more hacks at pitches around the center of the zone.
...
ggplot(least_aggressive, aes(reorder(name_season, -`Aggression Factor`), `Aggression Factor`)) + geom_bar(stat="identity", fill="#8e001c") + 
  coord_flip() + fgt + labs(x="") + ggtitle("Least Aggressive Hitter-Seasons, 2015 - 2017")
...
ggplot(most_aggressive, aes(reorder(name_season, -`Aggression Factor`), `Aggression Factor`)) + geom_bar(stat="identity", fill="#8e001c") + 
  coord_flip() + fgt + labs(x="") + ggtitle("Most Aggressive Hitter-Seasons, 2015 - 2017")
```
...
```{r}
ggplot(plate_discipline_overall, aes(aggression_factor, wrc_plus)) + geom_point(color="#8e001c") + 
...
```{r}
ggplot(year_to_year_changes_2017, aes(disc_diff, wrc_diff)) + geom_point(color="#8e001c") +
...
player_aggression_relative %>%
  ...
  geom_bar(stat="identity", fill="#8e001c") + coord_flip() + labs(x="", y="AF Multiplier when Runner on Third") + 
  fgt + ggtitle("Hitters Who Change their Approach the Least with a Runner on Third")
