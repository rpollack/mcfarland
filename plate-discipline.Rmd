...
plate_discipline_overall <- savant_data %>%
  ...
  mutate(discipline_sq_in = horiz_disc * vertical_disc,
         discipline_per_100_pitches = discipline_sq_in / num_swings * 100) %>%
...
year_to_year_changes_2017 <- plate_discipline_overall %>%
  group_by(playerid) %>%
  arrange(game_year) %>%
  mutate(disc_diff = discipline_per_100_pitches - lag(discipline_per_100_pitches),
...
plate_discipline_deciles <- plate_discipline_overall %>%
  ungroup() %>%
  mutate(decile = ntile(discipline_per_100_pitches, 10)) %>%
  group_by(decile) %>%
  summarize(from_disc = min(discipline_per_100_pitches),
            to_disc = max(discipline_per_100_pitches),
            mean_wrc_plus = mean(wrc_plus))

largest_zone_2017 <- plate_discipline_overall %>%
  filter(game_year == 2017) %>%
  arrange(desc(discipline_per_100_pitches)) %>%
  head(10)
...
```{r}
discipline_per_100_pitches_model <- lm(plate_discipline_overall$wrc_plus ~ 
                                   poly(plate_discipline_overall$discipline_per_100_pitches, 2))
summary(discipline_per_100_pitches_model)
...
```{r}
ggplot(plate_discipline_overall, aes(discipline_per_100_pitches, wrc_plus)) + geom_point() + 
...
ggplot(year_to_year_changes_2017, aes(disc_diff, wrc_diff)) + geom_point() +
  ...
  ggtitle("Expanding the Zone is Associated with a Decrease in Offense") + fgt
...
ggplot(year_to_year_changes_2017 %>% arrange(disc_diff))
...
ggplot(plate_discipline_overall, aes(as.character(game_year),
                                     discipline_per_100_pitches)) + geom_boxplot() + coord_flip() + 
  labs(x="", y="Aggression per 100 Pitches (sq. in)") + ggtitle("Hitters Are Becoming More Aggressive")
```
...
pitcher_plate_discipline_overall <- savant_data %>%
  ...
  mutate(discipline_sq_in = horiz_disc * vertical_disc,
         discipline_per_100_pitches = discipline_sq_in / num_swings * 100) %>%
  filter(num_swings >= 200) %>%    #this is about 1/6 of a full season, to reduce survivor bias
  arrange(discipline_per_100_pitches)
