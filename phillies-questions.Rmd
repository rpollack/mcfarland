...
This project was prepared by Ryan Pollack in response to question #2 listed at https://questionnaire-148920.appspot.com/qa/. Contact Ryan at ryan9379@gmail.com or 512-350-5107.
...
* The pitcher's 2016 full-season K%. Strikeout rate is among the most stable of pitching metrics. Adding this information improved the model's predictions.
...
For example, in this dataset FB% and GB% are mutually exclusive and so are highly correlated (r= -0.92). With another algorithm I'd have to choose one or the other. The xgboost algorithm helps ensure I can include both without worrying about overfitting. 

For reference, the following figure shows a correlation plot of all features I used:

```{r correlation matrix, echo=TRUE, fig.align='center}

# compute and display correlation matrix for features.
# remove features we don't care about, compute its correlation matrix, add in row names, tidy, and plot
pitchers_2017_train %>%
  ...
  gather(other_metric, r, -metric) %>% View()
...
I use the caret package to define a grid of tuning parameters for the xgboost algorithm and then perform 10-fold cross validation with each set of parameters. The final model is the one that minimizes the RMSE of the predictions.

I used the caret package to tune and train the model iteratively. I defined a set of tuning parameters, performed 10-fold cross-validation for each combination of parameters, and selected the model that minimized the RMSE of the predictions. 

This process is iterative in itself, but choosing the parameters for `parameter_grid` is iterative too. The `nrounds` and `max_depth` parameters affect the model the most, so I experiemented with several options for each. After each training run I evaluated the resulting model.
