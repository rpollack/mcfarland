...
The following code removes unimportant variables from the dataset and defines 75% of it for training/validation and 25% of it for testing/model evaluation.

```{r create train/validate and test datasets }
...
I used the caret package with the training/validation dataset to find the optimal hypermarameters for xgboost, neural network, support vector machine, k nearest neighbors, and linear model types. Choosing a variety of nonlinear models, as well as a linear one, helps ensure we find the model that provides the best possible fit.
...
```{r train and validate models, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}

# establish training characteristics
...
```{r model testing and evaluation, echo=FALSE, message=FALSE, warning=FALSE}
...
The RMSE of these predictions is `r round(RMSE(final_results$predicted_second_half_k_rate, final_results$actual_second_half_k_rate), 3)`.

NOTE: The model used to make these predictions was built on some of the data it is predicting here. This means the model is not suitable to predict values beyond the second half of 2017. But it does accomplish the stated request.

# Areas for Improvement

In the future I'd most like to build a generalized K% predictor. The request here was for one that predicting K% in a specific timeframe; a more useful program would do something like take a requested prediction year and timeframe, build an appropriate model, and make the predictions.
