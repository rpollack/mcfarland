...
xwoba_offense <- savant_data %>%
  ...
  summarize(mean_xwoba_plus = round(mean(xwoba_plus, na.rm = TRUE), 0)) %>%
...
xwoba_allowed <- savant_data %>%
  ...
  summarize(mean_xwoba_allowed_plus = round(mean(xwoba_allowed_plus, na.rm = TRUE), 0)) %>%
...
xwoba_diff <- inner_join(xwoba_allowed, xwoba_offense, by = c("fielding_team" = "batting_team", "game_year")) %>%
  mutate(xwoba_plus_diff = round(mean_xwoba_plus - mean_xwoba_allowed_plus, 5)) %>%
  select(
    team = fielding_team, game_year, mean_xwoba_plus, mean_xwoba_allowed_plus, xwoba_plus_diff, division = division.y,
