...
savant_data <- fg_db %>%
  tbl("gd_savant_new") %>%
  filter(pitch_type != "null") %>%  # games played on 3-20-2019 and 3-21-2019 were in japan & not tracked. data is bad
  select(
    batter, player_name, game_date, game_year, pitch_type, release_speed, events, description, des, plate_x, plate_z,
...
team_lookup <- fg_db %>%
  ...
  collect(n = Inf) %>%
  mutate(
    AbbName = ifelse(AbbName == "WSN", "WSH",
...
savant_data <- savant_data %>%
  ...
  mutate(
    batting_team = ifelse(inning_top_bottom == "N", home_team, away_team),
...
# davidson_plt <- ggplot(xwoba_gainers_batters %>% filter(game_year == 2017, mean_xwoba_plus >= 96,
#                                                         mean_xwoba_plus <= 98),
#                        aes(reorder(Name, -mean_xwoba_plus), mean_xwoba_plus)) + geom_bar(stat = "identity") +
#   coord_flip()
# davidson_plt
...
team_colors <- tibble(
  ...
  `team_color` = c(
    "#ED4C09", "#1C2841", "#003DA5", "#C60C30", "#9ECEEE",
...
xwoba_offense <- savant_data %>%
  filter(des != "") %>%
  #group_by(game_year) %>%
  #mutate(xwoba_plus = 100 * xwoba / mean(xwoba, na.rm = TRUE)) %>%
  #ungroup() %>%
  group_by(batting_team, game_year) %>%
  summarize(mean_xwoba = round(mean(xwoba, na.rm = TRUE), 3)) %>%
...
xwoba_offense_plt <- ggplot(xwoba_offense, aes(as.character(game_year), mean_xwoba, color = batting_team, group = 1)) + geom_point() + geom_line() + facet_wrap(~division)
...
xwoba_allowed <- savant_data %>%
  filter(des != "") %>%
  #group_by(game_year) %>%
  #mutate(xwoba_allowed_plus = 100 * xwoba / mean(xwoba, na.rm = TRUE)) %>%
  #ungroup() %>%
  group_by(fielding_team, game_year) %>%
  summarize(mean_xwoba_allowed = round(mean(xwoba, na.rm = TRUE), 3)) %>%
...
xwoba_allowed_plt <- ggplot(xwoba_allowed, aes(as.character(game_year), mean_xwoba_allowed, color = fielding_team, group = 1)) +
...
xwoba_diff <- inner_join(xwoba_allowed, xwoba_offense, by = c("fielding_team" = "batting_team", "game_year")) %>%
  mutate(xwoba_diff = round(mean_xwoba - mean_xwoba_allowed, 3)) %>%
  select(
    team = fielding_team, game_year, mean_xwoba, mean_xwoba_allowed, xwoba_diff, division = division.y,
    pct = pct.x, run_diff = run_diff.x, R_plus = R_plus.x, RA_plus = RA_plus.x
  )

xwoba_diff_plt <- ggplot(xwoba_diff, aes(as.character(game_year), xwoba_diff, color = team, group = 1)) +
  geom_point() + geom_line() + facet_wrap(~division)

ggplotly(xwoba_diff_plt)

run_diff_plt <- ggplot(xwoba_diff, aes(as.character(game_year), run_diff, color = team, group = 1)) +
  geom_point() + geom_line() + facet_wrap(~division)
...
xwoba_diff %>% 
  ...
  cor(method="spearman") %>%
  as_tibble() %>%
  bind_cols(xwoba_diff %>% ungroup() %>% select_if(is.numeric) %>% names() %>% as_tibble()) %>%
  dplyr::select(metric = value, everything()) %>%
  pivot_longer(-metric, names_to = "other_metric", values_to = "r") %>%
  filter(metric %in% c("run_diff", "pct")) %>%
  ggplot(aes(metric, other_metric, fill = r)) + geom_tile() 
...
xwoba_diff_tbl <- xwoba_diff %>%
  filter(game_year < 2017) %>%
  unite(team_year, team, game_year, sep = " ")

xwoba_diff_plt <- ggplot(xwoba_diff_tbl, aes(xwoba_plus_diff, run_diff, label = team_year)) + geom_point() + geom_smooth(method = "lm")
ggplotly(xwoba_diff_plt)
...
hard_hit_outs <- outs %>%
  ...
  summarize(
    mean_launch_speed_of_outs = mean(launch_speed, na.rm = TRUE),
...
year_diff_mean_launch_speed <- hard_hit_outs %>%
  ...
  arrange(game_year) %>%
  mutate(
    diff_mean_launch_speed_of_outs = mean_launch_speed_of_outs - lag(mean_launch_speed_of_outs),
...
ggplot(year_diff_mean_launch_speed, aes(reorder(fielding_team, diff_mad_launch_speed_of_outs), diff_mad_launch_speed_of_outs)) + geom_bar(stat = "identity") + coord_flip() + labs(x = "", y = "Change in MAD Launch Speed of Outs")
...
ggplot(outs_2016, aes(launch_speed)) + geom_histogram(bindiwth = .1) + facet_wrap(~fielding_team)
ggplot(outs_2017, aes(launch_speed)) + geom_histogram(bindiwth = .1) + facet_wrap(~fielding_team)

# ggplot(outs_2016, aes(launch_speed, fielding_team)) + geom_joy() + labs(x="Launch Speed of Fielding Out") +
#   ggtitle("Launch Speed Distributions of Outs by Team, 2016")
#
# ggplot(outs_2017, aes(launch_speed, fielding_team)) + geom_joy() + labs(x="Launch Speed of Fielding Out") +
#   ggtitle("Launch Speed Distributions of Outs by Team, 2017")
```
