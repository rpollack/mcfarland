---
title: "MLB Free Agent Analysis, 2010–2019 offseasons"
...
*   Free agent signing data, such as who was a free agent and when they signed the contract, comes from the ever-reliable Baseball-Reference.
*   Domestic free agent contract information (salaries) comes from MLB Trade Rumors. Example page   
*   International free agenty contract information comes from Spotrac. MLBTR has this data but it mixes in contract extensions Example page
...
First up is a timeline of free agent spending for each team. Hover over a bullet point to see the players signed that offseason and their salaries.
...
fa_signing_plot_total <-
  ...
  ggplot(aes(reorder(to_team, total_salary), total_salary, fill = to_team, text = players_signed)) +
  geom_col() +
  theme(legend.position = "none") +
  coord_flip() +
  scale_color_manual(values = team_colors) +
...
ggplotly(fa_signing_plot_total, tooltip = c("to_team", "num_players_signed", "total_salary", "players_signed"),
         dynamicTicks = TRUE)
...
What we noticed in 2017 and 2018 was the result 

```{r}
# median salary by player type, by year
# salary_plt <-
#   all_free_agents %>%
...
#   ggplot(aes(offseason, median_salary, text = player_names, group = 1)) +
...
# ggplotly(salary_plt, tooltip = c("offseason", "median_salary", "player_names"))
```
...
age_type_plt <- 
  ...
  group_by(offseason, age_type) %>%
  arrange(name) %>%
  summarize(median_days_to_sign = median(days_to_sign, na.rm = TRUE),
            players = paste(name, collapse = "\n")) %>%
  ggplot(aes(as.character(offseason), median_days_to_sign, color = age_type, group = age_type, label = players)) +
  fgt +
...
Older players have always taken about 10–15 days longer to sign, but the disparity jumped to about 33 days 2017. That gap formed a large part of the delay that we as fans felt.
...
ggplotly(signing_timeline, tooltip = "players") %>%
...
all_free_agents %>%
  filter(sign_type == "signed", player_type %in% c("Good Player", "All-Star", "Superstar", "MVP")) %>%
  group_by(offseason, player_type, sign_month) %>%
  summarize(players = (paste(name, collapse = "\n"))) %>%
  ggplot(aes(sign_month, y = 1, fill = player_type)) +
  geom_col() +
  facet_grid(vars(offseason))
#
#
