...
get_international_signings <- function(year) {
  ...
  read_html(glue("https://www.spotrac.com/mlb/international/{year}")) %>%
    ...
    as_tibble() %>%
    filter(Amount != '') %>%
    dplyr::select(name = 1, `To Team` = Team, sign_salary = Amount) %>%
...
extensions <-
  ...
  html_table() %>%
  as_tibble() %>%
  bind_rows(tibble(Date = "7/22/2020",
                   Player = "Mookie Betts",
                   Team = "LAD",
                   Pos = "RF",
                   Yrs = "12",
  ...
  filter(!is.na(Date)) %>%
  dplyr::select(name, offseason, sign_month, sign_date = Date, `To Team`, sign_salary, sign_years) %>%
...
all_free_agents <-
  ...
  mutate(
    ...
    sign_salary = round(sign_salary, 1),
...
extension_plt <- 
  ...
  arrange(desc(sign_salary)) %>%
  summarize(total_salary = round(sum(sign_salary), 1),
  ...
  scale_x_discrete(breaks = c("2010", "2015", "2019")) +
  ggtitle("MLB Contract Extensions by Team, 2010-2020")
...
ggplotly(top_extensions_plt, tooltip = c("sign_salary", "player_label"))
```
...
all_free_agents %>%
  ...
  labs(x = "", y = "Contract Amount (USD Millions)") +
  ggtitle("MLB Contract Extensions - Top 5 Contracts Per Team")
...
all_free_agents %>% 
  ...
  summarize(total_spending = sum(sign_salary, na.rm = TRUE),
            players = paste(name, collapse = "\n")) %>%
  ungroup() %>%
  complete(offseason, to_team, sign_type, fill = list(sign_salary = 0)) %>%
