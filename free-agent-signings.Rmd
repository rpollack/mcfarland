...
signing_timeline <-
  ...
  labs(y = "Number of Players Signed", x = "")

ggplotly(signing_timeline, tooltip = c("player_type", "players"))

all_free_agents %>%
  filter(sign_type == "signed", offseason >= 2015, position == "Position Player", player_type %in% c("Good Player", "All-Star", "Superstar", "MVP")) %>%
  ggplot(aes(sign_month, name, color = player_type)) +
  geom_point() +
  facet_grid(vars(offseason), scales = "free_y") +
  labs(y = "")
