...
fa_salaries <-
  ...
  mutate(name = str_replace_all(name, c("Jorge de la Rosa" = "Jorge De La Rosa",
...
all_free_agents <-
  ...
  mutate(three_year_war = replace_na(three_year_war, 0),
         position = case_when(
           (is.na(GS/G) | AB >= 50) ~ "Position Player",  # prevent pos. players with relief appearances from being classified as pitchers
           GS / G >= 0.5 ~ "Starting Pitcher",
           TRUE ~ "Relief Pitcher"
         ),
         position = factor(position, levels = c("Position Player", "Starting Pitcher", "Relief Pitcher")),
...
length_salary_combined <-
  ...
  filter(!is.na(sign_salary)) %>%
  group_by(offseason, player_type, age_bucket) %>%
  summarize(avg_salary = mean(sign_salary, na.rm = TRUE),
...
length_salary_combined %>%
  ...
  arrange(offseason) %>%
  mutate(salary_diff = avg_salary - lag(avg_salary),
