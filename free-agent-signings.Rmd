...
*   International free agent contract information comes from [Spotrac](www.spotrac.com).
...
fa_signing_plt <-
  all_free_agents %>%
  filter(
    sign_type %in% c("Domestic Free Agent", "International Free Agent"), !is.na(sign_salary), sign_salary != 0,
    offseason >= 2010
  ) %>%
  mutate(
...
fa_signing_plot_total <-
  all_free_agents %>%
  filter(sign_type %in% c("Domestic Free Agent", "International Free Agent"), !is.na(sign_salary), sign_salary != 0) %>%
  mutate(
    player_salary = glue("{name} - ${round(sign_salary,1)}m"),
    offseason = as.character(offseason)
  ) %>%
...
all_free_agents %>%
  ...
  geom_col() +
  facet_wrap(vars(to_team), scales = "free_y", ncol = 3) +
  coord_flip() +
...
extension_plt <-
  ...
  arrange(desc(sign_salary)) %>%
  summarize(
    total_salary = round(sum(sign_salary), 1),
  ...
  scale_x_discrete(breaks = c("2010", "2015", "2019")) +
  ggtitle("MLB Contract Extensions by Team, 2010-2019 OffSeasons")
...
ggplotly(top_5_extension_plt, tooltip = c("sign_salary"))

```

# Total Expenditure

This last graph ties it all together. You can see total dollars given out, by team and by contract time, throughout the decade. This shows you the ebb and flow of focusing on extensions vs. free agent contracts, as well as which teams are consistently major players in the international market.

Thi graph is a little tricky because while free agent contracts are categorized as being in an 'offseason', extensions are categorized as being part of a 'season'. But, they're close enough to be meaningful. 
...
all_free_agents %>%
  ...
  summarize(
    total_spending = sum(sign_salary, na.rm = TRUE),
    players = paste(name, collapse = "\n")
  ) %>%
  ungroup() %>%
  ...
  labs(x="Season", y="Total Spending (USD Millions)")
```
...
# minor league deals
all_free_agents %>%
  filter(sign_salary == 0 & sign_type == "Domestic Free Agent") %>%
  group_by(offseason, player_type, age_bucket) %>%
