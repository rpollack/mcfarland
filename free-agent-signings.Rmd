...
unsigned_batters_clean <-
  unsigned_batters %>%
  mutate(sign_date = as.Date(sign_date),
         G = as.integer(G),
         GS = as.integer(GS))
...
all_free_agents <-
  ...
  mutate(`To Team` = str_replace_all(`To Team`, 
                                     c("Florida" = "Miami",
                                       "of Anahaeim" = ""))) %>%
...
# TODO idea: concatenate player-salaries and show them as part of the labels. This way when you hover over
# a point on the plot, you see the player AND their salary in that signing year.

# TODO fix "LA Angels" matching up with "... OF Anaheim"
...
fa_signing_plt <-
  ...
  group_by(offseason, `To Team`) %>%
  summarize(num_players_signed = n(),
            total_salary = round(sum(sign_salary, na.rm = TRUE), 1),
            players_signed = paste(name, collapse = "\n")) %>%
  ggplot(aes(as.character(offseason), total_salary, group = 1, label = players_signed)) +
  geom_point() + geom_line() +
  facet_wrap(vars(`To Team`)) +
  scale_x_discrete(breaks=c("2010", "2015", "2019")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(x="", y="Total FA Contracts Signed (USD Million, Adj. for Inflation")

ggplotly(fa_signing_plt)
```
...
all_free_agents %>%
  ...
  ggplot(aes(offseason, median_sign_days)) + geom_point() + geom_line() +
  expand_limits(y=0) + fgt +
    labs(
    y = "Median # of Days 
...
signing_timeline <-
  ...
  labs(y = "Number of Players Signed", x = "Month Signed",
...
ggplotly(signing_timeline, tooltip = c("player_type", "players")) %>%
  layout(title = list(text = paste0('Ten Years of Free Agency in Baseball',
                                    '<br>',
                                    '<sup>',
...
everyone <-
  ...
  mutate(days_to_sign_diff = mean_days_to_sign - lag(mean_days_to_sign),
  ...
  mutate(time_direction = case_when(
...
everyone %>%
  mutate(g_label = case_when(
    label %in% c("Position Player, All-Star, 31 - 33",
                 "Position Player, Good Player, 28 - 30",
                 "Starting Pitcher, Solid Starter, 28 - 30") ~ label,
    TRUE ~ NA_character_
  )) %>%
  ggplot(aes(days_to_sign_diff, contract_diff)) +
  geom_point() + fgt +
  labs(x = "Difference in Average Time to Sign Contract (Days)",
