---
title: "MLB Free Agency During the 2010's"
...
get_international_signings <- function(year) {
  ...
  read_html(glue("https://www.spotrac.com/mlb/international/{year}")) %>%
    ...
    as_tibble() %>%
    filter(Amount != '') %>%
    select(name = 1, `To Team` = Team, sign_salary = Amount) %>%
    mutate(offseason = year,
           offseason = offseason - 1)  #correct for the way spotrac reports its years relative to bb-ref
}
...
intl_fa <-
  ...
  mutate(name = str_extract(name, "[A-Z][a-z]*[\\s|-][A-Z][a-z]*(\\s*[A-Z][a-z]*)*"),
         sign_salary = str_remove_all(sign_salary, c(",")),
         sign_salary = str_remove(sign_salary, "\\$"),
         sign_salary = as.numeric(sign_salary)) %>%
  filter(sign_salary >= 10000000) %>%
  mutate(sign_salary = sign_salary / 1000000) %>%
  
  # clean up some team names and correct some data inaccuracies
  mutate(`To Team` = case_when(
    `To Team` == "KC" ~ "KCR",
    `To Team` == "TB" ~ "TBR",
    `To Team` == "SD" ~ "SDP",
  ...
  bind_rows(tibble(
    name = c("Yasmany Tomas", "Kenta Maeda", "Eric Thames", "Miles Mikolas", "Byung-ho Park", "Yaisel Sierra", "Alexander Guerrero",
             "Erisbel Arruebarrena"),
             `To Team`= c("ARI", "LAD", "MIL", "STL", "MIN", "LAD", "LAD", "LAD"),
             sign_salary = c(68.5, 25, 16, 15.5, 12.85, 30, 28, 25),
             offseason = c(2014, 2015, 2016, 2017, 2015, 2015, 2013, 2013))) %>%
  mutate(sign_type = "signed_intl") %>%
  inner_join(tibble(name = c("Adeiny Hechavarria", "Adrian Morejon", "Alexander Guerrero", )))
...
extensions <-
  ...
  mutate(Date = mdy(Date),
         ...
         sign_salary = round(as.numeric(Amt), 1),
         sign_years = as.numeric(Yrs),
         sign_month = factor(month(Date), levels = c(
      "Sep", "Oct", "Nov", "Dec", "Jan", "Feb",
      "Mar", "Apr", "May", "Jun", "Jul", "Aug"
    ), ordered = TRUE)) %>%
  filter(!is.na(Date)) %>%
  dplyr::select(name, offseason, sign_month, sign_date = Date, to_team, sign_salary, sign_years) %>%
  mutate(sign_type = "extension")
...
all_free_agents <-
  ...
  mutate(
    sign_salary = case_when(
      offseason == 2010 ~ sign_salary * 1.182,
      offseason == 2011 ~ sign_salary * 1.146,
      offseason == 2012 ~ sign_salary * 1.123,
      offseason == 2013 ~ sign_salary * 1.107,
      offseason == 2014 ~ sign_salary * 1.089,
      offseason == 2015 ~ sign_salary * 1.088,
      offseason == 2016 ~ sign_salary * 1.074,
      offseason == 2017 ~ sign_salary * 1.052,
  ...
  bind_rows(tibble(name = c("Ted Lilly", "Clayton Kershaw"),
...
# fa_salaries %>% mutate(offseason = as.double(offseason), sign_years = as.double(sign_years)) %>% anti_join(all_free_agents, by=c("name", "offseason")) %>% arrange(desc(sign_salary)) %>% View()
```

# Introduction

This document contains interactive plots that analyze MLB team free agent spending during the 2010-2019 offseasons. 

# Data
The data come from three sources:

*   Free agent signing data, such as who was a free agent and when they signed the contract, comes from the ever-reliable [Baseball-Reference](www.baseball-reference.com).
*   Domestic free agent contract information (salaries) comes from [MLB Trade Rumors](www.mlbtraderumors.com).   
*   International free agent contract information comes from [Spotrac](www.spotrac.com). MLBTR has this data but it mixes in contract extensions, which I didn't want to include.

I adjusted all salaries for inflation relative to 2020 using www.usinflationcalculator.com. 
...
* Anything I missed, which is likely something given I had to combine information from three data sources :-)
...
fa_signing_plt <-
  all_free_agents %>%
  filter(sign_type %in% c("signed", "signed_intl"), !is.na(sign_salary), sign_salary != 0,
         offseason >= 2010) %>%
  mutate(
...
## Top 5 Free Agent Signings per Team
...
all_free_agents %>%
  filter(
    sign_type %in% c("signed", "signed_intl"), !is.na(sign_salary), sign_salary != 0,
    offseason >= 2010
  ) %>%
  mutate(sign_label = glue("{name} - {offseason}}")) %>%
  group_by(to_team) %>%
  top_n(5, wt = sign_salary) %>%
  ggplot(aes(reorder_within(sign_label, sign_salary, to_team), sign_salary, fill = to_team, label = age)) +
  geom_col() +
  facet_wrap(vars(to_team), scales = "free_y") +
  coord_flip() +
#  scale_y_reordered() +
...
top_signings_plt <-
  ...
  top_n(25, wt=sign_salary) %>%
  ggplot(aes(reorder(player_label, sign_salary), sign_salary, label = player_label)) +
  geom_col() + coord_flip() +
  labs(x="", y="Contract Salary (USD Millions)") +
  ggtitle("MLB Top 25 Free Agent Contracts, 2010-2019", subtitle = "Adjusted to 2020 dollars") +
  fgt

ggplotly(top_signings_plt, tooltip = c("sign_salary", "player_label"))
...
age_type_plt <- 
  ...
  filter(!is.na(age_bucket),
         offseason >= 2010,
         sign_type %in% c("signed", "signed_intl")) %>%
  ...
  arrange(name) %>%
  summarize(median_days_to_sign = median(days_to_sign, na.rm = TRUE),
...
extension_plt <- 
  all_free_agents %>%
  filter(sign_type == "extension") %>%
  mutate(label = glue("{name} - ${sign_salary}m")) %>%
  group_by(to_team, offseason) %>%
  arrange(desc(sign_salary)) %>%
  summarize(total_salary = sum(sign_salary),
...
```{r}
all_free_agents %>% 
  filter(sign_type %in% c("signed", "signed_intl", "extension")) %>%
  group_by(offseason, sign_type) %>%
  summarize(total_spending = sum(sign_salary, na.rm = TRUE)) %>%
  ungroup() %>%
  complete(offseason, sign_type, fill = list(sign_salary = 0)) %>%
  ggplot(aes(as.character(offseason), total_spending, group = sign_type, fill = sign_type)) +
