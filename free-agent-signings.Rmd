...
cumulative_sign_plt <- 
  ...
  mutate(total_signings = cumsum(num_signings),
...
temporal distribution of signings

```{r, fig.height=3, fig.width=5}
cumulative_sign_plt %>%
  ungroup() %>%
  mutate(offseason = factor(offseason)) %>%
  ggplot(aes(x = sign_month, y = offseason, group = offseason, height = num_signings)) + 
...
name_brand_fas %>%
  ...
  ggplot(aes(offseason, signed_perc)) + 
  geom_point() + geom_line() +
  ggtitle("Name-Brand Players are Signing Later",
          subtitle = "% of name-brand free agents signed through January of each offseason") +
  labs(x="Offseason", y="% of Name Brand FA's Signed", caption="Source: baseball-reference.com\nRetirees are considered unsigned FA's\nGraph by Ryan Pollack: ryanpollack.com") + fgt + theme(legend.position = "none")

# maybe bucket players into WAR3 groups (0-5, 6-11, 12-17, 18+ etc) and see which one has been hit the hardest 
