...
all_free_agents <-
  ...
  mutate(
    ...
    age_bucket = case_when(
      age <= 27 ~ "27 and under",
      age >= 28 & age <= 30 ~ "Between 28 and 30",
      age >= 31 & age <= 34 ~ "Between 31 and 33",
      age >= 35 ~ "33+"
    ),
    age_bucket = factor(age_bucket, levels = c("27 and under", "Between 28 and 30", "Between 31 and 33", "33+")),
...
money_differences <-
  all_free_agents %>%
  group_by(offseason, position, player_type, age_bucket) %>%
  summarize(average_signing_salary = round(mean(sign_salary, na.rm = TRUE), 1)) %>%
  filter(offseason %in% c(2017, 2018)) %>%
  group_by(position, player_type, age_bucket) %>%
  arrange(offseason) %>%
  mutate(
    from_to = str_c(dplyr::lag(offseason), offseason, sep = " - "),
    diff_mean_sign_salary = average_signing_salary - dplyr::lag(average_signing_salary)
  ) %>%
  filter(
    !is.na(diff_mean_sign_salary)
  ) %>%
  arrange(desc(diff_mean_sign_salary)) %>%
  dplyr::select(from_to, position, player_type, age_bucket, diff_mean_sign_salary) %>%
...
money_differences %>%
  ...
  mutate(z_time_change = (diff_mean_days_to_sign - mean(diff_mean_days_to_sign)) / sd(diff_mean_days_to_sign),
         z_money_change = (diff_mean_sign_salary - mean(diff_mean_sign_salary)) / sd(diff_mean_sign_salary)) %>%
  ggplot(aes(z_time_change, z_money_change, fill = label, color = label)) +
 geom_point() +
...
all_free_agents %>%
  ...
  ggplot(aes(offseason, avg_signing, group = 1)) +
  geom_point(size = 0.7) + geom_line() +
  ggtitle("Trends in Free Agent Signing Money") + expand_limits(y = 0) +
  labs(y = "Average Signing Salary (Millions USD)", x = "") +
  fgt
