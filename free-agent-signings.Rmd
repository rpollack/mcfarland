...
# get international signings $10m or greater from spotrac. baseball-reference doesn't list these and MLBTR mixes them in with extensions. $10m is a good indication that the player will hit the majors soon and is of note
intl_fa <-
  seq(2010, 2020) %>%
  map_df(get_international_signings) %>%
  mutate(name = str_extract(name, "[A-Z][a-z]* [A-Z][a-z]*"),
  ...
  # i've prioritized manually correcting the largest deals
  offseason = case_when(
  ...
  bind_rows(tibble(
    name = c("Yasmany Tomas", "Kenta Maeda"),
             `To Team`= c("ARI", "LAD"),
             sign_salary = c(68.5, 25),
             offseason = c(2014, 2015))) %>%
  mutate(sign_type = "signed_intl")


# TODO: match all team names up from the spotrac dataset to this one

# TODO: i get [first name] [last name] but i am missing some names. i think they are hyphenated. need to update the regex
# also Wei-Yin Chen is dorked up it gets "Yin Chen"



# this code shows folks we have salaries for that we don't have an entry in FA signings for. perhaps we can combine? 
# in many cases the signing is international; looks like bb-ref doesn't track those. need 
...
fa_signing_plot_total <-
  ...
  ggplot(aes(num_players_signed, total_salary, color = to_team, text = players_signed)) +
...
top_signings_plt <-
  ...
  ggtitle("Top 25 Free Agent Contracts, 2010-2019", subtitle = "Adjusted to 2020 dollars")
