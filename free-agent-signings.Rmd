...
all_free_agents <- 
  ...
  mutate(player_type = case_when(
    three_year_war < 3 ~ "Replacement",
    three_year_war >= 3 & three_year_war < 6 ~ "Regular",
    three_year_war >= 6 & three_year_war < 9 ~ "Recognizable",
    three_year_war >= 9 & three_year_war < 12 ~ "Name-Brand",
    three_year_war >= 12 & three_year_war < 15 ~ "Star",
    three_year_war >= 15 ~ "Superstar"))
...
cumulative_sign_plt <- 
  ...
  mutate(percent_signed_this_month = 100 * (num_signings / total_available_fas),
...
now do the above for "name-brand" players (those with at least 2 WAR per yer the prior 3 years)

for each offseason, for each player type, get the number of FA's available
make sure to zero-pad

for each offseason, for each month, for each player type, get the number of FA's signed
make sure to zero-pad
...
fa_types_annual <-
  ...
  complete(offseason, player_type = c("Replacement", "Regular", "Recognizable", "Name-Brand", "Star", "Superstar"),
...
sign_type_plt <-
  ...
  group_by(offseason, player_type) %>%
  arrange(sign_month) %>%
  mutate(cume_signed = cumsum(num_signed),
...
sign_type_plt %>%
  ...
  filter(sign_month == "Jan") %>%
  mutate(offseason = as.numeric(offseason),
         player_type = factor(player_type, levels = c("Replacement", "Recognizable", "Regular", "Name-Brand", "Star", "Superstar"))) %>%
  ggplot(aes(offseason, cume_percent_signed)) + 
  geom_point() + geom_line() +
  ggtitle("How Each Class of Players has Fared in Free Agency",
          subtitle = "% of free agents signed through January") +
  labs(x="Offseason", y="% of FA's Signed", 
