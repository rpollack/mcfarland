...
wpa_data <- fg_db %>%
  tbl("wpa_log") %>%
  filter(!is.na(PitcherSubId), str_detect(GameDate, "2018")) %>%
  collect(n = Inf)
```
...
```{r, fig.height=3}
reliever_use_plt <-
  ggplot(li_grouped, aes(first_half_K_BB_perc_plus, second_half_mean_gmLI, text = pitcher_subbed_in)) +
  geom_point() + 
  facet_wrap(vars(pitching_team)) + geom_smooth(method = "lm")
