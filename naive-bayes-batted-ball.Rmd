...
```{r}
#predict all batted balls in 2019

base_data <- 
  savant_data_full %>%
  filter(game_year == 2019)

batted_ball_predictions_2019 <-
  base_data %>%
  dplyr::select(launch_speed, launch_angle, spray_angle, sprint_speed, batter_stands, is_oracle_park, is_fenway, is_comerica, is_coors, events) %>%
  mutate(predicted_event = predict.train(batted_ball_model_caret, ., type = "raw")) %>%
...
failed_homers <-
  batted_ball_predictions_2019 %>%
  filter(events != "home_run" & predicted_event == "home_run") %>%
  group_by(batter_name) %>%
  filter(n() >= 10) %>%
  summarize(num_pred_homers_but_not = n()) %>%
  arrange(desc(num_pred_homers_but_not))

failed_homers %>%
  head(20) %>% 
  ggplot(aes(reorder(batter_name, num_pred_homers_but_not), num_pred_homers_but_not)) +
  geom_col() + coord_flip() +
  labs(x="", y = "Number of Predicted Home Runs that Weren't")

failed_homers %>%
  ggplot(aes(num_pred_homers_but_not)) + geom_histogram(bins = 10)

bases_lost <-
  batted_ball_predictions_2019 %>%
  mutate(actual_bases = case_when(
    events == "field_out" ~ 0,
    events == "single" ~ 1,
    events == "double" ~ 2,
    events == "triple" ~ 3,
  ...
  predicted_bases = case_when(
    predicted_event == "field_out" ~ 0,
    predicted_event == "single" ~ 1,
    predicted_event == "double" ~ 2,
    predicted_event == "triple" ~ 3,
    predicted_event == "home_run" ~ 4
  ),
  bases_lost = predicted_bases - actual_bases) %>%
  group_by(batter_name) %>%
  filter(n() >= 350) %>%
  summarize(total_bases_lost = sum(bases_lost)) %>%
  arrange(desc(total_bases_lost))
