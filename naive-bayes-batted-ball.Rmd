...
sprint_speed <-
  ...
  bind_rows(read_csv("Sprint Speed - 2019.csv")) %>%
...
savant_data_full <-
  ...
  tbl("gd_savant_new") %>%
  filter(
    events %in% c("home_run", "triple", "double", "single", "field_out"),
    launch_speed != "null",
    launch_angle != "null"
  ) %>%
  dplyr::select(
    batter, pitcher, game_year, game_date, hc_x, hc_y, launch_speed,
  ...
  mutate(
    events = factor(events, levels = c(
      "field_out", "single", "double",
      "triple", "home_run"
    )), # factorize data so RF model can be built
    ...
    batter_stands = factor(stand),
    game_month = factor(month(game_date)),
    is_oracle_park = if_else(park == "SF", 1, 0),
    is_coors = if_else(park == "DET", 1, 0),
    is_fenway = if_else(park == "BOS", 1, 0),
    is_comerica = if_else(park == "DET", 1, 0)
  ) %>%
  dplyr::select(
    playerid, batter_name,
    batter_stands, sprint_speed, park, defending_team, inning_top_bottom, game_date, game_month, game_year, pitcher, hc_x, hc_y, launch_speed, launch_angle, spray_angle, is_oracle_park, is_coors, is_fenway, is_comerica, events
  )
```
...
data_grouped <-
  ...
  mutate(
    ...
    outcome = if_else(str_detect(description, "strike") | 
                        (str_detect(description, "hit_into_play") & xwoba <= median_xwoba_2019), "good", "bad") # strikes of any kind are good
  ) %>%
...
# plt <-
#   ggplot(data_grouped, aes(pitch_percentage, mean_xwoba)) +
#   geom_point() +
#   facet_wrap(vars(p_throws, pitch_type, zone_desc, stand), scales = "free_y")
# 
# ggplotly(plt)

# pitchers with the most and least good outcomes
pitch_cutoff <- 10
...
data_grouped %>%
 ...
 # filter(total_pitches < 500) %>% # get starters
  ...
  mutate(good_pitch_percentage = round(num_good_pitches / total_pitches * 100, 1))
  

#percentage_cutoff <- 15
...
#interesting but ... should actually compare to that pitcher's own repertoire against batters of that handedness
# since every pitcher is going to be different unto themselves
...
sample_rows <- sample(nrow(savant_data_full), nrow(savant_data_full) * .75)

savant_data_train <- savant_data_full %>%
...
savant_data_test <- savant_data_full %>%
  filter(!row_number() %in% sample_rows)
...
savant_data_full_predicted <- savant_data_full
...
full_prediction <- matrix(full_pred,
  nrow = 5,
  ...
  mutate(predicted_event = str_replace_all(
    predicted_event,
    c(
      "1" = "field_out",
      "2" = "single",
...
# cut unneeded factors from data; retain only factors used for predicting outcome
savant_data <-
  savant_data_full %>%
  dplyr::select(launch_speed, launch_angle, spray_angle, sprint_speed, batter_stands, is_oracle_park, is_fenway, is_comerica, is_coors, events)

# split data into 75/25 train and test sets
batted_ball_test <-
  createDataPartition(savant_data$events, p = 0.25)
...
# TODO: adding in hit_distance_sc might be a useful predictor
```
# evaluate model events by applying the model to test dataset



```{r}
batted_ball_predicted <-
  savant_data_test %>%
  mutate(predicted_event = predict.train(batted_ball_model_caret, ., type = "raw"))

confusionMatrix(batted_ball_predicted$events, batted_ball_predicted$predicted_event, mode = "prec_recall")
```

use model to predict ALL batted balls in dataset; find interesting patterns
...
correct_predictions_batted_ball_type <- predictions %>%
  ...
  summarize(num_correct_predictions = sum(correct_prediction)) %>%
  mutate(
    correct_predict_percentage = 100 * (num_correct_predictions / num_events),
...
ggplot(correct_predictions_batted_ball_type, aes(
  reorder(events, -correct_predict_percentage),
