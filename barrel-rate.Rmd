...
barrel_data_batters <- savant_data_clean %>%
  select(playerid, batter_name = Name, game_year, stand, barrel) %>%
  group_by(playerid, batter_name, game_year) %>%
  ...
  filter(PA >= 80) #remove pitchers, mostly
...
ggplot(barrel_data_batters, aes(barrel_rate)) + geom_histogram(binwidth = .01, aes(y=..density..)) +
  stat_function(fun = dbeta,
                ...
                lwd=2,
                col="red") +
  ggtitle("Probability Distribution for Barrel Rates, 2015 – 2017")

```

#estimate posterior based on prior + evidence

next step: need to weight prior years at .6 and .3

and then need to add handedness to priors

```{r}
barrel_data_batters_weighted <- barrel_data_batters %>%
  group_by(playerid) %>%
  arrange(playerid, game_year) %>%
  
  #project barrel rate by regressing to mean + weighting past 2 years at .6 and .3, respectively.
  #for missing years, assume average performance: alpha0 / (alpha0 + beta0)
  mutate(est_barrels_this_year = num_barrels + alpha0,
         est_barrels_last_year = ifelse(is.na(lag(num_barrels)), alpha0, .6 * lag(num_barrels) + alpha0),
         est_barrels_two_years_ago = ifelse(is.na(lag(num_barrels, 1)), alpha0, .3 * lag(num_barrels, 1) + alpha0),
         ...
         est_barrel_rate_weighted = (est_barrels_this_year + est_barrels_last_year + 
                                       est_barrels_two_years_ago) / (est_pa_this_year + est_pa_last_year +
...
ggplot(barrels_weighted_2017, aes(reorder(batter_name, est_barrel_rate_weighted), est_barrel_rate_weighted)) +
  ...
  coord_flip() +
  labs(y="Est. Barrel Rate, Weighted, w/95% Credible Interval", x="")
