...
barrels_combined_2017_only <- barrels_combined %>%
  filter(lastYear == 2017,
         PA >= 525) %>%
  arrange(desc(barrels_per_pa))

ggplot(barrels_combined_2017_only %>% head(25), aes(reorder(Name, barrels_per_pa), barrels_per_pa)) +
...
barrels_accounting_for_pa <- barrels_combined %>%
  ...
  filter(lastYear == 2017) %>%  #all players are used to compute priors, but plot & rank only active players
...
sprintf("andujar 95% credible interval: %s", quantile(judge_sim$value, c(.025, .975))
)
