...
ggplot(barrels_accounting_for_pa %>% head(25), aes(est_barrel_rate, reorder(Name, est_barrel_rate))) +
  ...
  geom_errorbarh(aes(xmin = low_credible, xmax = high_credible)) +
  labs(y="", x="Estimated Barrels / PA with 95% Credible Interval") +
  ggtitle("Estimated Barrel Rate Leaders, 2015 – 2017") + fgt +
...
ggplot() + geom_histogram(binwidth = 0.009, aes(y=..density..)) +
  stat_function(fun = dbeta,
                args=list(shape1 = 41.8521, 480.5778),
                lwd=1.5,
  ...
  ggtitle("Barrel Rate: Matt Davidson vs. Aaron Judge") +
  scale_color_brewer(palette="Set1", direction = -1) +
  labs(color = "Posterior Distributions") + fgt +
  ggsave("davidson-judge-posterior.png",  width=9.92, height=6.19, units="in")


judge_sim <- rbeta(1e6, 95.13189 ,757.2989)
davidson_sim <- rbeta(1e6, 41.8521, 480.5778)
mean(judge_sim > davidson_sim)
#60% chance judge' rate is greater than stanton's rate

#integration
x <- seq(.05, .15, .001)
crossing(judge_x = x, davidson_x = x) %>%
  mutate(judge_density = dbeta(judge_x, 95.13189, 757.2989),
         davidson_density = dbeta(davidson_x, 41.8521, 480.5778),
         joint = judge_density * davidson_density) %>%
  ggplot(aes(judge_x, davidson_x, fill = joint)) +
  geom_tile() +
  geom_abline() +
  scale_fill_gradient2(low = "white", high = "red") +
  labs(x = "Judge Barrel Rate",
       y = "Davidson Barrel Rate",
       fill = "Joint density") +
  #theme(legend.position = "none")
  fgt +
  ggtitle("Joint Distribution of Barrel Rates") +
  ggsave("joint-distro-davidson-judge.png", width=9.92, height=6.19, units="in")
