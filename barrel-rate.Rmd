...
barrel_data_batters <- savant_data_clean %>%
  group_by(PlayerId, Name, game_year, PA) %>%
...
fit <- gamlss(cbind(barrels, PA - barrels) ~ log(PA),
              data = barrel_data_batters_unweighted,
...
barrels_accounting_for_pa <- barrel_data_batters_unweighted %>%
  ungroup() %>%
  ...
  mutate(mu = mu,
         ...
         est_barrel_rate = round(100 * (alpha1 / (alpha1 + beta1)), 1),
...
barrel_rate_priors <- crossing(x = seq(0, .2, .001), PA = c(100, 250, 500, 750, 1000, 1500)) %>%
...
ggplot(barrel_rate_priors, aes(x, density, color = PA, group = PA)) +
  ...
  ggtitle("Barrel Rate Probability Density Curves, Based on PA")

# plot barrel rate leaders & laggards based on expected rate. also show credible intervals
ggplot(barrel_data_batters_unweighted %>% head(25), aes(est_barrel_rate, reorder(Name, est_barrel_rate))) +
  geom_point() +
...
ggplot(barrel_data_batters_unweighted %>% tail(25), aes(est_barrel_rate, reorder(Name, est_barrel_rate))) +
  geom_point() +
  geom_errorbarh(aes(xmin = low_credible, xmax = high_credible)) +
  labs(y="", x="Estimated Barrels / PA, with 95% Credible Interval") +
