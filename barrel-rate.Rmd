...
barrels_combined <- bind_rows(barrels_2015, barrels_2016, barrels_2017) %>%
  group_by(Player) %>%
  summarize(barrels = sum(Barrels)) %>%
  inner_join(pa_totals, by=c("Player" = "Name")) %>%
  dplyr::select(Name = Player, barrels, PA)
```
...
barrels_accounting_for_pa <- barrels_combined %>%
  ...
  mutate(mu = mu,
         ...
         percentile_mlb_barrel_rate_rank = round(percent_rank(est_barrel_rate) * 100, 1)) %>%
...
ggplot(barrels_accounting_for_pa %>% head(30), aes(est_barrel_rate, reorder(Name, est_barrel_rate))) +
  geom_point() +
  geom_errorbarh(aes(xmin = low_credible, xmax = high_credible)) +
  labs(y="", x="Estimated Barrels / PA with 95% Credible Interval") +
  ggtitle("Estimated Barrel Rate Leaders, 2015 – 2017: Top 30") + fgt
