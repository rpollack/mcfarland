...
#find batters who have batted since 2015 but who are not in gd_roster
# not_found <- anti_join(fg_info, gd_info, by=c("PlayerId" = "playerid")) %>%
#   inner_join(batter_info, c("PlayerId" = "playerid")) %>%
#   group_by(PlayerId, batter_name) %>%
#   distinct(PlayerId, batter_name, BISPosition) %>%
#   write_csv("no-entry-in-gd-roster-table.csv")
