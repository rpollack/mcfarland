...
barrels_2016 <- suppressMessages(read_csv("~/Downloads/barrels - 2016.csv")) %>%
  mutate(Season = 2016)
barrels_2017 <- suppressMessages(read_csv("~/Downloads/barrels - 2017.csv")) %>%
  mutate(Season = 2017)
...
barrels_combined <- barrels_seasonal %>%
  ...
  summarize(barrels = sum(Barrels),
            PA = sum(PA)) %>%
  filter(PA >= 3) %>%
  dplyr::select(Name = Player, lastYear, barrels, PA)
```
...
# mancini_plt <- ggplot(barrels_accounting_for_pa %>% filter(percentile_mlb_barrel_rate_rank > 96), 
#        aes(est_barrel_rate, reorder(Name, est_barrel_rate))) +
#   geom_point() +
#   geom_errorbarh(aes(xmin = low_credible, xmax = high_credible)) +
#   labs(y="", x="Estimated Barrels / PA with 95% Credible Interval") +
#   ggtitle("Estimated Barrel Rates 2015 – 2017, Top 5% of MLB Players") + fgt
# mancini_plt
